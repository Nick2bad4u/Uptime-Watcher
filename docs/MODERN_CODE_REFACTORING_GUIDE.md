 ...
 persist(
  (set, get) => ({
   ...
   getSelectedMonitorId: (siteId) => {
    ...
    return Object.prototype.hasOwnProperty.call(ids, siteId) ? ids[siteId] : undefined;
   },

   getSelectedSite: () => {
    const { selectedSiteId, sites } = get();
    return selectedSiteId ? sites.find((s) => s.identifier === selectedSiteId) : undefined;
   },
...
export const useSiteOperations = () => {
 ...
 const deleteSiteOperation = useAsyncOperation(async (identifier: string) => {
  clearError();
  ...
  if (site) {
   await Promise.all(
    site.monitors.map((monitor) =>
     window.electronAPI.monitoring.stopMonitoringForSite(identifier, monitor.id).catch((err) => {
 ...
 const syncSitesOperation = useAsyncOperation(async () => {
 ...
 const createSite = useCallback(
  async (siteData: Parameters<typeof createSiteOperation.execute>[0]) => {
   try {
    setLoading(true);
...
const addSiteSchema = z
 .discriminatedUnion("addMode", [
  z.object({
 ...
 const onSubmit = handleSubmit(async (data) => {
  try {
   ...
   if (data.addMode === "new") {
    ...
    await window.electronAPI.sites.addMonitorToSite(data.selectedExistingSite, monitor);
   }
   ...
   <ThemedBox>
    <div className="flex items-center justify-between">
     <div>
      ...
      {siteStats.averageResponseTime && <p className="text-xs text-gray-500">Avg: {siteStats.averageResponseTime}ms</p>}
     </div>

     <div className="flex gap-2">
      <ThemedButton onClick={handleViewDetails} size="sm" variant="secondary">
       View Details
      </ThemedButton>

      <ThemedButton onClick={handleDelete} size="sm" variant="danger">
       Delete
      </ThemedButton>
     </div>
    </div>
...
export default defineConfig(({ command, mode }) => {
 ...
 return {
   ...
   electron([
    {
     ...
     vite: {
      build: {
       ...
       outDir: "dist-electron",
       rollupOptions: {
        external: ["electron", "node-sqlite3-wasm", "electron-log", "electron-updater"],
       },
      },
      define: {
  ...
  // Optimization
  optimizeDeps: {
   include: ["react", "react-dom", "zustand", "react-hook-form", "@hookform/resolvers/zod", "zod"],
   exclude: ["electron"],
  },
