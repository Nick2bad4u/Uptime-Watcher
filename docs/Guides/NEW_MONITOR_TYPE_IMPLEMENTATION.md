...
* __Shared__ (Declare monitor type, config, validation)
...
* __Electron backend__ (Service logic, registry metadata, retry policies)
...
* __Persistence__ (Persist dynamic fields and history)
...
* __Renderer__ (Collect config, validate, display)
...
* __Monitor type missing from dropdowns__
...
* __Validation errors differ between backend and frontend__
...
* __Database errors on startup__
...
* __Monitor never runs in production__
