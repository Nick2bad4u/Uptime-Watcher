...
doc_title: "Renderer Integration Guide"
...
| Task                                                            | Status | Notes                                                                   |
| --------------------------------------------------------------- | ------ | ----------------------------------------------------------------------- |
| Normalize all invoke channels and preload bridges               | Done   | Run `npm run generate:ipc` after refactors.                             |
| Update renderer services (`EventsService`, `MonitoringService`) | Done   | Ensure new listeners return cleanup functions.                          |
| Wire optimistic manual-check handling into stores               | Done   | Verify `createSiteMonitoringActions` is in use.                         |
| Handle `settings:history-limit-updated` in all settings views   | Done   | Remember to invalidate cached retention copy in Zustand store/services. |
| Add CI drift detection (`npm run check:ipc`)                    | Done   | Integrate into lint/test workflows.                                     |
| Update product docs & release notes                             | Done   | Reference this guide and the changelog entry.                           |
