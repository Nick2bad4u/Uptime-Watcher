 ...
 return async <T>(
  ...
  for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
   try {
    ...
    const delay = options.backoff === "exponential" ? options.delay * Math.pow(2, attempt - 1) : options.delay;
...
export class TypedEventBus<EventMap extends Record<string, any>> extends EventEmitter {
 ...
 onTyped<K extends keyof EventMap>(event: K, listener: (data: EventMap[K]) => void): this {
...
export const useErrorHandling = () => {
 return {
  withErrorContext: async <T>(operation: string, fn: (correlationId: string) => Promise<T>): Promise<T> => {
   const correlationId = generateCorrelationId();
...
export const useCache = <T extends (...args: any[]) => Promise<any>>(fn: T, options: CacheOptions = {}) => {
...
export class SiteRepository {
 ...
 private findByIdCached = useCache(this.findById.bind(this), {
  ttl: 5 * 60 * 1000,
  keyGenerator: (id) => `site:${id}`,
 });
