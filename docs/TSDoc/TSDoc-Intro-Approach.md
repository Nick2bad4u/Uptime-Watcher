\<!doctype html\>
<html lang="en" dir="ltr" class="docs-wrapper docs-doc-page docs-version-current plugin-docs plugin-id-default docs-doc-id-pages/intro/approach">
<head>
<meta charset="UTF-8">
...
<meta data-rh="true" property="og:url" content="https://tsdoc.org/pages/intro/approach/">
...
<meta data-rh="true" property="og:title" content="Approach | TSDoc">
<meta data-rh="true" name="description" content="The TSDoc specification aims to meet these requirements:">
<meta data-rh="true" property="og:description" content="The TSDoc specification aims to meet these requirements:">

`<link data-rh="true" rel="icon" href="/images/site/favicon.ico">`{=html}`<link data-rh="true" rel="canonical" href="https://tsdoc.org/pages/intro/approach/">`{=html}`<link data-rh="true" rel="alternate" href="https://tsdoc.org/pages/intro/approach/" hreflang="en">`{=html}`<link data-rh="true" rel="alternate" href="https://tsdoc.org/pages/intro/approach/" hreflang="x-default">`{=html}`<link rel="search" type="application/opensearchdescription+xml" title="TSDoc" href="/opensearch.xml">`{=html}
...
<li class="theme-doc-sidebar-item-link theme-doc-sidebar-item-link-level-2 menu__list-item">
`<a class="menu__link menu__link--active" aria-current="page" tabindex="0" href="/pages/intro/approach/">`{=html}Approach`</a>`{=html}
</li>
<li class="theme-doc-sidebar-item-link theme-doc-sidebar-item-link-level-2 menu__list-item">
...
<ul>
<li>
`<strong>`{=html}Designed for TypeScript`</strong>`{=html}: ...while
aligning as closely as possible with the familiar JSDoc notations we
know and love.
</li>
<li>
`<strong>`{=html}Markdown integration`</strong>`{=html}: Doc comments
may incorporate
`<a href="http://commonmark.org" target="_blank" rel="noopener noreferrer">`{=html}CommonMark`</a>`{=html}
notations for rich text elements such as boldface, code fences,
headings, tables, etc. (This turned out to be the toughest requirement,
since the Markdown grammar is very irregular and very dependent on
context.) TSDoc makes special accommodations for entrenched pipelines
(e.g. GitHub Pages) that expect to bring their own Markdown renderer to
the party.
</li>
<li>
`<strong>`{=html}Common core`</strong>`{=html}: Common tags such as
`<code>`{=html}@param`</code>`{=html} and
`<code>`{=html}@returns`</code>`{=html} will have consistent behavior
across all tools.
</li>
<li>
`<strong>`{=html}Extensible`</strong>`{=html}: Each tool can supplement
the core tags with custom tags for specialized scenarios.
</li>
<li>
`<strong>`{=html}Interoperable`</strong>`{=html}: The TSDoc standard
guarantees that unsupported custom tags won\'t interfere with parsing of
other content. TSDoc also eliminates Markdown ambiguities. (Are
backticks allowed inside a `<code>`{=html}{@link}`</code>`{=html} tag?
What happens if there is only one backtick? etc.)
</li>
<li>
`<strong>`{=html}Multi-package support`</strong>`{=html}: Many teams
ship a collection of NPM packages that work together and are documented
as a set. The cross-referencing syntax
(e.g. `<code>`{=html}{@link}`</code>`{=html} or
`<code>`{=html}{@inheritDoc}`</code>`{=html}) needs a portable way to
reference API items imported from other packages. We also define
`<em>`{=html}package.json`</em>`{=html} metadata that enables tooling to
detect whether a dependency\'s `<!-- -->`{=html}\*`<!-- -->`{=html}.d.ts
doc comments should be parsed as TSDoc or not.
</li>
<li>
`<strong>`{=html}Open standard`</strong>`{=html}: TSDoc is an open
source, community-driven standard. You are encouraged to contribute your
own ideas and pull requests.
</li>
</ul>
<p>
The `<strong>`{=html}@microsoft/tsdoc`</strong>`{=html} library package
brings in some additional goals:
</p>
<ul>
<li>
`<strong>`{=html}\"Strict\" and \"Lax\" modes`</strong>`{=html}: Many
projects don't have the time/desire to change their existing code, so
they want a \"`<em>`{=html}lax`</em>`{=html}\" mode that makes a best
attempt to render their doc comments as-is. Other projects want a
\"`<em>`{=html}strict`</em>`{=html}\" mode that ensures consistent
syntax everywhere and catches typos that might result in rendering
errors. Some projects want to be \"`<em>`{=html}strict`</em>`{=html}\"
eventually, but they can\'t migrate everything overnight; they need a
\"`<em>`{=html}transitional`</em>`{=html}\" mode similar to tslint
suppressions.
</li>
<li>
`<strong>`{=html}Comment-emitter for roundtripping`</strong>`{=html}:
The parser reads doc comments as input and produces an abstract syntax
tree (AST) as output. This should be reversible: given an AST input
(possibly with modifications), the library can regenerate the
corresponding doc comment.
</li>
<li>
`<strong>`{=html}Self-contained`</strong>`{=html}: The implementation
will be small, fast, and self-contained. It will not have a dependency
on the TypeScript compiler API. Doc comments will be received as a plain
text string, and the AST will be a simple JavaScript tree object. This
makes it easier for tools to accept
`<strong>`{=html}@microsoft/tsdoc`</strong>`{=html} as a dependency.
</li>
</ul>
:::
...
`<a class="pagination-nav__link pagination-nav__link--prev" href="/">`{=html}
...
`</a>`{=html}`<a class="pagination-nav__link pagination-nav__link--next" href="/pages/intro/using_tsdoc/">`{=html}
