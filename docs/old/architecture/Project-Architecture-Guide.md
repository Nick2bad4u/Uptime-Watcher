...
// Store usage pattern
const { sites, createSite, setError, isLoading } = useStore();
```
...
const subscribeToStatusUpdates = (callback) => {
 window.electronAPI.onStatusUpdate((update) => {
  ...
  const updatedSites = state.sites.map(
   (site) =>
...
class MonitorScheduler {
 async startMonitoring(): Promise<void> {
  ...
  await Promise.allSettled(
   sites.flatMap((site) => site.monitors.map((monitor) => this.startMonitoringForMonitor(monitor)))
  );
 }
}
```
