 ...
 * chains, correlation tracking, bulk event dispatching, subscription management,
 * and concurrent event handling that form the backbone of the application's
 * reactive architecture.
 *
 * Covers event emission, filtering, transformation, error handling, and
 * complex event processing scenarios with high throughput requirements.
 *
...
type EventType = SiteStatusChangedEvent | MonitorCheckCompletedEvent | AlertTriggeredEvent | DatabaseOperationEvent;

// Event handler types
type EventHandler<T extends BaseEvent = BaseEvent> = (event: T) => Promise<void> | void;
type EventMiddleware<T extends BaseEvent = BaseEvent> = (event: T, next: () => Promise<void>) => Promise<void>;
...
class MockTypedEventBus {
    ...
    async emit<T extends BaseEvent>(event: T): Promise<void> {
        ...
        try {
            ...
            if (event.correlationId) {
                const correlated = this.correlationTracker.get(event.correlationId) ?? [];
            ...
            this.processingStats.middlewareExecutionTime += performance.now() - middlewareStartTime;

            // Execute handlers
            const handlerStartTime = performance.now();
            await this.executeHandlers(event);
            this.processingStats.handlerExecutionTime += performance.now() - handlerStartTime;
    ...
    bulkUnsubscribe(subscriptionIds: string[]): number {
        return subscriptionIds.reduce((count, id) => this.unsubscribe(id) ? count + 1 : count, 0);
    }
    ...
    analyzeEventCorrelations(correlationId: string): {
        ...
        const timeline = sortedEvents.map((event, index) => ({
            ...
            gap: index > 0 ? event.timestamp - sortedEvents[index - 1].timestamp : 0
        }));

        const totalDuration = sortedEvents.length > 1 ?
            sortedEvents.at(-1)!.timestamp - sortedEvents[0].timestamp : 0;
    ...
    getActiveSubscriptions(): Map<string, number> {
        ...
        for (const [eventType, subscriptions] of this.subscriptions) {
            counts.set(eventType, subscriptions.filter(sub => sub.enabled).length);
        }
        return counts;
    }
    ...
    private async executeHandlers(event: BaseEvent): Promise<void> {
        ...
        const handlerPromises = enabledSubscriptions.map(async subscription => {
            try {
    ...
    private updateProcessingStats(processingTime: number): void {
        ...
        stats.averageProcessingTime = (
            (stats.averageProcessingTime * (stats.processedEvents - 1)) + processingTime
...
function generateSiteStatusEvents(count: number, siteIds: string[]): SiteStatusChangedEvent[] {
    ...
    for (let i = 0; i < count; i++) {
        ...
        events.push({
            ...
            correlationId: Math.random() > 0.7 ? `corr-${Math.floor(i / 5)}` : undefined,
            payload: {
                siteId,
                previousStatus,
                currentStatus,
...
function generateMonitorCheckEvents(count: number, siteIds: string[]): MonitorCheckCompletedEvent[] {
    ...
    for (let i = 0; i < count; i++) {
        ...
        events.push({
            ...
            correlationId: Math.random() > 0.8 ? `check-corr-${Math.floor(i / 3)}` : undefined,
            payload: {
...
function generateAlertEvents(count: number, siteIds: string[]): AlertTriggeredEvent[] {
    const events: AlertTriggeredEvent[] = [];
    const severities = ["low", "medium", "high", "critical"] as const;

    for (let i = 0; i < count; i++) {
        const severity = severities[Math.floor(Math.random() * severities.length)];
        const affectedSites = siteIds.slice(0, Math.floor(Math.random() * 3) + 1);
...
function generateDatabaseEvents(count: number): DatabaseOperationEvent[] {
    ...
    const tables = ["sites", "monitors", "status_history", "alerts", "settings"];
    ...
    for (let i = 0; i < count; i++) {
        ...
        events.push({
            ...
            correlationId: Math.random() > 0.6 ? `db-corr-${Math.floor(i / 4)}` : undefined,
            payload: {
                operation,
                table,
...
describe("TypedEventBus and Event Processing Benchmarks", () => {
    ...
    describe("Subscription Management", () => {
        bench("Subscribe to single event type", () => {
            eventBus.subscribe("site-status-changed", async () => {
                // Simple handler
            });
        }, { iterations: 1000 });

        bench("Bulk subscription - 50 handlers", () => {
        ...
        bench("Bulk subscription - mixed event types", () => {
            const eventTypes = ["site-status-changed", "monitor-check-completed", "alert-triggered", "database-operation"];
        ...
        bench("Subscribe and unsubscribe cycle", () => {
            const subscriptionId = eventBus.subscribe("site-status-changed", async () => {});
            eventBus.unsubscribe(subscriptionId);
        }, { iterations: 500 });
    });

    describe("Middleware Processing", () => {
        bench("Single middleware - simple processing", async () => {
        ...
        bench("Multiple middleware - complex chain", async () => {
            // Logging middleware
            eventBus.use(async (event, next) => {
                const startTime = performance.now();
                await next();
                const duration = performance.now() - startTime;
                event.metadata = { ...event.metadata, processingTime: duration };
            });
        ...
        bench("Heavy middleware processing", async () => {
    ...
    describe("Concurrent Event Processing", () => {
        bench("Parallel processing - 50 events", async () => {
            const events = generateMixedEvents(50, siteIds);
            await eventBus.emitConcurrent(events, "parallel");
        }, { iterations: 50 });

        bench("Sequential processing - 50 events", async () => {
            const events = generateMixedEvents(50, siteIds);
            await eventBus.emitConcurrent(events, "sequential");
        }, { iterations: 30 });

        bench("Batch processing - 100 events, batch size 10", async () => {
            const events = generateMixedEvents(100, siteIds);
            await eventBus.emitConcurrent(events, "batch", 10);
        }, { iterations: 20 });

        bench("High-throughput processing - 500 events", async () => {
    ...
    describe("Event Filtering and Transformation", () => {
        bench("Filter critical events only", async () => {
            const events = generateMixedEvents(100, siteIds);
        ...
        bench("Transform and enrich events", async () => {
            const events = generateMixedEvents(50, siteIds);
        ...
        bench("Complex filtering with multiple conditions", async () => {
            const events = generateMixedEvents(200, siteIds);
            ...
            await eventBus.processFilteredEvents(
                ...
                event => {
                    ...
                    const isHighPriority =
                        (event.type === "alert-triggered" && (event as AlertTriggeredEvent).payload.severity === "high") ||
                        (event.type === "site-status-changed" && (event as SiteStatusChangedEvent).payload.currentStatus === "down");
    ...
    describe("Event Replay and History Management", () => {
        bench("Replay last hour events", async () => {
            // Populate history
        ...
        bench("Replay with event type filter", async () => {
            // Populate history
            ...
            await eventBus.replayEvents(sixHoursAgo, Date.now(), "alert-triggered");
        }, { iterations: 15 });

        bench("Event history analysis", () => {
            // Populate history
            ...
            const correlatedEvents = history.filter(event => event.correlationId);
            const recentEvents = history.filter(event =>
                Date.now() - event.timestamp < 1_800_000 // Last 30 minutes
            );
        }, { iterations: 50 });
    });

    describe("Event Correlation and Analysis", () => {
        bench("Correlation tracking and analysis", async () => {
            // Generate correlated events
            const correlationId = "test-correlation-1";
            const correlatedEvents = generateMixedEvents(20, siteIds).map(event => ({
                ...event,
                correlationId
            }));
            ...
            const analysis = eventBus.analyzeEventCorrelations(correlationId);
            ...
            const averageGap = analysis.timeline.reduce((sum, item) => sum + item.gap, 0) /
                (analysis.timeline.length - 1 || 1);
        }, { iterations: 100 });

        bench("Multiple correlation analysis", async () => {
            // Generate multiple correlation chains
            const correlationIds = ["corr-1", "corr-2", "corr-3", "corr-4", "corr-5"];

            for (const corrId of correlationIds) {
                const events = generateMixedEvents(15, siteIds).map(event => ({
                    ...event,
            ...
            // Aggregate analysis
            const totalEvents = analyses.reduce((sum, analysis) => sum + analysis.eventCount, 0);
            const averageDuration = analyses.reduce((sum, analysis) => sum + analysis.totalDuration, 0) /
                analyses.length;
        }, { iterations: 20 });
    });

    describe("High-Load Stress Testing", () => {
        bench("High-frequency event emission - 1000 events", async () => {
            const events = generateMixedEvents(1000, siteIds);
            await eventBus.emitConcurrent(events, "parallel");
        }, { iterations: 3 });

        bench("Complex handlers with processing", async () => {
            // Setup complex handlers
            eventBus.subscribe("site-status-changed", async (event) => {
            ...
            eventBus.subscribe("monitor-check-completed", async (event) => {
                // Simulate database write
                await new Promise(resolve => setTimeout(resolve, Math.random() * 10));

                // Type-safe check for monitor event
                if (event.type === "monitor-check-completed") {
                    const monitorEvent = event as MonitorCheckCompletedEvent;
        ...
        bench("Memory intensive event processing", async () => {
            // Create many subscriptions
            const subscriptionIds: string[] = [];
            for (let i = 0; i < 200; i++) {
                const id = eventBus.subscribe("site-status-changed", async (event) => {
        ...
        bench("Event system performance under load", async () => {
            ...
            eventBus.use(async (event, next) => {
                event.metadata = { ...event.metadata, middleware2: Date.now() };
                await next();
            });
            ...
            for (const eventType of eventTypes) {
                for (let i = 0; i < 10; i++) {
                    const id = eventBus.subscribe(eventType, async () => {
