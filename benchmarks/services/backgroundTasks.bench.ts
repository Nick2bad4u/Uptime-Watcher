...
class MockTaskQueue {
    ...
    async dequeue(workerCapabilities?: string[]): Promise<Task | null> {
        ...
        // Find eligible tasks
        let eligibleTasks = Array.from(this.tasks.values()).filter(
        ...
        return {
            ...
            averageLoad:
                activeWorkers.length > 0
                    ? activeWorkers.reduce((sum, w) => sum + w.currentLoad, 0) /
    ...
    private async processQueue(): Promise<void> {
        ...
        try {
            ...
            for (const worker of availableWorkers) {
                ...
                const assigned = await this.workerPool.assignTask(
                    worker.id,
                    task.id
                );
    ...
    private async executeTask(task: Task, worker: Worker): Promise<void> {
        ...
        if (!processor) {
            await this.queue.fail(
                task.id,
                `No processor registered for task type: ${task.type}`
            );
...
): (payload: any, context: TaskExecutionContext) => Promise<any> {
...
describe("Background Task Service Performance", () => {
    ...
    bench(
        ...
        () => {
            ...
            const emailWorker = service.createWorker(
                "email-worker",
                ["email"],
                1
            );
    ...
    bench(
        ...
        () => {
            ...
            const urgentTasks = service.submitTask(
                "urgent",
                { data: "urgent" },
                { priority: "urgent" }
            );
            const normalTasks = service.submitTask(
                "normal",
                { data: "normal" },
                { priority: "normal" }
            );
            const lowTasks = service.submitTask(
                "low",
                { data: "low" },
                { priority: "low" }
            );
    ...
    bench(
        ...
        () => {
            ...
            service.registerProcessor(
                "scheduled",
                createMockProcessor("scheduled", 5)
            );
    ...
    bench(
        ...
        () => {
            ...
            Promise.all(tasks).then((taskIds) => {
                ...
                const cancelPromises = taskIds
                    .slice(0, 5)
                    .map((id) => service.cancelTask(id));
    ...
    bench(
        ...
        () => {
            ...
            service.registerProcessor(
                "continuous",
                createMockProcessor("continuous", 10)
            );
            service.startWorker(worker.id);
    ...
    bench(
        ...
        () => {
            ...
            service.scheduleJob("test-job", "0 * * * *", "type1", {
                data: "test",
            });
