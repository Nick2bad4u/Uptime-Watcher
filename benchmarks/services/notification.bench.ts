 ...
 *
 * @file Performance benchmarks for notification delivery, queuing, and management.
 *
...
class MockNotificationQueue {
    ...
    async enqueue(request: NotificationRequest): Promise<string> {
        ...
        const queuedNotification: QueuedNotification = {
            ...
            status: 'pending',
            attempts: 0,
            maxAttempts: request.priority === 'urgent' ? 5 : request.priority === 'high' ? 3 : 2,
            createdAt: now,
    ...
    async dequeue(priority?: string): Promise<QueuedNotification | null> {
        ...
        let candidates = Array.from(this.queue.values())
            .filter(n => 
                n.status === 'pending' && 
                n.scheduledAt <= now &&
        ...
        if (priority) {
            candidates = candidates.filter(n => n.request.priority === priority);
        }
    ...
    async markComplete(id: string, success: boolean, receipts?: DeliveryReceipt[], error?: string): Promise<void> {
    ...
    async updateStatus(id: string, status: QueuedNotification['status']): Promise<void> {
        const notification = this.queue.get(id);
        if (notification) {
    ...
    getPendingCount(): number {
        return Array.from(this.queue.values())
            .filter(n => n.status === 'pending' || n.status === 'retrying').length;
    }
}
...
class MockChannelProvider {
    ...
    async send(channelId: string, recipient: string, subject: string, body: string): Promise<DeliveryReceipt> {
    ...
    private getRateLimitCounters(channelId: string): { minute: number; hour: number; day: number; lastReset: Date } {
        const now = new Date();
        ...
        const channels = [
            {
                ...
                isActive: true,
                rateLimits: { maxPerMinute: 10, maxPerHour: 100, maxPerDay: 1000 }
            },
            {
                id: 'sms-primary',
                type: 'sms' as const,
            ...
            },
            {
                ...
                isActive: true,
                rateLimits: { maxPerMinute: 20, maxPerHour: 200, maxPerDay: 2000 }
            },
            {
                id: 'webhook-primary',
                ...
                isActive: true,
                rateLimits: { maxPerMinute: 30, maxPerHour: 300, maxPerDay: 3000 }
            }
        ];
        ...
        const templates = [
            {
                ...
                variables: ['siteName', 'siteUrl', 'timestamp', 'error'],
                channelTypes: ['email', 'sms', 'slack'],
                metadata: {}
            },
            {
                id: 'site-up',
                ...
                variables: ['siteName', 'siteUrl', 'downtime'],
            ...
            },
            {
                ...
                variables: ['siteName', 'startTime', 'duration'],
                channelTypes: ['email', 'slack', 'in-app'],
                metadata: {}
            },
            {
                id: 'daily-report',
                ...
                variables: ['date', 'totalSites', 'uptime', 'incidents'],
                channelTypes: ['email'],
                metadata: {}
            }
        ];

        templates.forEach(template => this.templates.set(template.id, template as NotificationTemplate));
    }
...
class MockNotificationService {
    ...
    private async processBatch(priority: string, batchSize: number): Promise<void> {
        ...
        try {
            ...
            for (const channelId of notification.request.channelIds) {
                for (const recipient of notification.request.recipients) {
                    const receipt = await this.channelProvider.send(channelId, recipient, subject, body);
                    receipts.push(receipt);
                }
            }

            // Check if all deliveries were successful
            const allSuccessful = receipts.every(r => r.status === 'delivered');
            
            await this.queue.markComplete(notification.id, allSuccessful, receipts);
        } catch (error) {
            await this.queue.markComplete(notification.id, false, [], error.message);
        }
    }
...
describe("Notification Service Performance", () => {
    ...
    bench("send single notification", () => {
        service = new MockNotificationService();
        const siteData = { id: 'site-1', name: 'Test Site', url: 'https://test.com' };
    ...
    bench("process single notification", () => {
        service = new MockNotificationService();
        const siteData = { id: 'site-1', name: 'Test Site', url: 'https://test.com' };
        const notification = createSiteDownNotification(siteData);
    ...
    bench("bulk notification sending", () => {
        service = new MockNotificationService();
    ...
    bench("priority queue handling", () => {
        ...
        const urgentNotifications = Array.from({ length: 5 }, (_, i) => ({
            ...
            variables: { siteName: `Urgent Site ${i}`, siteUrl: `https://urgent${i}.com`, timestamp: new Date().toISOString(), error: 'Critical error' },
        ...
        const normalNotifications = Array.from({ length: 10 }, (_, i) => ({
            ...
            variables: { siteName: `Normal Site ${i}`, startTime: new Date().toISOString(), duration: '1 hour' },
            priority: 'normal' as const
        }));
        
        const allNotifications = [...urgentNotifications, ...normalNotifications];
    ...
    bench("notification retry logic", () => {
        service = new MockNotificationService();
        ...
        const failingNotification: NotificationRequest = {
            ...
            variables: { siteName: 'Test Site', siteUrl: 'https://test.com', timestamp: new Date().toISOString(), error: 'Test error' },
            priority: 'normal'
        };
        
        queue.enqueue(failingNotification).then(id => {
            // Simulate failure
            queue.markComplete(id, false, [], 'Simulated failure').then(() => {
                // Process retry
                queue.dequeue();
            });
        });
    ...
    bench("multi-channel delivery", () => {
        service = new MockNotificationService();
        
        const multiChannelNotification: NotificationRequest = {
            templateId: 'site-down',
            channelIds: ['email-primary', 'sms-primary', 'slack-alerts', 'webhook-primary'],
            recipients: ['admin@example.com', '+1234567890', '#alerts', 'webhook-endpoint'],
            variables: {
    ...
    bench("scheduled notifications", () => {
        service = new MockNotificationService();
        
        const scheduledNotifications = Array.from({ length: 10 }, (_, i) => ({
            templateId: 'maintenance',
            channelIds: ['email-primary'],
