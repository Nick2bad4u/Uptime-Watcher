...
class MockMetricsStorage {
    ...
    async store(
        ...
        for (const point of points) {
            ...
            // Update time index
            const timeKey =
                Math.floor(point.timestamp.getTime() / 60_000) * 60_000; // Round to minute
...
class MockAlertEngine {
    ...
    async evaluateRules(metrics: AggregatedMetric[]): Promise<Alert[]> {
        ...
        for (const metric of metrics) {
            const rulesForMetric = Array.from(this.rules.values()).filter(
                (rule) => rule.metricName === metric.name && rule.isActive
            );
...
class MockQueryCache {
    private cache = new Map<
        string,
        { result: AnalyticsResult; expiry: Date }
    >();
    ...
    get(query: AnalyticsQuery): AnalyticsResult | null {
        ...
        if (cached && cached.expiry > new Date()) {
            return {
                ...cached.result,
                queryInfo: { ...cached.result.queryInfo, cached: true },
            };
        }
...
class MockAnalyticsService {
    ...
    async executeQuery(query: AnalyticsQuery): Promise<AnalyticsResult> {
        ...
        const totalPoints = timeSeries.reduce(
            (sum, series) => sum + series.points.length,
            0
        );
...
function generateMetricData(
    ...
    for (let i = 0; i < count; i++) {
        const timestamp = new Date(
            now.getTime() - (timeRangeMinutes - i) * 60 * 1000
        );
        data.push({
            name: metricName,
...
describe("Analytics Service Performance", () => {
    ...
    bench(
        ...
        () => {
            ...
            const largeDataset = generateMetricData(
                "large.dataset",
                1000,
                24 * 60
            );
    ...
    bench(
        ...
        () => {
            ...
            const concurrentMetrics = generateMetricData(
                "concurrent.metric",
                150,
                90
            );
            ...
            service.recordBatchMetrics(concurrentMetrics).then(() => {
                ...
                Promise.all(
                    queries.map((query) => service.executeQuery(query))
                );
            });
        },
