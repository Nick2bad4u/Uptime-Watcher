...
/** Captures session state for authenticated users. */
interface Session {
    id: string;
    userId: string;
    token: string;
...
/** Represents credential payloads used during login. */
...
/** Return payload generated by the authentication service. */
...
/** Describes a permission applied to roles or users. */
...
/** Aggregates permissions into assignable roles. */
interface Role {
    id: string;
    name: string;
    description: string;
...
/** Contextual data supplied during authorization checks. */
...
class MockPasswordService {
    ...
    /** Validates a password against the synthetic hash. */
    ...
    /** Lightweight hash used to avoid external dependencies. */
...
class MockTokenService {
    ...
    /** Issues a JWT-like access token with synthetic signing. */
    ...
    /** Generates a deterministic signature for token verification. */
...
/**
 * In-memory repository simulating user persistence operations.
 */
...
/**
 * In-memory session repository modelling access token lifecycle.
 */
class MockSessionRepository {
    private sessions = new Map<string, Session>();
    private nextId = 1;

    /** Creates a session record with generated identifiers. */
    async create(
    ...
    /** Retrieves a session by its access token. */
    ...
    /** Retrieves active sessions associated with a user. */
    ...
    /** Removes all sessions associated with a user. */
...
/**
 * Provides role and permission resolution utilities for benchmarks.
 */
class MockPermissionService {
    ...
    /** Checks whether the supplied context grants the requested permission. */
    hasPermission(
    ...
    /** Aggregates unique permissions for the supplied roles. */
    ...
    /** Seeds default roles and permissions used in benchmarks. */
...
/**
 * Coordinates authentication workflows for benchmarking scenarios.
 */
class MockAuthenticationService {
    ...
    /** Executes the login workflow and establishes a session. */
    ...
    /** Deactivates a single session by token. */
    ...
    /** Terminates all active sessions for the given user. */
    async logoutAllSessions(userId: string): Promise<void> {
        await this.sessionRepository.deleteByUserId(userId);
    }

    /**
     * Validates a token and returns contextual authorization details.
     */
    ...
    /** Exchanges a refresh token for a new session and access token. */
    ...
    /** Delegates permission checks to the permission service. */
    ...
    /** Creates a user with hashed password and assigns default role set. */
    ...
    /** Purges expired sessions and returns the count of removals. */
    async cleanupExpiredSessions(): Promise<number> {
        return await this.sessionRepository.cleanup();
    }

    /**
     * Returns derived statistics covering users and active sessions.
     */
    getStats(): any {
        return {
    ...
    /** Resets repositories to a pristine state. */
    reset(): void {
...
/** Constructs synthetic user credentials for tests. */
...
/** Builds a login request with randomized client metadata. */
