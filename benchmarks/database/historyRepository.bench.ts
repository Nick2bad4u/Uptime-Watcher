        ...
        return {
            run: (...params: any[]) => {
                if (sql.includes('INSERT')) {
                    const record = { id: this.nextId++, timestamp: Date.now(), ...params };
                ...
                if (sql.includes('DELETE')) {
                    ...
                    if (sql.includes('timestamp <')) {
                        ...
                        const index = this.history.findIndex(h => h.id === params[0]);
                        if (index !== -1) {
            ...
            all: () => {
                if (sql.includes('ORDER BY timestamp DESC LIMIT')) {
                    return this.history.sort((a, b) => b.timestamp - a.timestamp).slice(0, 100);
                }
...
class MockHistoryRepository {
    ...
    private initialize() {
        ...
        for (let i = 0; i < 10000; i++) {
            this.logEvent({
                siteId: (i % 100) + 1,
                monitorId: (i % 1000) + 1,
                eventType: ['status_change', 'check_result', 'error', 'recovery'][i % 4],
    ...
    logEvent(event: any) {
        ...
        `);
        return stmt.run(
            event.siteId, event.monitorId, event.eventType, event.status,
            event.message, event.metadata, event.timestamp
        );
    }
    ...
    findByMonitor(monitorId: number, limit: number = 100) {
        const stmt = this.db.prepare('SELECT * FROM history WHERE monitorId = ? ORDER BY timestamp DESC LIMIT ?');
        return stmt.all();
    }

    findBySite(siteId: number, limit: number = 100) {
        const stmt = this.db.prepare('SELECT * FROM history WHERE siteId = ? ORDER BY timestamp DESC LIMIT ?');
        return stmt.all();
    }

    findByDateRange(startDate: number, endDate: number) {
        const stmt = this.db.prepare('SELECT * FROM history WHERE timestamp BETWEEN ? AND ? ORDER BY timestamp DESC');
        return stmt.all();
    }

    findByEventType(eventType: string) {
        const stmt = this.db.prepare('SELECT * FROM history WHERE eventType = ? ORDER BY timestamp DESC');
        return stmt.all();
    }
    ...
    bulkLogEvents(events: any[]) {
        const stmt = this.db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
        return events.map(event => stmt.run(
            event.siteId, event.monitorId, event.eventType, event.status,
            event.message, event.metadata, event.timestamp
        ));
    }
    ...
    getStatistics() {
        ...
        const eventTypes = ['status_change', 'check_result', 'error', 'recovery'];
        return {
            total: all.length,
            byEventType: Object.fromEntries(
                eventTypes.map(type => [type, all.filter((h: any) => h.eventType === type).length])
            ),
            lastWeek: all.filter((h: any) => h.timestamp > Date.now() - 7 * 24 * 60 * 60 * 1000).length
        };
    }
    ...
    getResponseTimeStats(monitorId: number, hours: number = 24) {
        ...
        const responseTimes = records.map((r: any) => {
            try {
        ...
        if (responseTimes.length === 0) return { avg: 0, min: 0, max: 0, count: 0 };
...
describe("History Repository Performance", () => {
    ...
    bench("log single event", () => {
        ...
        repository.logEvent({
            ...
            metadata: JSON.stringify({ responseTime: 150, statusCode: 200 }),
            timestamp: Date.now()
        });
    ...
    bench("find records by monitor", () => {
        repository = new MockHistoryRepository();
        repository.findByMonitor(Math.floor(Math.random() * 1000) + 1, 100);
    }, { warmupIterations: 5, iterations: 1000 });

    bench("find records by site", () => {
        repository = new MockHistoryRepository();
        repository.findBySite(Math.floor(Math.random() * 100) + 1, 100);
    }, { warmupIterations: 5, iterations: 1000 });

    bench("find records by date range", () => {
        repository = new MockHistoryRepository();
        const now = Date.now();
        repository.findByDateRange(now - 24 * 60 * 60 * 1000, now);
    }, { warmupIterations: 5, iterations: 500 });

    bench("find records by event type", () => {
        repository = new MockHistoryRepository();
        const types = ['status_change', 'check_result', 'error', 'recovery'];
        repository.findByEventType(types[Math.floor(Math.random() * types.length)]);
    }, { warmupIterations: 5, iterations: 1000 });

    bench("bulk log events (50 events)", () => {
        repository = new MockHistoryRepository();
        const events = Array.from({ length: 50 }, (_, i) => ({
            ...
            metadata: JSON.stringify({ responseTime: Math.floor(Math.random() * 1000) }),
            timestamp: Date.now() - (i * 1000)
        }));
        repository.bulkLogEvents(events);
    }, { warmupIterations: 2, iterations: 200 });

    bench("delete old records", () => {
        repository = new MockHistoryRepository();
    ...
    bench("cleanup old records", () => {
        repository = new MockHistoryRepository();
        repository.cleanup(30);
    }, { warmupIterations: 5, iterations: 100 });

    bench("get response time stats", () => {
        repository = new MockHistoryRepository();
        repository.getResponseTimeStats(Math.floor(Math.random() * 1000) + 1, 24);
    }, { warmupIterations: 5, iterations: 500 });
});
