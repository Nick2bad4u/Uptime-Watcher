 ...
 *
 * @file Performance benchmarks for site repository operations including CRUD operations,
...
class MockSiteRepository {
    ...
    create(site: any) {
        const stmt = this.db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        `);
        return stmt.run(site.name, site.url, site.isActive, site.checkInterval, site.timeout, site.metadata);
    }
    ...
    update(id: number, updates: any) {
        const stmt = this.db.prepare(`
            ...
            WHERE id = ?
        `);
        return stmt.run(updates.name, updates.url, updates.isActive, updates.checkInterval, updates.timeout, updates.metadata, id);
    }
    ...
    bulkCreate(sites: any[]) {
        const stmt = this.db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        `);
        return sites.map(site => stmt.run(site.name, site.url, site.isActive, site.checkInterval, site.timeout, site.metadata));
    }

    search(query: string) {
        const stmt = this.db.prepare('SELECT * FROM sites WHERE name LIKE ? OR url LIKE ?');
        return stmt.all();
    }
}
...
describe("Site Repository Performance", () => {
    ...
    bench("bulk create sites (100 sites)", () => {
        repository = new MockSiteRepository();
