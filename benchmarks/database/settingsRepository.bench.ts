        ...
        return {
            ...
            all: () => {
                ...
                if (sql.includes('WHERE isSystem =')) {
                    return Array.from(this.settings.values()).filter(s => !s.isSystem);
                }
...
class MockSettingsRepository {
    ...
    private initialize() {
        ...
        const defaultSettings = [
            ...
            { key: 'app.notifications', value: 'true', type: 'boolean', category: 'notifications' },
            { key: 'monitor.defaultInterval', value: '60000', type: 'number', category: 'monitoring' },
            { key: 'monitor.timeout', value: '5000', type: 'number', category: 'monitoring' },
            { key: 'monitor.retries', value: '3', type: 'number', category: 'monitoring' },
            { key: 'ui.autoRefresh', value: 'true', type: 'boolean', category: 'ui' },
            { key: 'ui.refreshInterval', value: '30000', type: 'number', category: 'ui' },
            { key: 'logging.level', value: 'info', type: 'string', category: 'system' },
            { key: 'logging.maxFiles', value: '5', type: 'number', category: 'system' },
            { key: 'backup.enabled', value: 'true', type: 'boolean', category: 'backup' }
        ];

        defaultSettings.forEach((setting, index) => {
            this.set(setting.key, setting.value, setting.type, setting.category, index < 5);
        });
    ...
    set(key: string, value: string, type: string = 'string', category: string = 'general', isSystem: boolean = false) {
    ...
    getAll() {
        const stmt = this.db.prepare('SELECT * FROM settings ORDER BY category, key');
        return stmt.all();
    }
    ...
    getUserSettings() {
        const stmt = this.db.prepare('SELECT * FROM settings WHERE isSystem = 0 ORDER BY category, key');
        return stmt.all();
    }

    getSystemSettings() {
        const stmt = this.db.prepare('SELECT * FROM settings WHERE isSystem = 1 ORDER BY key');
        return stmt.all();
    }
    ...
    bulkUpdate(settings: { key: string; value: string; type?: string; category?: string }[]) {
        ...
        `);
        return settings.map(setting => stmt.run(
            setting.key,
            setting.value,
            setting.type || 'string',
    ...
    import(settingsData: any) {
        const settings = Object.entries(settingsData).map(([key, data]: [string, any]) => ({
            key,
            value: data.value,
...
describe("Settings Repository Performance", () => {
    ...
    bench("reset category settings", () => {
        repository = new MockSettingsRepository();
        repository.reset('test');
    }, { warmupIterations: 5, iterations: 200 });

    bench("export settings", () => {
        repository = new MockSettingsRepository();
        repository.export();
    }, { warmupIterations: 5, iterations: 500 });

    bench("import settings", () => {
        repository = new MockSettingsRepository();
        const settingsData = {
            'import.setting1': { value: 'value1', type: 'string', category: 'import' },
            'import.setting2': { value: 'true', type: 'boolean', category: 'import' },
            'import.setting3': { value: '42', type: 'number', category: 'import' }
        };
        repository.import(settingsData);
    }, { warmupIterations: 2, iterations: 500 });

    bench("validate setting value", () => {
        repository = new MockSettingsRepository();
