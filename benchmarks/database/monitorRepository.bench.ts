...
class MockMonitorRepository {
    ...
    create(monitor: any) {
        ...
        `);
        return stmt.run(
            monitor.siteId, monitor.type, monitor.name, monitor.isEnabled,
            monitor.interval, monitor.timeout, monitor.retries, monitor.configuration
        );
    }
    ...
    findEnabled() {
        const stmt = this.db.prepare('SELECT * FROM monitors WHERE isEnabled = 1');
        return stmt.all();
    }
    ...
    update(id: number, updates: any) {
        ...
        `);
        return stmt.run(
            updates.type, updates.name, updates.isEnabled, updates.interval,
            updates.timeout, updates.retries, updates.configuration, id
        );
    }
    ...
    bulkUpdateStatus(updates: Array<{ id: number; status: string; lastChecked: string }>) {
        const stmt = this.db.prepare(`
            UPDATE monitors SET status = ?, lastChecked = ? WHERE id = ?
        `);
        return updates.map(update => stmt.run(update.status, update.lastChecked, update.id));
    }
...
describe("Monitor Repository Performance", () => {
    ...
    bench("find enabled monitors", () => {
        repository = new MockMonitorRepository();
        repository.findEnabled();
    }, { warmupIterations: 5, iterations: 500 });

    bench("find monitors by type", () => {
        repository = new MockMonitorRepository();
        const types = ['http', 'ping', 'port'];
        repository.findByType(types[Math.floor(Math.random() * types.length)]);
    }, { warmupIterations: 5, iterations: 1000 });
    ...
    bench("delete monitor", () => {
        repository = new MockMonitorRepository();
        const id = Math.floor(Math.random() * 1000) + 1;
        repository.delete(id);
    }, { warmupIterations: 5, iterations: 1000 });

    bench("delete monitors by site id", () => {
        repository = new MockMonitorRepository();
