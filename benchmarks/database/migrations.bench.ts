...
class MockMigrationSystem {
    ...
    async rollbackMigration(version: number) {
        ...
        this.currentVersion = Math.max(
            0,
            ...Array.from(this.appliedMigrations)
        );
    }
...
describe("Database Migration Performance", () => {
    ...
    bench(
        "simple migration (create table)",
        async () => {
            migrationSystem = new MockMigrationSystem();
            await migrationSystem.runMigration(1, "create_users_table", [
                async () =>
                    await migrationSystem.createTable("users", [
                        "id",
                        "name",
                        "email",
                    ]),
    ...
    bench(
        ...
        async () => {
            ...
            await migrationSystem.runMigration(1, "setup_core_tables", [
                ...
                async () =>
                    await migrationSystem.createTable("posts", [
                        "id",
                        "user_id",
                        "title",
                        "content",
                ...
                async () =>
                    await migrationSystem.createIndex("posts", "idx_user_id", [
                        "user_id",
                    ]),
            ]);
        },
    ...
    bench(
        ...
        async () => {
            ...
            await migrationSystem.runMigration(2, "add_email_column", [
                async () =>
                    await migrationSystem.addColumn("users", "email", null),
            ]);
        },
        { warmupIterations: 5, iterations: 500 }
    );
    ...
    bench(
        ...
        async () => {
            ...
            await migrationSystem.runMigration(2, "remove_temp_field", [
                async () =>
                    await migrationSystem.dropColumn("users", "temp_field"),
            ]);
        },
        { warmupIterations: 5, iterations: 500 }
    );
    ...
    bench(
        ...
        async () => {
            ...
            await migrationSystem.runMigration(2, "add_email_index", [
                async () =>
                    await migrationSystem.createIndex("users", "idx_email", [
                        "email",
                    ]),
            ]);
        },
    ...
    bench(
        "data insertion migration (100 records)",
        async () => {
            migrationSystem = new MockMigrationSystem();
            await migrationSystem.createTable("users", [
                "id",
    ...
    bench(
        ...
        async () => {
            ...
            await migrationSystem.runMigration(2, "normalize_names", [
                async () =>
                    await migrationSystem.transformData("users", (row) => ({
                        ...row,
                        name: row.name
                            .split(" ")
                            .map(
    ...
    bench(
        ...
        async () => {
            migrationSystem = new MockMigrationSystem();
            await migrationSystem.createTable("old_users", [
                "id",
                "name",
                "email",
            ]);
            await migrationSystem.createTable("new_users", [
                "id",
                "full_name",
                "email_address",
            ]);
            ...
            await migrationSystem.runMigration(2, "migrate_user_data", [
                async () =>
                    await migrationSystem.migrateData(
                        "old_users",
                        "new_users",
                        (row) => ({
                            id: row.id,
    ...
    bench(
        "multi-migration execution (5 migrations)",
        async () => {
            migrationSystem = new MockMigrationSystem();
            const migrations = [
                {
                ...
                },
                {
                    ...
                    operations: [
                        async () =>
                            await migrationSystem.addColumn(
                                "users",
                                "email",
                                null
                            ),
                ...
                },
                {
                    ...
                    operations: [
                        async () =>
                            await migrationSystem.createIndex(
                                "posts",
                                "idx_user",
                                ["user_id"]
                            ),
    ...
    bench(
        ...
        async () => {
            ...
            await migrationSystem.runMigration(1, "test_migration", [
                async () =>
                    await migrationSystem.createTable("test", ["id", "data"]),
            ]);
