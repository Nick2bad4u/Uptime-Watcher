/**
 * Performance benchmarks for database performance monitoring operations Tests
 * the performance of metrics collection, performance analysis, and monitoring
 * systems
 */
...
describe("Database Performance Monitoring", () => {
    ...
    const generatePerformanceThresholds = (
        count: number
    ...
    bench("metrics aggregation - real-time processing", () => {
        ...
        for (const windowSize of aggregationWindows) {
            const windowMetrics = performanceMetrics.filter(
                (m) => m.timestamp >= Date.now() - windowSize
            );

            // Group by metric type
            const metricGroups = windowMetrics.reduce(
                (groups, metric) => {
    ...
    bench("threshold evaluation - real-time alerting", () => {
        ...
        for (const metric of performanceMetrics) {
            const relevantThresholds = performanceThresholds.filter(
                (t) => t.metricType === metric.metricType && t.enabled
            );
    ...
    bench("performance analysis - trend detection", () => {
        ...
        for (const session of monitoringSessions.slice(0, 20)) {
            ...
            const latencyMetrics = sessionMetrics.filter(
                (m) => m.metricType === "query_time"
            );
            const throughputMetrics = sessionMetrics.filter(
                (m) => m.metricType === "throughput"
            );
            const errorMetrics = sessionMetrics.filter(
            ...
            const analysis: PerformanceAnalysis = {
                ...
                metrics: {
                    ...
                    throughput:
                        throughputs.reduce((sum, val) => sum + val, 0) /
    ...
    bench("anomaly detection - statistical analysis", () => {
        ...
        const metricsByType = performanceMetrics.reduce(
            (groups, metric) => {
                if (!groups[metric.metricType]) {
                    groups[metric.metricType] = [];
                }
    ...
    bench("performance report generation", () => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const relevantSessions = monitoringSessions.filter(
                (s) =>
            ...
            const performanceValues = relevantSessions.map(
                (s) => s.summary.avgResponseTime
            );
                ...
                sections: [
                    {
                        ...
                        charts: [
                            {
                                type: "line",
                                data: performanceValues.map((val, idx) => ({
                                    x: idx,
                                    y: val,
                                })),
                                config: {
                    ...
                    },
                    {
                        ...
                        content:
                            "In-depth analysis of performance patterns and anomalies",
                        charts: [
                            {
                                type: "histogram",
                                data: performanceValues.map((val) => ({
                                    value: val,
                                    count: 1,
                                })),
                                config: {
