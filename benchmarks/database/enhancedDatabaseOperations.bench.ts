...
describe("Enhanced Database Performance Benchmarks", () => {
    ...
    function generateMonitor(siteId: string, monitorId: number): Monitor {
        ...
        return {
            ...
            type,
            url: type === "http" ? `https://example${monitorId}.com` : undefined,
    ...
    function generateStatusHistory(monitorId: string, count: number): StatusHistory[] {
    ...
    describe("Small-Scale Operations (1-10 sites)", () => {
        bench("Insert 10 sites with 5 monitors each", async () => {
            await databaseService.executeTransaction(async () => {
                for (let i = 0; i < 10; i++) {
                    ...
                    for (let j = 0; j < 5; j++) {
                        const monitor = generateMonitor(site.identifier, i * 5 + j);
        ...
        bench("Query all sites with monitors (small dataset)", async () => {
            await siteRepository.findAll();
        }, { iterations: 100 });

        bench("Update 50 monitors status", async () => {
            await databaseService.executeTransaction(async () => {
                for (let i = 0; i < 50; i++) {
    ...
    describe("Medium-Scale Operations (50-100 sites)", () => {
        bench("Bulk insert 100 sites with 10 monitors each", async () => {
            await databaseService.executeTransaction(async () => {
                for (let i = 100; i < 200; i++) {
                    ...
                    for (let j = 0; j < 10; j++) {
                        const monitor = generateMonitor(site.identifier, i * 10 + j);
                        await monitorRepository.createInternal(monitor);
                    }
                }
            });
        ...
        bench("Complex query: Sites with failing monitors", async () => {
            await siteRepository.findSitesWithStatus("down");
        }, { iterations: 50 });

        bench("Bulk history insertion (1000 entries)", async () => {
        ...
        bench("History cleanup (remove old entries)", async () => {
    ...
    describe("Large-Scale Operations (500+ sites)", () => {
        bench("Massive bulk insert: 500 sites with 5 monitors each", async () => {
            await databaseService.executeTransaction(async () => {
        ...
        bench("Full database query: All sites with all data", async () => {
            await siteRepository.findAllWithMonitors();
        }, { iterations: 10 });

        bench("Concurrent status updates (2500 monitors)", async () => {
        ...
        bench("Database backup operation", async () => {
    ...
    describe("Memory Pressure Operations", () => {
        bench("Load large result set (10,000 history entries)", async () => {
            const monitorId = "benchmark-monitor-0";
            await historyRepository.findByMonitorId(monitorId, 10000);
        }, { iterations: 5 });

        bench("Aggregate operations: Calculate uptime statistics", async () => {
            const monitorId = "benchmark-monitor-0";
            const last24Hours = new Date(Date.now() - 24 * 60 * 60 * 1000);
            await historyRepository.calculateUptimePercentage(monitorId, last24Hours);
        }, { iterations: 20 });

        bench("Complex JOIN: Sites with monitor statistics", async () => {
    ...
    describe("Transaction Performance", () => {
        bench("Nested transactions: Site with monitors and history", async () => {
            await databaseService.executeTransaction(async () => {
                ...
                await databaseService.executeTransaction(async () => {
                    for (let i = 0; i < 5; i++) {
                        const monitor = generateMonitor(site.identifier, 9999 * 5 + i);
                        await monitorRepository.createInternal(monitor);
                        
                        const history = generateStatusHistory(monitor.id, 100);
        ...
        bench("Long-running transaction (100 operations)", async () => {
            await databaseService.executeTransaction(async () => {
                for (let i = 0; i < 100; i++) {
                    const monitor = generateMonitor("long-transaction-site", i);
                    await monitorRepository.createInternal(monitor);
                }
            });
        }, { iterations: 3 });
    ...
    describe("Real-World Simulation Scenarios", () => {
        bench("Morning startup: Load all sites and refresh status", async () => {
            // Simulate app startup - load all sites
            const sites = await siteRepository.findAll();
            
            // Simulate status refresh for all monitors
            for (const site of sites.slice(0, 50)) { // Limit for benchmark
        ...
        bench("Peak monitoring: Concurrent updates from all monitors", async () => {
        ...
        bench("Data maintenance: Cleanup and optimization", async () => {
