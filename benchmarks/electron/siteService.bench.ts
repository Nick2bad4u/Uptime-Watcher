/**
 * Performance benchmarks for Site Service operations Tests the performance of
 * site management, loading, and coordination operations
 */
...
describe("Site Service Performance", () => {
        ...
        for (let i = 0; i < 200; i++) {
            ...
            if (site) {
                ...
                const result = {
                    ...
                    success: true,
                    loadTime:
                        loadOperation.loadTime + relatedMonitors.length * 0.1,
                };
            } else {
                const result = {
                    ...loadOperation,
    ...
    bench("site loading simulation - with full history", () => {
        for (let i = 0; i < 100; i++) {
            ...
            const totalLoadTime =
                baseLoadTime + monitorPenalty + historyPenalty;
        ...
        for (let i = 0; i < 250; i++) {
            ...
            if (existingSite) {
                ...
                const intervalChanged =
                    existingSite.monitoringInterval !==
        ...
        for (let i = 0; i < 400; i++) {
            ...
            const results = mockSites.filter((site) => {
                if (
                    ...
                    site.enabled !== filter.enabled
                ) {
                    return false;
                }
                if (
                    filter.status !== undefined &&
                    site.status !== filter.status
                ) {
                    return false;
                }
                return true;
    ...
    bench("complex site coordination simulation", () => {
        for (let i = 0; i < 100; i++) {
            ...
            switch (operation.type) {
                case "full_reload": {
                    ...
                    steps.push(
                        ...
                        {
                            step: "validate_monitors",
                            time: processingTime * 0.4,
                        },
                        { step: "update_cache", time: processingTime * 0.3 }
                    );
                    break;
                }
                case "health_check": {
                ...
                case "config_sync": {
                    ...
                    steps.push(
                        { step: "sync_monitors", time: processingTime * 0.5 },
                        {
                            ...
                            time: processingTime * 0.3,
                        },
                        { step: "validate_config", time: processingTime * 0.2 }
                    );
                    break;
                }
            }
            ...
            const result = {
                ...
                totalTime,
                complexity:
                    operation.type === "config_sync" ||
                    operation.type === "full_reload"
                        ? "high"
                        : "medium",
    ...
    bench("high-volume site operations simulation", () => {
        ...
        for (const batchSize of batchSizes) {
            ...
            const result = {
                ...
                successRate:
                    results.filter((r) => r.success).length / batchSize,
