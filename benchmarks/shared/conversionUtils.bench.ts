/**
 * Performance benchmarks for shared conversion and validation utilities
 * Tests the performance of data conversion, type checking, and validation functions
 */
...
import {
  safeStringify,
} from "../../shared/utils/stringConversion";
import {
  isString,
  isNumber,
  isBoolean,
...
describe("Shared Conversion and Validation Performance", () => {
  ...
  const validNumbers = [123, 456.78, -999, 0, 3.14159];
  const stringNumbers = ["123", "456.78", "-999", "0", "3.14159"];
  const invalidNumbers = ["abc", "not-a-number", "", "   ", null, undefined];

  const timeoutStrings = ["5s", "30s", "1m", "5m", "30m"];
  const invalidTimeouts = ["invalid", "5x", "", "toolong123"];

  const validPorts = [80, 443, 3000, 8080, 65535];
  const invalidPorts = [0, -1, 65536, 99999, "80", "invalid"];

  const validTimestamps = [Date.now(), Date.now() - 3600000, 1640995200000];
  const invalidTimestamps = [-1, "invalid", null, undefined, Infinity, NaN];
  ...
  bench("safeParseFloat - mixed values", () => {
    const allValues = [...validNumbers, ...stringNumbers, ...invalidNumbers];
  ...
  bench("isFiniteNumber - mixed values", () => {
    const testValues = [...validNumbers, Infinity, -Infinity, NaN, "123"];
  ...
  bench("isNonNegativeNumber - mixed values", () => {
    const testValues = [...validNumbers, -1, -999, 0, 123];
    for (const value of testValues) {
  ...
  bench("isPositiveNumber - mixed values", () => {
    const testValues = [...validNumbers, -1, 0, 123];
    for (const value of testValues) {
  ...
  bench("high-volume type checking", () => {
    const values = Array.from({ length: 1000 }, (_, i) => {
      const options = [i, String(i), i % 2 === 0, null, undefined];
  ...
  bench("complex conversion workflow", () => {
    ...
    for (const config of testConfigs) {
      ...
      const enabled = isString(config.enabled) ? 
        config.enabled.toLowerCase() === "true" : false;
  ...
  bench("complex object stringification", () => {
    const complexObjects = [
      ...
      { mixed: { string: "test", number: 42, boolean: true, array: [1, 2] } },
      null,
      undefined,
      "simple string",
      42,
