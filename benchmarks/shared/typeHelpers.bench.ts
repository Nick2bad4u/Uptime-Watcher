/**
 * Performance benchmarks for shared type utilities and helpers
 * Tests the performance of type checking, conversion, and safety functions
 */
...
import type { Monitor, Site, MonitorStatus, MonitorType, SiteStatus } from "../../shared/types";
...
function isMonitorType(type: unknown): type is MonitorType {
  return typeof type === "string" && ["http", "port", "ping", "dns"].includes(type);
}

describe("Shared Type Utilities Performance", () => {
  // Test data for type checking benchmarks
  const validMonitorStatuses: MonitorStatus[] = ["up", "down", "pending", "paused"];
  const invalidMonitorStatuses = ["invalid", "unknown", 123, null, undefined];

  const validMonitorTypes: MonitorType[] = ["http", "port", "ping", "dns"];
  const invalidMonitorTypes = ["ftp", "smtp", "invalid", 456, null];

  const validSiteStatuses: SiteStatus[] = ["up", "down", "pending", "paused", "mixed", "unknown"];
  const invalidSiteStatuses = ["broken", "offline", 789, null, undefined];
  ...
  const complexSite: Site = {
    ...
    monitors: Array.from({ length: 5 }, (_, i) => ({
      ...complexMonitor,
      id: `monitor-${i}`,
      type: (["http", "port", "ping", "dns"] as MonitorType[])[i % 4],
    })),
  };
  ...
  bench("validateMonitor - invalid monitor", () => {
    const invalidMonitor = { ...complexMonitor, type: "invalid" as MonitorType };
    try {
  ...
  bench("high-volume monitor status validation", () => {
    const statuses = Array.from({ length: 1000 }, (_, i) => 
      validMonitorStatuses[i % validMonitorStatuses.length]
    );
  ...
  bench("high-volume monitor type validation", () => {
    const types = Array.from({ length: 1000 }, (_, i) => 
      validMonitorTypes[i % validMonitorTypes.length]
    );
  ...
  bench("memory-intensive deep equality", () => {
    const largeObject1 = {
      data: Array.from({ length: 500 }, (_, i) => ({ id: i, value: `item-${i}` })),
    };
