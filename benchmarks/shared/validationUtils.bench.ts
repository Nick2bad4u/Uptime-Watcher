/**
 * Performance benchmarks for shared validation utilities
 * Tests the performance of validation functions under various load conditions
 */
...
describe("Shared Validation Utils Performance", () => {
  ...
  const invalidPorts = [0, -1, 65536, 99999, -5000];

  const mixedStringNumbers = ["123", "456", "789", "0", "-999"];
  const invalidStringNumbers = ["abc", "12.34", "not-number", "", "   "];
  ...
  bench("high-volume url validation", () => {
    const largeUrlSet = Array.from({ length: 100 }, (_, i) => 
      `https://monitor-${i}.example.com/health/check`
    );
  ...
  bench("high-volume identifier validation", () => {
    const largeIdentifierSet = Array.from({ length: 100 }, (_, i) => 
      `monitor-${i}-health-check`
    );
