/**
 * Performance benchmarks for shared error handling and safety utilities
 * Tests the performance of error handling, validation, and safety mechanisms
 */
...
describe("Shared Error Handling and Safety Performance", () => {
  ...
  const failingAsyncOperation = () => Promise.reject(new Error("Operation failed"));
  ...
  bench("safeJsonParseWithFallback - valid JSON", () => {
    for (const jsonStr of validJsonStrings) {
      safeJsonParseWithFallback(jsonStr, (data): data is number => typeof data === "number", 3);
    }
  });
  ...
  bench("safePropertyAccess - valid objects", () => {
    const validObjects = testObjects.filter(obj => obj !== null && obj !== undefined);
  ...
  bench("validateAndConvert - string validation", () => {
    ...
    const testValues = ["valid", 123, null, undefined, true];
  ...
  bench("validateAndConvert - number validation", () => {
    ...
    const testValues = [42, "123", null, undefined, true];
  ...
  bench("high-volume JSON operations", () => {
    ...
    for (const op of operations) {
      ...
      safeJsonParseWithFallback(op.parse, (data): data is number => typeof data === "number", 3);
    }
  });
  ...
  bench("complex safety workflow", () => {
    const complexOperations = [
      {
        ...
        operation: () => safeJsonStringifyWithFallback({ result: "success" }, "fallback", 3),
      },
      {
        data: null,
        operation: () => { throw new Error("Invalid data"); },
      },
      {
    ...
    for (const { data, operation } of complexOperations) {
      try {
        ...
        // Format success
        safeJsonStringifyWithFallback({ success: true, data }, "fallback", 3);
      } catch (error) {
        // Handle error
        isError(error);
      }
