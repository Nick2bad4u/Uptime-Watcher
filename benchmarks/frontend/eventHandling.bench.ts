 ...
 * Tests the performance of event listeners, synthetic events, and event delegation
 */
...
class MockEventSystem {
  ...
  batchProcessEvents(events: Array<{ element: string; type: string; data?: any }>): EventMetrics[] {
    ...
    for (const eventSpec of events) {
      const metrics = this.dispatchEvent(eventSpec.element, eventSpec.type, eventSpec.data);
  ...
  getStatistics(): {
    ...
    for (const handlers of this.handlers.values()) {
      ...
      handlers.forEach(handler => {
        handlersByType[handler.eventType] = (handlersByType[handler.eventType] || 0) + 1;
      });
    }
    ...
    return {
      ...
      avgProcessingTime: totalProcessingTime / this.eventHistory.length || 0,
      handlersByType,
    };
  }
}
...
describe("React Event Handling Performance", () => {
  ...
  const generateEventSpecs = (count: number) => {
    const eventTypes = ["click", "mouseover", "mouseout", "keydown", "keyup", "focus", "blur", "input", "change"];
    const elementTypes = ["button", "input", "div", "span", "li", "a"];
  ...
  bench("event handler registration - many handlers", () => {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      const eventType = ["click", "mouseover", "keydown"][Math.floor(Math.random() * 3)];
  ...
  bench("event delegation - complex selectors", () => {
    ...
    const containers = ["main-container", "sidebar-container", "modal-container"];
    const selectors = [".button", ".input-field", "[data-action]", ".menu-item", ".card"];
    
    for (const container of containers) {
      for (const selector of selectors) {
        for (const eventType of ["click", "mouseover", "focus"]) {
  ...
  bench("event dispatching - high frequency", () => {
    ...
    for (const spec of eventSpecs.slice(0, 300)) {
      const metrics = eventSystem.dispatchEvent(spec.element, spec.type, spec.data);
  ...
  bench("synthetic event creation - complex events", () => {
    ...
    for (let i = 0; i < 1000; i++) {
      const eventType = ["click", "mousemove", "wheel", "touchstart", "keypress"][
      ...
      const complexEventData = {
        ...
        touches: Array.from({ length: Math.floor(Math.random() * 5) }, (_, j) => ({
          identifier: j,
          clientX: Math.random() * 1920,
          clientY: Math.random() * 1080,
        })),
  ...
  bench("custom events - event bus", () => {
    ...
    const customEventTypes = ["user-action", "data-update", "ui-change", "system-event"];
    ...
    for (let i = 0; i < 200; i++) {
      const customEvent: CustomEvent = {
        id: `custom-${i}`,
        type: customEventTypes[Math.floor(Math.random() * customEventTypes.length)],
        detail: {
  ...
  bench("event system stress test", () => {
    ...
    for (let i = 0; i < 200; i++) {
      eventSystem.addEventListener(
        ...
        "click",
        (event) => {
          const computation = Array.from({ length: 10 }, () => Math.random()).reduce((a, b) => a + b, 0);
        }
      );
