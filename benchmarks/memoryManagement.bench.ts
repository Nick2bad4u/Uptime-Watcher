...
class MemoryPressureSimulator {
    ...
    getMemoryUsage(): number {
        return this.allocatedObjects.reduce((total, obj) => total + obj.length * 8, 0) / (1024 * 1024);
    }
}
...
function generateMonitorData(siteId: string, monitorIndex: number, historySize: number): MonitorData {
    ...
    return {
        ...
        url: monitorIndex % 3 === 0 ? `https://example${monitorIndex}.com` : undefined,
        config: {
...
function generateSiteData(siteIndex: number, monitorCount: number, historySize: number): SiteData {
    ...
    return {
        ...
        metadata: {
            ...
            description: `Description for site ${siteIndex}`.repeat(Math.floor(Math.random() * 10) + 1),
            configuration: {
...
describe("Memory Usage and Cache Performance Benchmarks", () => {
    ...
    describe("Small-Scale Cache Operations", () => {
        bench("LRU Cache - Basic operations (100 items)", () => {
        ...
        bench("Memory allocation patterns - Small objects", () => {
            const objects: any[] = [];
            ...
            for (let i = 0; i < 500; i++) {
                ...
                obj.data[Math.floor(Math.random() * obj.data.length)] = Math.random();
                obj.metadata.lastAccessed = Date.now();
            }
        }, { iterations: 200 });

        bench("Cache miss handling and reconstruction", () => {
    ...
    describe("Medium-Scale Cache Operations", () => {
        bench("LRU Cache - Medium load (1000 items)", () => {
        ...
        bench("Memory-intensive data processing", () => {
            ...
            const processedData = sites.map(site => {
                const aggregatedHistory = site.monitors.reduce((acc, monitor) => {
                    return acc.concat(monitor.history);
                }, [] as HistoryEntry[]);
                
                const uptimeStats = aggregatedHistory.reduce((stats, entry) => {
                    if (entry.status === "up") {
                        stats.uptime++;
                    } else {
                        stats.downtime++;
            ...
            const summary = processedData.reduce((acc, data) => {
                acc.totalSites++;
                ...
                return acc;
            }, { totalSites: 0, totalUptime: 0, totalDowntime: 0, averageResponseTime: 0 });
        }, { iterations: 20 });

        bench("Cache eviction and memory pressure", () => {
            const cache = new MockLRUCache<string, SiteData>(50);
    ...
    describe("Large-Scale Cache Operations", () => {
        bench("LRU Cache - Large dataset (5000 items)", () => {
        ...
        bench("Memory allocation stress test", () => {
                ...
                const largeObject = {
                    ...
                    data: new Array(10000).fill(null).map(() => ({
                        ...
                        metadata: {
                            ...
                            tags: new Array(5).fill(null).map(() => `tag-${Math.floor(Math.random() * 100)}`),
                        },
                    })),
                    created: Date.now(),
                };
        ...
        bench("Cache coherency with concurrent operations", () => {
    ...
    describe("Memory Leak Detection and Prevention", () => {
        bench("Circular reference handling", () => {
            ...
            for (let i = 0; i < 1000; i++) {
                ...
                if (i > 0) {
                    const parent = objects[Math.floor(Math.random() * objects.length)];
                    obj.parent = parent;
        ...
        bench("WeakMap vs Map for object references", () => {
            ...
            for (let i = 0; i < 1000; i++) {
                ...
                // Store in both maps
                regularMap.set(obj, { metadata: `meta-${i}`, timestamp: Date.now() });
                weakMap.set(obj, { metadata: `meta-${i}`, timestamp: Date.now() });
            }
        ...
        bench("Event listener cleanup simulation", () => {
    ...
    describe("Garbage Collection Impact", () => {
        bench("Frequent allocation and deallocation", () => {
            for (let i = 0; i < 1000; i++) {
        ...
        bench("Object pooling vs new allocation", () => {
        ...
        bench("Large object vs many small objects", () => {
            ...
            largeObjects.forEach(obj => obj.data.forEach(item => item.processed = true));
        }, { iterations: 20 });
    });
});
