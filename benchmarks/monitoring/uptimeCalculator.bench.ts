...
class MockUptimeCalculator {
    calculateUptime(data: Array<{ timestamp: number; isUp: boolean }>, period: number): number {
        const now = Date.now();
    ...
    calculateSLA(data: Array<{ timestamp: number; isUp: boolean }>, targetSLA: number): any {
...
describe("Uptime Calculator Performance", () => {
    ...
    let testData: Array<{ timestamp: number; isUp: boolean }>;

    bench("calculator initialization", () => {
        calculator = new MockUptimeCalculator();
        testData = Array.from({ length: 10000 }, (_, i) => ({
