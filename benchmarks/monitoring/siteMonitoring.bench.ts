 ...
 *
 * @file Performance benchmarks for site monitoring operations including health
 *   checks, status updates, and monitoring logic.
 *
...
class MockSiteMonitor {
    ...
    getAggregatedStats(siteIdentifier: string) {
        ...
        const uptime =
            (results.filter((r) => r.isHealthy).length / results.length) * 100;
    ...
    private getStatusCodeDistribution(results: any[]) {
        ...
        for (const result of results) {
            distribution[result.statusCode] =
                (distribution[result.statusCode] || 0) + 1;
        }
        return distribution;
    }
    ...
    private evaluateAlertRule(rule: any, results: any[], site: any) {
        switch (rule.type) {
            case "uptime": {
                const uptime =
                    (results.filter((r) => r.isHealthy).length /
                        results.length) *
                    100;
    ...
    getSystemHealth() {
        ...
        const totalChecks = Array.from(this.monitoringResults.values()).reduce(
        ...
        return {
            ...
            totalChecks,
            systemUptime:
                (activeSites / (activeSites + offlineSites)) * 100 || 0,
        };
    }
}
