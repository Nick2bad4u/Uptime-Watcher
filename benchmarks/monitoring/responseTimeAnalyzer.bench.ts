...
class MockResponseTimeAnalyzer {
    ...
    detectAnomalies(responseTimes: number[], threshold: number = 2): number[] {
        ...
        const stdDev = Math.sqrt(
            responseTimes.reduce((sum, rt) => sum + (rt - avg) ** 2, 0) /
                responseTimes.length
        );

        return responseTimes.filter(
            (rt) => Math.abs(rt - avg) > threshold * stdDev
        );
    }
    ...
    getTrend(responseTimes: { timestamp: number; value: number }[]): string {
        ...
        const recentAvg =
            recent.reduce((sum, rt) => sum + rt.value, 0) / recent.length;
        const olderAvg =
            older.reduce((sum, rt) => sum + rt.value, 0) / older.length;
...
describe("Response Time Analyzer Performance", () => {
    ...
    bench(
        "calculate stats (1000 data points)",
        () => {
