...
function filterHistoryByTimeRange(history: StatusEntry[], startTime: number, endTime: number): StatusEntry[] {
...
function detectOutages(history: StatusEntry[], minimumDuration: number = 60000): Array<{ start: number; end: number; duration: number }> {
...
function calculateSLACompliance(history: StatusEntry[], slaThreshold: number): { compliant: boolean; actualUptime: number; requiredUptime: number } {
...
describe("Heavy Computational Operations Benchmarks", () => {
    ...
    describe("Small Dataset Operations (50 monitors)", () => {
        bench("Calculate uptime statistics for all sites", () => {
            aggregateSiteStatistics(smallDataset);
        }, { iterations: 100 });

        bench("Filter history by time range (1000 entries)", () => {
            const last24Hours = Date.now() - (24 * 60 * 60 * 1000);
            const now = Date.now();
            filterHistoryByTimeRange(smallHistory, last24Hours, now);
        }, { iterations: 1000 });

        bench("Detect outages in monitor history", () => {
            detectOutages(smallHistory, 60000);
        }, { iterations: 500 });

        bench("Calculate SLA compliance", () => {
    ...
    describe("Medium Dataset Operations (500 monitors)", () => {
        ...
        bench("Filter history by time range (10,000 entries)", () => {
            const last24Hours = Date.now() - (24 * 60 * 60 * 1000);
            const now = Date.now();
            filterHistoryByTimeRange(mediumHistory, last24Hours, now);
        }, { iterations: 100 });

        bench("Detect outages in large monitor history", () => {
            detectOutages(mediumHistory, 60000);
        }, { iterations: 50 });

        bench("Calculate SLA compliance for large dataset", () => {
            calculateSLACompliance(mediumHistory, 99.9);
        }, { iterations: 100 });

        bench("Process concurrent monitor updates", () => {
            const sites = mediumDataset;
            const results: UptimeStats[] = [];
            
            for (const site of sites) {
                for (const monitor of site.monitors) {
                    const stats = calculateUptimeStatistics(monitor.history.slice(0, 100));
                    results.push(stats);
                }
            }
        }, { iterations: 20 });
    ...
    describe("Large Dataset Operations (2000 monitors)", () => {
        ...
        bench("Filter history by time range (50,000 entries)", () => {
            const last24Hours = Date.now() - (24 * 60 * 60 * 1000);
            const now = Date.now();
            filterHistoryByTimeRange(largeHistory, last24Hours, now);
        }, { iterations: 20 });

        bench("Detect outages in massive history", () => {
            detectOutages(largeHistory, 60000);
        }, { iterations: 10 });

        bench("Calculate comprehensive site analytics", () => {
            const analytics = new Map();
            
            for (const site of largeDataset.slice(0, 50)) { // Limit for benchmark
                const siteStats = {
                    totalMonitors: site.monitors.length,
                    activeMonitors: site.monitors.filter(m => m.monitoring).length,
                    averageResponseTime: 0,
                    worstResponseTime: 0,
                    outageCount: 0,
                    slaCompliance: true,
                ...
                for (const monitor of site.monitors) {
                    const stats = calculateUptimeStatistics(monitor.history);
                    totalResponseTime += stats.averageResponseTime;
                    worstResponseTime = Math.max(worstResponseTime, stats.averageResponseTime);
                    
                    const outages = detectOutages(monitor.history, 60000);
                    totalOutages += outages.length;
                    
                    const sla = calculateSLACompliance(monitor.history, 99.5);
                    if (!sla.compliant) {
                        siteStats.slaCompliance = false;
                    }
                }
                
                siteStats.averageResponseTime = totalResponseTime / site.monitors.length;
    ...
    describe("Massive Dataset Operations (5000 monitors) - Stress Test", () => {
        bench("Calculate uptime statistics for massive dataset", () => {
            aggregateSiteStatistics(massiveDataset.slice(0, 100)); // Limit for benchmark
        }, { iterations: 3 });

        bench("Process 100,000 status updates", () => {
            const last7Days = Date.now() - (7 * 24 * 60 * 60 * 1000);
            const now = Date.now();
            
            // Simulate processing massive amount of status updates
            const filteredHistory = filterHistoryByTimeRange(massiveHistory, last7Days, now);
        ...
        bench("Memory-intensive data aggregation", () => {
            ...
            for (const site of massiveDataset.slice(0, 50)) {
                ...
                for (const range of timeRanges) {
                    ...
                    for (const monitor of site.monitors) {
                        const filtered = filterHistoryByTimeRange(monitor.history, startTime, now);
                        combinedHistory = combinedHistory.concat(filtered);
                    }
                    
                    siteData[range.name] = calculateUptimeStatistics(combinedHistory);
                }
    ...
    describe("Algorithm Performance - Edge Cases", () => {
        ...
        bench("Handle single-status datasets", () => {
            ...
            const upOnlyHistory = Array(1000).fill(null).map((_, i) => ({
            ...
            const downOnlyHistory = Array(1000).fill(null).map((_, i) => ({
                timestamp: Date.now() - (i * 1000),
        ...
        bench("Handle rapid status changes", () => {
            // Create history with status changing every second
            const rapidChanges = Array(5000).fill(null).map((_, i) => ({
                timestamp: Date.now() - (i * 1000),
