...
function decompressData(compressed: string): string {
    ...
    return compressed.replaceAll(/(?<char>.)\\*(?<count>\d+)/g, (
        match,
        char,
        count
...
describe("File Operations Performance Benchmarks", () => {
    ...
    describe("JSON Serialization/Deserialization Benchmarks", () => {
        ...
        bench(
            "serialize medium JSON (site config)",
            () => {
        ...
        bench(
            "serialize large JSON (full state)",
            () => {
        ...
        bench(
            ...
            () => {
                ...
                imported.sites.every(
                    (site: any) => site.id && site.name && site.url
                );
            },
            {
                time: 2000,
        ...
        bench(
            ...
            () => {
                ...
                const hasAllSections = requiredSections.every(
                    (section) => section in parsed
                );

                // Validate required keys
                const hasMonitoringKeys =
                    parsed.monitoring &&
        ...
        bench(
            ...
            () => {
                ...
                const incrementalData = {
                    ...
                    recentHistory: backupData.history
                        .filter((h) => h.timestamp > changesSince)
                        .slice(0, 1000),
                    metadata: {
                        type: "incremental",
                        since: changesSince,
