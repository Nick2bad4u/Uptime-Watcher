...
function parseAndValidateJson(jsonString: string): { valid: boolean; data?: any; error?: string } {
    try {
...
describe("Validation Operations Performance Benchmarks", () => {
    ...
    const { sites, monitors, invalidData, complexObjects, malformedJson } = testData;

    describe("Schema Validation Benchmarks", () => {
        bench("validate single site object", () => {
        ...
        bench("validate bulk sites (1000 items)", () => {
        ...
        bench("validate single monitor object", () => {
        ...
        bench("validate bulk monitors (500 items)", () => {
            monitors.every(monitor => validateSchema(monitor, monitorSchema));
        }, {
            time: 2000,
            iterations: 200,
        });

        bench("validate invalid data (error paths)", () => {
    ...
    describe("Data Parsing Benchmarks", () => {
        ...
        bench("parse and validate simple JSON", () => {
        ...
        bench("parse and validate complex nested JSON", () => {
        ...
        bench("handle malformed JSON gracefully", () => {
        ...
        bench("parse individual monitor objects", () => {
    ...
    describe("Type Checking Benchmarks", () => {
        bench("check number ranges (timeout values)", () => {
        ...
        bench("check number ranges (intervals)", () => {
            monitors.every(monitor => 
        ...
        bench("type checking primitives", () => {
            const values = [
                "string", 123, true, [], {}, null, undefined,
                "another string", 456, false, [1,2,3], {a:1}
            ];
    ...
    describe("String Sanitization Benchmarks", () => {
        ...
        bench("sanitize bulk strings (1000 items)", () => {
    ...
    describe("Complex Validation Workflows", () => {
        bench("full site validation pipeline", () => {
        ...
        bench("full monitor validation pipeline", () => {
            monitors.slice(0, 100).every(monitor => {
                // Schema validation
                if (!validateSchema(monitor, monitorSchema)) return false;
                
                // Range validation
                if (!validateNumberRange(monitor.timeout, 1000, 30000)) return false;
                if (!validateNumberRange(monitor.checkInterval, 10000, 300000)) return false;
        ...
        bench("validate complex nested objects", () => {
            complexObjects.slice(0, 50).every(obj => {
                // Check structure exists
                if (!obj.nested?.level1?.level2?.level3?.data) return false;
                
                // Validate nested arrays
                if (!Array.isArray(obj.nested.level1.level2.level3.data)) return false;
                ...
                const numbersValid = obj.arrays.numbers.every((n: any) => typeof n === "number");
                const stringsValid = obj.arrays.strings.every((s: any) => typeof s === "string");
    ...
    describe("Error Recovery Benchmarks", () => {
        bench("recover from validation errors", () => {
            invalidData.forEach(item => {
                try {
                    ...
                    const fallback = {
                        ...
                        monitors: Array.isArray(item.monitors) ? item.monitors : []
                    };
        ...
        bench("handle JSON parsing failures gracefully", () => {
            malformedJson.forEach(json => {
                ...
                if (!result.valid) {
                    ...
                    try {
                        ...
                        const partial = json.substring(0, json.indexOf('{') + 1) + '}';
