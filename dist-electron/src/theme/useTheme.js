...
function useTheme() {
    ...
    const getColor = (path) => {
        ...
        const value = keys.reduce((acc, key) => acc && typeof acc === "object" && Object.prototype.hasOwnProperty.call(acc, key)
            ? // eslint-disable-next-line security/detect-object-injection -- Object.prototype.hasOwnProperty ensures safety
                acc[key]
            : undefined, currentTheme.colors);
        return typeof value === "string" ? value : "#000000";
    };
    // Get status color
    const getStatusColor = (status) => {
        // Only allow known status keys
        const allowedStatuses = ["up", "down", "pending", "unknown"];
