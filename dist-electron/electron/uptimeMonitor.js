...
class UptimeMonitor extends events_1.EventEmitter {
        ...
        try {
            ...
            for (const siteRow of siteRows) {
                ...
                for (const row of monitorRows) {
                    ...
                    const historyRows = (await this.db.all("SELECT timestamp, status, responseTime, details FROM history WHERE monitor_id = ? ORDER BY timestamp DESC", [String(row.id)]));
    ...
    async checkMonitor(site, monitorId) {
        ...
        try {
            await this.db.run(`INSERT INTO history (monitor_id, timestamp, status, responseTime, details) VALUES (?, ?, ?, ?, ?)`, [monitor.id, historyEntry.timestamp, historyEntry.status, historyEntry.responseTime, details]);
