...
function requireGetIntrinsic() {
  ...
  var needsEval = {};
  var TypedArray =
    typeof Uint8Array === "undefined" || !getProto2
      ? undefined$1
      : getProto2(Uint8Array);
  var INTRINSICS = {
    __proto__: null,
    "%AggregateError%":
    ...
    "%ArrayIteratorPrototype%":
      hasSymbols2 && getProto2 ? getProto2([][Symbol.iterator]()) : undefined$1,
    ...
    "%EvalError%": $EvalError,
    "%Float16Array%":
      typeof Float16Array === "undefined" ? undefined$1 : Float16Array,
    "%Float32Array%":
...
function requireEsSetTostringtag() {
  ...
  esSetTostringtag = function setToStringTag(object, value) {
    ...
    if (
      (typeof overrideIfSet !== "undefined" &&
        typeof overrideIfSet !== "boolean") ||
      (typeof nonConfigurable !== "undefined" &&
        typeof nonConfigurable !== "boolean")
    ) {
      throw new $TypeError(
...
function requireForm_data() {
  ...
  FormData2.prototype._getContentDisposition = function (value, options) {
    ...
    if (typeof options.filepath === "string") {
      ...
      filename = path$12.basename(
        options.filename || (value && (value.name || value.path)),
      );
...
const proxyFromEnv =
  /* @__PURE__ */ getDefaultExportFromCjs(proxyFromEnvExports);
...
function requireFollowRedirects() {
  ...
  function spreadUrlObject(urlObject, target) {
    ...
    spread2.path = spread2.search
      ? spread2.pathname + spread2.search
      : spread2.pathname;
    return spread2;
  }
...
const followRedirects = /* @__PURE__ */ getDefaultExportFromCjs(
      ...
      async function dispatchHttpRequest(resolve, reject, onDone) {
        ...
        function abort(reason) {
          emitter.emit(
            "abort",
            !reason || reason.type
              ? new CanceledError$1(null, config, req)
              : reason,
          );
        }
        ...
        const parsed = new URL(
          fullPath,
          platform.hasBrowserEnv ? platform.origin : void 0,
        );
        ...
        req = transport.request(options, function handleResponse(res) {
          ...
          responseStream =
            streams.length > 1
              ? stream.pipeline(streams, utils$1.noop)
              : streams[0];
  ...
  if (utils$1.isFormData(data)) {
    if (
      ...
      const [type2, ...tokens] = contentType
        ...
        : [];
      headers.setContentType(
        [type2 || "multipart/form-data", ...tokens].join("; "),
      );
    }
  }
...
function getTempFilename(file) {
  const f =
    file instanceof URL ? node_url.fileURLToPath(file) : file.toString();
  return path$1.join(path$1.dirname(f), `.${path$1.basename(f)}.tmp`);
}
...
add_fn = function (data) {
  ...
  return new Promise((resolve, reject) => {
    var _a;
    (_a = __privateGet(this, _nextPromise)) == null
      ? void 0
    ...
    await retryAsyncOperation(
      async () => {
        await promises.rename(
          __privateGet(this, _tempFilename),
          __privateGet(this, _filename),
        );
      },
      10,
      100,
...
class DataFile {
  ...
  write(obj) {
    return __privateGet(this, _adapter).write(
      __privateGet(this, _stringify).call(this, obj),
    );
  }
}
...
async function JSONFilePreset(filename, defaultData) {
  const adapter =
    process.env.NODE_ENV === "test" ? new Memory() : new JSONFile(filename);
  const db = new Low(adapter, defaultData);
  await db.read();
  return db;
}
class UptimeMonitor extends events.EventEmitter {
  constructor() {
    super();
  ...
  async initDatabase() {
    const dbPath = path.join(electron.app.getPath("userData"), "uptime.json");
    const defaultData = {
      sites: [],
      settings: {
        checkInterval: 6e4,
  ...
  async checkAllSites() {
    const promises2 = Array.from(this.sites.values()).map((site) =>
      this.checkSite(site),
    );
...
class Main {
  ...
  setupIPC() {
    electron.ipcMain.handle("add-site", async (_, site) => {
      return this.uptimeMonitor.addSite(site);
    });
    electron.ipcMain.handle("remove-site", async (_, url) => {
      return this.uptimeMonitor.removeSite(url);
    });
    electron.ipcMain.handle("get-sites", async () => {
    ...
    this.uptimeMonitor.on("status-update", (data) => {
      var _a;
      (_a = this.mainWindow) == null
        ? void 0
        : _a.webContents.send("status-update", data);
    });
