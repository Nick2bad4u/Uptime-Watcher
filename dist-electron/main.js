...
function isDev() {
    return process.env.NODE_ENV === "development" || !require$$0$1.app.isPackaged;
}
...
function requireCombined_stream() {
    ...
    CombinedStream.isStreamLike = function (stream2) {
        return (
            typeof stream2 !== "function" &&
            typeof stream2 !== "string" &&
            typeof stream2 !== "boolean" &&
            typeof stream2 !== "number" &&
            !Buffer.isBuffer(stream2)
        );
    };
...
function requireGetIntrinsic() {
    ...
    var INTRINSICS = {
        ...
        "%MapIteratorPrototype%":
            typeof Map === "undefined" || !hasSymbols2 || !getProto2
                ? undefined$1
        ...
        "%SetIteratorPrototype%":
            typeof Set === "undefined" || !hasSymbols2 || !getProto2
                ? undefined$1
...
function requireForm_data() {
    ...
    FormData2.prototype._trackLength = function (header, value, options) {
        ...
        if (
            !value ||
            (!value.path && !(value.readable && hasOwn(value, "httpVersion")) && !(value instanceof Stream))
        ) {
            return;
        }
...
function toFormData$1(obj, formData, options) {
    ...
    function build(value, path2) {
        ...
        utils$1.forEach(value, function each(el, key) {
            const result =
                !(utils$1.isUndefined(el) || el === null) &&
                visitor.call(formData, el, utils$1.isString(key) ? key.trim() : key, path2, exposedHelpers);
            if (result === true) {
...
const isURLSameOrigin = platform.hasStandardBrowserEnv
    ? /* @__PURE__ */ ((origin2, isMSIE) => (url) => {
          ...
          return (
              origin2.protocol === url.protocol && origin2.host === url.host && (isMSIE || origin2.port === url.port)
          );
...
const resolveConfig = (config) => {
    ...
    if (utils$1.isFormData(data)) {
        if (platform.hasStandardBrowserEnv || platform.hasStandardBrowserWebWorkerEnv) {
            ...
            const [type2, ...tokens] = contentType
                ? contentType
                      .split(";")
...
isFetchSupported &&
    ((res) => {
        ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type2) => {
            !resolvers[type2] &&
                (resolvers[type2] = utils$1.isFunction(res[type2])
                    ? (res2) => res2[type2]()
                    : (_, config) => {
...
class Writer {
    ...
    async write(data) {
        return __privateGet(this, _locked)
            ? __privateMethod(this, _Writer_instances, add_fn).call(this, data)
            : __privateMethod(this, _Writer_instances, write_fn).call(this, data);
    }
}
...
add_fn = function (data) {
    ...
    __privateGet(this, _nextPromise) ||
        __privateSet(
            this,
            _nextPromise,
    ...
    class ElectronExternalApi extends NodeExternalApi {
        ...
        getAppName() {
            ...
            try {
                appName =
                    this.appName ||
                    ((_a = this.electron.app) == null ? void 0 : _a.name) ||
                    ((_b = this.electron.app) == null ? void 0 : _b.getName());
            } catch {}
        ...
        onEveryWebContentsEvent(eventName, handler) {
            var _a, _b, _c;
            (_b = (_a = this.electron.webContents) == null ? void 0 : _a.getAllWebContents()) == null
                ? void 0
                : _b.forEach((webContents) => {
                      webContents.on(eventName, handler);
                  });
            (_c = this.electron.app) == null ? void 0 : _c.on("web-contents-created", onWebContentsCreated);
            return () => {
                var _a2, _b2;
                (_a2 = this.electron.webContents) == null
                    ? void 0
                    : _a2.getAllWebContents().forEach((webContents) => {
        ...
        onIpcInvoke(channel, listener) {
            var _a, _b;
            (_b = (_a = this.electron.ipcMain) == null ? void 0 : _a.handle) == null
                ? void 0
                : _b.call(_a, channel, listener);
        }
        /**
         * @param {string} url
         * @param {Function} [logFunction]
        ...
        sendIpc(channel, message) {
            var _a, _b;
            (_b = (_a = this.electron.BrowserWindow) == null ? void 0 : _a.getAllWindows()) == null
                ? void 0
                : _b.forEach((wnd) => {
                      var _a2, _b2;
                      if (
                          ((_a2 = wnd.webContents) == null ? void 0 : _a2.isDestroyed()) === false &&
                          ((_b2 = wnd.webContents) == null ? void 0 : _b2.isCrashed()) === false
                      ) {
...
function requireInitialize() {
    ...
    function initializePreload({ externalApi, getSessions, includeFutureSession, preloadOption }) {
        let preloadPath = typeof preloadOption === "string" ? preloadOption : void 0;
        try {
            preloadPath = path2.resolve(__dirname, "../renderer/electron-log-preload.js");
        } catch {}
        if (!preloadPath || !fs.existsSync(preloadPath)) {
            preloadPath = path2.join(externalApi.getAppUserDataPath() || os.tmpdir(), "electron-log-preload.js");
            const preloadCode = `
      try {
...
function requireErrorHandler() {
    ...
    class ErrorHandler {
        ...
        createIssue(pageUrl, queryParams) {
            var _a;
            (_a = this.externalApi) == null
                ? void 0
                : _a.openUrl(`${pageUrl}?${new URLSearchParams(queryParams).toString()}`);
        }
        handleError(error) {
...
function requireEventLogger() {
    ...
    class EventLogger {
            ...
            __publicField(this, "formatters", {
                app: {
                    "certificate-error": ({ args }) => {
                        return this.arrayToObject(args.slice(1, 4), ["url", "error", "certificate"]);
                    },
        ...
        setOptions({ events, externalApi, level, logger: logger2, format: format2, formatters, scope: scope2 }) {
        ...
        startLogging(options = {}) {
            ...
            for (const eventName of this.getEventNames(this.events.webContents)) {
                this.disposers.push(
                    this.externalApi.onEveryWebContentsEvent(eventName, (...handlerArgs) => {
                        this.handleEvent({ eventSource: "webContents", eventName, handlerArgs });
                    })
                );
            }
        }
...
function requireFormat() {
    ...
    function timeZoneFromOffset(minutesOffset) {
        ...
        const hours = Math.floor(minutesPositive / 60)
            .toString()
            .padStart(2, "0");
...
function requireConsole() {
    ...
    function consoleTransportFactory(logger2) {
        return Object.assign(transport, {
            ...
            level: "silly",
            transforms: [addTemplateColors, format2, formatStyles, concatFirstStringElements, maxDepth, toJSON2],
...
function requireFile$1() {
    ...
    class File extends EventEmitter {
        constructor({ path: path2, writeOptions = { encoding: "utf8", flag: "a", mode: 438 }, writeAsync = false }) {
            super();
        ...
        nextAsyncWrite() {
            ...
            fs.writeFile(this.path, text, this.writeOptions, (e) => {
                ...
                if (e) {
                    file2.emit("error", new Error(`Couldn't write to ${file2.path}. ${e.message}`), this);
                } else {
...
function requireFile() {
    ...
    const { format: format2, concatFirstStringElements } = requireFormat();
    ...
    function fileTransportFactory(logger2, { registry = globalRegistry, externalApi } = {}) {
        ...
        function readAllLogs({ fileFilter = (f) => f.endsWith(".log") } = {}) {
            ...
            return fs
                ...
                .map((fileName) => path2.join(logsPath, fileName))
                .filter(fileFilter)
                .map((logPath) => {
                    try {
                        return {
...
function requireIpc() {
    ...
    function ipcTransportFactory(logger2, { externalApi }) {
        ...
        function transport(message) {
            var _a;
            if (
                ((_a = message == null ? void 0 : message.variables) == null ? void 0 : _a.processType) === "renderer"
            ) {
                return;
            }
            externalApi == null
...
class UptimeMonitor extends require$$0$4.EventEmitter {
    constructor() {
        super();
    ...
    async initDatabase() {
        const dbPath = require$$2.join(require$$0$1.app.getPath("userData"), "uptime.json");
        const defaultData = {
            sites: [],
            settings: {
                checkInterval: 6e4,
    ...
    startMonitoring() {
        if (this.isMonitoring) {
            logger$1.debug("Monitoring already running");
            return;
        }
        logger$1.info(`Starting monitoring with ${this.sites.size} sites (interval: ${this.checkInterval}ms)`);
    ...
    stopMonitoring() {
        if (this.monitoringInterval) {
            logger$1.info("Stopping monitoring");
            clearInterval(this.monitoringInterval);
            this.monitoringInterval = null;
        } else {
            logger$1.debug("No monitoring interval to stop");
        }
        this.isMonitoring = false;
    }
    async checkAllSites() {
        logger$1.debug(`Checking ${this.sites.size} sites`);
    ...
    async importData(data) {
        ...
        try {
            ...
            logger$1.error("Failed to import data", error);
            return false;
        }
    }
}
...
class Main {
    constructor() {
        ...
        __publicField(this, "uptimeMonitor");
    ...
    createMainWindow() {
        ...
        this.mainWindow = new require$$0$1.BrowserWindow({
            width: 1200,
            height: 800,
            minWidth: 800,
            minHeight: 600,
        ...
        if (isDev()) {
            ...
            this.mainWindow.loadFile(require$$2.join(__dirname, "../dist/index.html"));
        }
    ...
    setupIPC() {
        require$$0$1.ipcMain.handle("add-site", async (_, site) => {
            return this.uptimeMonitor.addSite(site);
        });
        require$$0$1.ipcMain.handle("remove-site", async (_, url) => {
            return this.uptimeMonitor.removeSite(url);
        });
        require$$0$1.ipcMain.handle("get-sites", async () => {
            return this.uptimeMonitor.getSites();
        });
        require$$0$1.ipcMain.handle("update-check-interval", async (_, interval) => {
            return this.uptimeMonitor.setCheckInterval(interval);
        });
        require$$0$1.ipcMain.handle("get-check-interval", async () => {
            return this.uptimeMonitor.getCheckInterval();
        });
        require$$0$1.ipcMain.handle("update-history-limit", async (_, limit) => {
            return this.uptimeMonitor.setHistoryLimit(limit);
        });
        require$$0$1.ipcMain.handle("get-history-limit", async () => {
            return this.uptimeMonitor.getHistoryLimit();
        });
        require$$0$1.ipcMain.handle("start-monitoring", async () => {
            this.uptimeMonitor.startMonitoring();
            return true;
        });
        require$$0$1.ipcMain.handle("stop-monitoring", async () => {
            this.uptimeMonitor.stopMonitoring();
            return true;
        });
        require$$0$1.ipcMain.handle("check-site-now", async (_, url) => {
            return this.uptimeMonitor.checkSiteManually(url);
        });
        require$$0$1.ipcMain.handle("export-data", async () => {
            return this.uptimeMonitor.exportData();
        });
        require$$0$1.ipcMain.handle("import-data", async (_, data) => {
            return this.uptimeMonitor.importData(data);
        });
        require$$0$1.ipcMain.handle("update-site", async (_, url, updates) => {
        ...
        this.uptimeMonitor.on("site-down", (site) => {
            ...
            if (require$$0$1.Notification.isSupported()) {
                new require$$0$1.Notification({
        ...
        this.uptimeMonitor.on("site-restored", (site) => {
