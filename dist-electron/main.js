...
function requireMain$2() {
  ...
  (function(exports) {
    ...
    var __exportStar = main$2 && main$2.__exportStar || function(m, exports2) {
...
function isDev() {
  return process.env.NODE_ENV === "development" || !require$$1$6.app.isPackaged;
}
...
const URLSearchParams$1 = require$$0$3.URLSearchParams;
...
const httpAdapter = isHttpAdapterSupported && function httpAdapter2(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (config.transport) {
      ...
      transport = isHttpsRequest ? require$$4$1 : require$$4;
    } else {
...
const logger$1 = {
  ...
  error: (message, error2, ...args) => {
    if (error2 instanceof Error) {
      log.error(`[MONITOR] ${message}`, { message: error2.message, stack: error2.stack }, ...args);
    } else {
      log.error(`[MONITOR] ${message}`, error2, ...args);
    }
  },
  debug: (message, ...args) => log.debug(`[MONITOR] ${message}`, ...args),
  warn: (message, ...args) => log.warn(`[MONITOR] ${message}`, ...args)
};
class UptimeMonitor extends require$$0$2.EventEmitter {
  constructor() {
    super();
  ...
  async loadSites() {
    ...
    if (typeof ((_b = this.db.data.settings) == null ? void 0 : _b.historyLimit) === "number") {
  ...
  async addSite(siteData) {
    ...
    const monitors = siteData.monitors && siteData.monitors.length > 0 ? siteData.monitors.map((m) => ({ ...m, history: m.history || [] })) : [{ type: "http", status: "pending", history: [] }];
    const site = {
      id,
      ...siteData,
      monitors
  ...
  async checkAllSites() {
    ...
    const promises2 = Array.from(this.sites.values()).flatMap(
      (site) => site.monitors.map((monitor) => this.checkMonitor(site, monitor.type))
    );
  ...
  async importData(data) {
    ...
    try {
      ...
      logger$1.error("Failed to import data", error2);
      return false;
    }
  }
}
...
const logger = {
  ...
  error: (message, error2, ...args) => {
    if (error2 instanceof Error) {
      log.error(`[MAIN] ${message}`, { message: error2.message, stack: error2.stack }, ...args);
    } else {
      log.error(`[MAIN] ${message}`, error2, ...args);
    }
  },
...
class Main {
  ...
  createMainWindow() {
    ...
    this.mainWindow = new require$$1$6.BrowserWindow({
      width: 1200,
      height: 800,
      minWidth: 800,
      minHeight: 600,
    ...
    if (isDev()) {
      ...
      this.mainWindow.loadFile(require$$1$2.join(__dirname, "../dist/index.html"));
    }
  ...
  setupIPC() {
    ...
    require$$1$6.ipcMain.handle("update-check-interval", async (_, interval) => {
      return this.uptimeMonitor.setCheckInterval(interval);
    });
    require$$1$6.ipcMain.handle("get-check-interval", async () => {
      return this.uptimeMonitor.getCheckInterval();
    });
    require$$1$6.ipcMain.handle("update-history-limit", async (_, limit) => {
      return this.uptimeMonitor.setHistoryLimit(limit);
    });
    require$$1$6.ipcMain.handle("get-history-limit", async () => {
      return this.uptimeMonitor.getHistoryLimit();
    });
    require$$1$6.ipcMain.handle("start-monitoring", async () => {
      this.uptimeMonitor.startMonitoring();
      return true;
    });
    require$$1$6.ipcMain.handle("stop-monitoring", async () => {
    ...
    require$$1$6.ipcMain.on("quit-and-install", () => {
  ...
  setupAutoUpdater() {
    ...
    mainExports$1.autoUpdater.on("checking-for-update", () => {
      var _a;
      (_a = this.mainWindow) == null ? void 0 : _a.webContents.send("update-status", { status: "checking" });
    });
    mainExports$1.autoUpdater.on("update-available", () => {
      var _a;
      (_a = this.mainWindow) == null ? void 0 : _a.webContents.send("update-status", { status: "available" });
    });
    mainExports$1.autoUpdater.on("update-not-available", () => {
      var _a;
      (_a = this.mainWindow) == null ? void 0 : _a.webContents.send("update-status", { status: "idle" });
    });
    mainExports$1.autoUpdater.on("download-progress", () => {
      var _a;
      (_a = this.mainWindow) == null ? void 0 : _a.webContents.send("update-status", { status: "downloading" });
    });
    mainExports$1.autoUpdater.on("update-downloaded", () => {
      var _a;
      (_a = this.mainWindow) == null ? void 0 : _a.webContents.send("update-status", { status: "downloaded" });
    });
    mainExports$1.autoUpdater.on("error", (err) => {
      var _a;
      (_a = this.mainWindow) == null ? void 0 : _a.webContents.send("update-status", { status: "error", error: (err == null ? void 0 : err.message) || String(err) });
    });
