...
describe('EnhancedMonitorChecker Coverage Tests', () => {
  ...
  describe('checkMonitor Method Coverage', () => {
    ...
    it('should handle monitor check returning null result', async () => {
      ...
      mockMonitorRepository.update.mockRejectedValue(new Error('Database failure'));

      const result = await enhancedChecker.checkMonitor(mockSite, 'monitor-1', false); // Use correlated check
  ...
  describe('startMonitoring Method Coverage', () => {
    ...
    it('should handle successful monitoring start with event emission', async () => {
      ...
      expect(result).toBe(true);
      expect(mockEventBus.emitTyped).toHaveBeenCalledWith('internal:monitor:started', expect.objectContaining({
        identifier: 'test-site',
        monitorId: 'monitor-2',
        operation: 'started'
      }));
    ...
    it('should handle event emission error gracefully', async () => {
      mockEventBus.emitTyped.mockRejectedValue(new Error('Event emission failed'));
  ...
  describe('stopMonitoring Method Coverage', () => {
    ...
    it('should handle successful stop with event emission', async () => {
      ...
      expect(result).toBe(true);
      expect(mockEventBus.emitTyped).toHaveBeenCalledWith('internal:monitor:stopped', expect.objectContaining({
        identifier: 'test-site',
        monitorId: 'monitor-2',
        operation: 'stopped',
        reason: 'user'
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle monitor factory returning null', async () => {
      ...
      mockMonitorRepository.update.mockRejectedValue(new Error('Database failure during setup'));
