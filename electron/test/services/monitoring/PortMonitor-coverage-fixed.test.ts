/**
 * @fileoverview Comprehensive coverage tests for PortMonitor.ts
 * This file addresses 42.37% coverage by testing all public methods and error paths
 */
...
describe('PortMonitor Coverage Tests', () => {
  ...
  describe('check method', () => {
    it('should successfully check a valid port monitor', async () => {
      ...
      expect(vi.mocked(validateMonitorHostAndPort)).toHaveBeenCalledWith(validPortMonitor);
      expect(vi.mocked(extractMonitorConfig)).toHaveBeenCalledWith(validPortMonitor, 5000);
    ...
    it('should throw error for non-port monitor type', async () => {
      ...
      await expect(portMonitor.check(httpMonitor)).rejects.toThrow(
        'PortMonitor can only check port-type monitors'
      );
    });
    ...
    it('should handle validation errors from validateMonitorHostAndPort', async () => {
      ...
      vi.mocked(validateMonitorHostAndPort).mockReturnValue(validationError);

      const result = await portMonitor.check(validPortMonitor);

      expect(vi.mocked(createMonitorErrorResult)).toHaveBeenCalledWith(validationError, 0);
    ...
    it('should handle monitor with missing host', async () => {
      ...
      expect(vi.mocked(createMonitorErrorResult)).toHaveBeenCalledWith(
        'Host is required for port monitoring',
        0
      );
    });
    ...
    it('should handle monitor with missing port', async () => {
      ...
      expect(vi.mocked(createMonitorErrorResult)).toHaveBeenCalledWith(
        'Port is required for port monitoring',
        0
      );
    });
    ...
    it('should use monitor-specific timeout and retry configuration', async () => {
      ...
      expect(vi.mocked(extractMonitorConfig)).toHaveBeenCalledWith(monitorWithCustomConfig, 5000);
      expect(vi.mocked(performPortCheckWithRetry)).toHaveBeenCalledWith(
        'example.com',
        443,
        {
    ...
    it('should handle different host formats', async () => {
      const hosts = ['example.com', 'localhost', '192.168.1.1', '2001:db8::1'];
    ...
    it('should use service default timeout when monitor timeout is undefined', async () => {
      ...
      expect(vi.mocked(extractMonitorConfig)).toHaveBeenCalledWith(monitorWithoutTimeout, 5000);
    });
    ...
    it('should work with custom service timeout configuration', async () => {
      ...
      vi.mocked(performPortCheckWithRetry).mockResolvedValue(customErrorResult);

      expect(vi.mocked(extractMonitorConfig)).toHaveBeenCalledWith(validPortMonitor, 10_000);
    });
  });
  ...
  describe('Error Handling Edge Cases', () => {
    ...
    it('should handle empty string validation error', async () => {
      ...
      expect(vi.mocked(createMonitorErrorResult)).toHaveBeenCalledWith('', 0);
    });
