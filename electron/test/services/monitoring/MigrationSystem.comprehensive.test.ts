 ...
 *
 * @remarks
 * Tests the migration system for monitor types including registry, orchestrator,
 * version manager, and migration path calculation. Ensures robust migration
 * workflows and error handling across all components.
 */
...
function createTestMigrationRule(
    ...
    transformFn?: (data: Record<string, unknown>) => Promise<Record<string, unknown>>
): MigrationRule {
    return {
        description,
        fromVersion,
        isBreaking,
        toVersion,
        transform: transformFn || ((data) => Promise.resolve({ ...data, migrated: true })),
    };
}
...
describe("MigrationSystem - Comprehensive Coverage", () => {
    ...
    describe("MigrationRegistry", () => {
        describe("registerMigration", () => {
            ...
            it("should register multiple rules and sort them by version", () => {
                ...
                const path = migrationRegistry.getMigrationPath("http", "0.9.0", "1.2.0");
        ...
        describe("getMigrationPath", () => {
            it("should return empty path for same versions", () => {
                const path = migrationRegistry.getMigrationPath("http", "1.0.0", "1.0.0");
                expect(path).toEqual([]);
            });
            ...
            it("should return correct migration path for valid route", () => {
                ...
                const path = migrationRegistry.getMigrationPath("http", "1.0.0", "1.2.0");
            ...
            it("should throw error for invalid version strings", () => {
                expect(() => {
                    migrationRegistry.getMigrationPath("http", "invalid", "1.1.0");
                }).toThrow("fromVersion \"invalid\" is not a valid semantic version");
                
                expect(() => {
                    migrationRegistry.getMigrationPath("http", "1.0.0", "invalid");
                }).toThrow("toVersion \"invalid\" is not a valid semantic version");
            });
            ...
            it("should throw error for non-string version parameters", () => {
                expect(() => {
                    migrationRegistry.getMigrationPath("http", null as any, "1.1.0");
                }).toThrow("fromVersion must be a non-empty string");
                
                expect(() => {
                    migrationRegistry.getMigrationPath("http", "1.0.0", undefined as any);
                }).toThrow("toVersion must be a non-empty string");
            });

            it("should throw error when no migration path exists", () => {
                expect(() => {
                    migrationRegistry.getMigrationPath("http", "1.0.0", "2.0.0");
            ...
            it("should detect circular migration paths", () => {
                ...
                expect(() => {
                    migrationRegistry.getMigrationPath("http", "1.0.0", "1.2.0");
            ...
            it("should throw error for paths exceeding maximum steps", () => {
                ...
                for (let i = 0; i < 105; i++) {
                    const rule = createTestMigrationRule(`1.${i}.0`, `1.${i + 1}.0`);
                    migrationRegistry.registerMigration("http", rule);
                }
                
                expect(() => {
                    migrationRegistry.getMigrationPath("http", "1.0.0", "1.105.0");
                }).toThrow("Migration path too long for http");
            });

            it("should validate version string with numeric parts", () => {
                expect(() => {
                    migrationRegistry.getMigrationPath("http", "1.0.0", "999999999999999999999.0.0");
        ...
        describe("canMigrate", () => {
            it("should return true when migration path exists", () => {
                ...
                expect(migrationRegistry.canMigrate("http", "1.0.0", "1.1.0")).toBe(true);
            });

            it("should return false when no migration path exists", () => {
                expect(migrationRegistry.canMigrate("http", "1.0.0", "2.0.0")).toBe(false);
            });

            it("should return true for same versions", () => {
                expect(migrationRegistry.canMigrate("http", "1.0.0", "1.0.0")).toBe(true);
            });

            it("should return false for invalid version strings", () => {
                expect(migrationRegistry.canMigrate("http", "invalid", "1.1.0")).toBe(false);
            });
        });
    ...
    describe("VersionManager", () => {
        ...
        describe("isVersionApplied", () => {
            ...
            it("should return false for different version", () => {
                versionManager.setVersion("http", "1.1.0");
                expect(versionManager.isVersionApplied("http", "1.0.0")).toBe(false);
            });

            it("should return false for unset monitor type", () => {
                expect(versionManager.isVersionApplied("unknown", "1.0.0")).toBe(false);
            });
        });
        ...
        describe("getAllVersions", () => {
            ...
            it("should include timestamp and applied status", () => {
                ...
                expect(httpVersion?.timestamp).toBeGreaterThanOrEqual(beforeTime);
    ...
    describe("MigrationOrchestrator", () => {
        ...
        describe("migrateMonitorData", () => {
            it("should return success for same versions", async () => {
                ...
                const result = await orchestrator.migrateMonitorData("http", data, "1.0.0", "1.0.0");
            ...
            it("should apply single migration successfully", async () => {
                const transformFn = vi.fn((data) => Promise.resolve({ ...data, timeout: 30000 }));
                const rule = createTestMigrationRule("1.0.0", "1.1.0", "Add timeout", false, transformFn);
                migrationRegistry.registerMigration("http", rule);
                
                const data = createTestData();
                const result = await orchestrator.migrateMonitorData("http", data, "1.0.0", "1.1.0");
                
                expect(result.success).toBe(true);
                expect(result.appliedMigrations).toEqual(["1.0.0_to_1.1.0"]);
                expect(result.data).toEqual({ ...data, timeout: 30000 });
                expect(result.errors).toEqual([]);
                expect(transformFn).toHaveBeenCalledWith(data);
            });

            it("should apply multiple migrations in sequence", async () => {
                const rule1 = createTestMigrationRule("1.0.0", "1.1.0", "First migration", false, 
                    (data) => Promise.resolve({ ...data, step1: true }));
                const rule2 = createTestMigrationRule("1.1.0", "1.2.0", "Second migration", false,
                ...
                const data = createTestData();
                const result = await orchestrator.migrateMonitorData("http", data, "1.0.0", "1.2.0");
                
                expect(result.success).toBe(true);
                expect(result.appliedMigrations).toEqual(["1.0.0_to_1.1.0", "1.1.0_to_1.2.0"]);
                expect(result.data).toEqual({ ...data, step1: true, step2: true });
            });

            it("should handle breaking migration with warning", async () => {
                const rule = createTestMigrationRule("1.0.0", "2.0.0", "Breaking change", true);
                migrationRegistry.registerMigration("http", rule);
                
                const data = createTestData();
                const result = await orchestrator.migrateMonitorData("http", data, "1.0.0", "2.0.0");
                
                expect(result.success).toBe(true);
                expect(result.warnings).toContain("Applied breaking migration: Breaking change");
            });

            it("should handle migration transform error", async () => {
                const errorMessage = "Transform failed";
                const rule = createTestMigrationRule("1.0.0", "1.1.0", "Failing migration", false,
                ...
                expect(result.errors[0]).toContain("Migration failed: Failing migration");
            ...
            it("should stop applying migrations after first error", async () => {
                const rule1 = createTestMigrationRule("1.0.0", "1.1.0", "Failing migration", false,
            ...
            it("should handle orchestration errors gracefully", async () => {
                ...
                expect(result.success).toBe(false);
                expect(result.errors).toHaveLength(1);
                expect(result.errors[0]).toContain("Migration orchestration failed");
            });
            ...
            it("should update version manager on successful migration", async () => {
                ...
                const data = createTestData();
                await orchestrator.migrateMonitorData("http", data, "1.0.0", "1.1.0");
            ...
            it("should not update version when no migrations applied", async () => {
                const data = createTestData();
                await orchestrator.migrateMonitorData("http", data, "1.0.0", "1.0.0");
    ...
    describe("Example Migrations", () => {
        describe("httpV1_0_to_1_1", () => {
            it("should add default timeout when missing", async () => {
                ...
                const result = await exampleMigrations.httpV1_0_to_1_1.transform(data);
            ...
            it("should preserve existing timeout", async () => {
                const data = { url: "https://example.com", timeout: 60000 };
                const result = await exampleMigrations.httpV1_0_to_1_1.transform(data);
            ...
            it("should have correct migration properties", () => {
                expect(exampleMigrations.httpV1_0_to_1_1.fromVersion).toBe("1.0.0");
                expect(exampleMigrations.httpV1_0_to_1_1.toVersion).toBe("1.1.0");
                expect(exampleMigrations.httpV1_0_to_1_1.isBreaking).toBe(false);
                expect(exampleMigrations.httpV1_0_to_1_1.description).toContain("timeout");
            });
        });
        ...
        describe("portV1_0_to_1_1", () => {
            it("should convert string port to number", async () => {
                ...
                const result = await exampleMigrations.portV1_0_to_1_1.transform(data);
            ...
            it("should throw for non-numeric port type", () => {
                ...
                expect(() => exampleMigrations.portV1_0_to_1_1.transform(data))
                    .toThrow("Port must be a number or numeric string, got: boolean");
            });

            it("should handle edge case ports (1 and 65535)", async () => {
                const data1 = { host: "localhost", port: "1" };
                const result1 = await exampleMigrations.portV1_0_to_1_1.transform(data1);
                expect(result1.port).toBe(1);

                const data2 = { host: "localhost", port: 65535 };
                const result2 = await exampleMigrations.portV1_0_to_1_1.transform(data2);
                expect(result2.port).toBe(65535);
            });

            it("should have correct migration properties", () => {
                expect(exampleMigrations.portV1_0_to_1_1.fromVersion).toBe("1.0.0");
                expect(exampleMigrations.portV1_0_to_1_1.toVersion).toBe("1.1.0");
                expect(exampleMigrations.portV1_0_to_1_1.isBreaking).toBe(false);
                expect(exampleMigrations.portV1_0_to_1_1.description).toContain("port");
            });
        });
    });
    ...
    describe("Integration Tests", () => {
        it("should handle complete migration workflow with example migrations", async () => {
            ...
            const httpResult = await orchestrator.migrateMonitorData("http", httpData, "1.0.0", "1.1.0");
            
            expect(httpResult.success).toBe(true);
            expect(httpResult.data?.timeout).toBe(30000);
            
            // Test port migration
            const portData = { host: "localhost", port: "8080" };
            const portResult = await orchestrator.migrateMonitorData("port", portData, "1.0.0", "1.1.0");
        ...
        it("should handle complex migration chains", async () => {
            ...
            const rules = [
                createTestMigrationRule("1.0.0", "1.1.0", "Add field A", false, 
                    (data) => Promise.resolve({ ...data, fieldA: "added" })),
                createTestMigrationRule("1.1.0", "1.2.0", "Modify field A", false,
                    (data) => Promise.resolve({ ...data, fieldA: "modified" })),
                createTestMigrationRule("1.2.0", "2.0.0", "Breaking change", true,
                    (data) => Promise.resolve({ ...data, version: 2 })),
            ];
            
            rules.forEach(rule => migrationRegistry.registerMigration("complex", rule));
            
            const orchestrator = createMigrationOrchestrator();
            const data = { id: "test" };
            const result = await orchestrator.migrateMonitorData("complex", data, "1.0.0", "2.0.0");
