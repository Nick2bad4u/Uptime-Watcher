...
describe("typedQueries - Comprehensive Database Query Helpers", () => {
    ...
    describe(insertWithReturning, () => {
        ...
        describe(queryForRecords, () => {
            ...
            it("should throw when validation fails for a row", () => {
                mockAll.mockReturnValue([{ foo: "bar" }]);

                expect(() =>
                    queryForRecords(mockDb, "SELECT * FROM invalid_rows", [], {
                        label: "TestRow",
                        validate: (
                            _row: UnknownRecord
                        ): _row is UnknownRecord => false,
            ...
            it("should handle records with parameters", async ({
                ...
                // Arrange
                interface MonitorRecord extends Record<
                    string | symbol,
                    unknown
                > {
                    id: number;
            ...
            it("should handle generic Record type", async ({
                task,
                annotate,
            }) => {
                await annotate(`Testing: ${task.name}`, "functional");
            ...
            it("should handle complex queries with JOINs", async ({
                ...
                // Arrange
                interface JoinedRecord extends Record<
                    string | symbol,
                    unknown
                > {
                    site_id: number;
        ...
        describe(queryForSingleRecord, () => {
            ...
            it("should throw when validation fails", () => {
                mockGet.mockReturnValue({ invalid: true });
