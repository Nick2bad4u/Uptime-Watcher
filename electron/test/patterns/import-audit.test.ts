...
describe("Import Pattern Audit", () => {
    ...
    function findBarrelExports(): string[] {
        ...
        for (const srcDir of srcDirs) {
            ...
            barrelFiles.push(
                ...files.filter((file) => path.basename(file) === "index.ts" || path.basename(file) === "index.tsx")
            );
        }
        ...
        it("should analyze import patterns across the codebase", () => {
            ...
            for (const file of allFiles.slice(0, 50)) {
                ...
                const analysis = {
                    ...
                    directImports: imports.filter(
                        (imp) => imp.startsWith(".") && !imp.endsWith("/index") && !imp.endsWith("/")
                    ).length,
            ...
            importAnalysis.slice(0, 10).forEach((analysis) => {
                console.log(`\n${analysis.file}:`);
                console.log(
                    `  Total: ${analysis.totalImports}, Relative: ${analysis.relativeImports}, External: ${analysis.externalImports}`
                );
            ...
            const barrelUsageRate =
                (importAnalysis.reduce((sum, a) => sum + a.barrelImports, 0) /
                    importAnalysis.reduce((sum, a) => sum + a.relativeImports, 0)) *
                100;
        ...
        it("should identify files that could benefit from barrel exports", () => {
            ...
            for (const file of allFiles.slice(0, 30)) {
                ...
                const deepImports = imports.filter(
                    (imp) => imp.startsWith(".") && (imp.match(/\.\.\//g) || []).length > 2
                );
