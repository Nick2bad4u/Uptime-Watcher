...
describe("eventTypes - Comprehensive Coverage", () => {
    describe("EVENT_CATEGORIES constant", () => {
        ...
        it("should contain expected events in each category", () => {
            ...
            expect(EVENT_CATEGORIES.DATABASE).toContain("database:backup-created");
            expect(EVENT_CATEGORIES.INTERNAL_DATABASE).toContain("internal:database:initialized");
            expect(EVENT_CATEGORIES.INTERNAL_MONITOR).toContain("internal:monitor:started");
            expect(EVENT_CATEGORIES.INTERNAL_SITE).toContain("internal:site:added");
            expect(EVENT_CATEGORIES.MONITOR).toContain("monitor:up");
            expect(EVENT_CATEGORIES.MONITORING).toContain("monitoring:started");
            expect(EVENT_CATEGORIES.PERFORMANCE).toContain("performance:metric");
    ...
    describe("EVENT_PRIORITIES constant", () => {
        it("should have all expected priority levels", () => {
            ...
            expect(actualPriorities).toEqual(expect.arrayContaining(expectedPriorities));
    ...
    describe("isEventOfCategory - Complete Branch Coverage", () => {
        describe("CACHE category", () => {
            it("should return true for cache events", () => {
                expect(isEventOfCategory("cache:invalidated", "CACHE")).toBe(true);
                expect(isEventOfCategory("site:cache-miss", "CACHE")).toBe(true);
                expect(isEventOfCategory("site:cache-updated", "CACHE")).toBe(true);
            });
        ...
        describe("CONFIG category", () => {
            it("should return true for config events", () => {
                expect(isEventOfCategory("config:changed", "CONFIG")).toBe(true);
            });
        ...
        describe("DATABASE category", () => {
            it("should return true for database events", () => {
                expect(isEventOfCategory("database:backup-created", "DATABASE")).toBe(true);
                expect(isEventOfCategory("database:error", "DATABASE")).toBe(true);
                expect(isEventOfCategory("database:retry", "DATABASE")).toBe(true);
                expect(isEventOfCategory("database:success", "DATABASE")).toBe(true);
                expect(isEventOfCategory("database:transaction-completed", "DATABASE")).toBe(true);
            });

            it("should return false for non-database events", () => {
                expect(isEventOfCategory("site:added", "DATABASE")).toBe(false);
                expect(isEventOfCategory("internal:database:initialized", "DATABASE")).toBe(false);
            });
        });

        describe("INTERNAL_DATABASE category", () => {
            it("should return true for internal database events", () => {
                expect(isEventOfCategory("internal:database:backup-downloaded", "INTERNAL_DATABASE")).toBe(true);
                expect(isEventOfCategory("internal:database:data-exported", "INTERNAL_DATABASE")).toBe(true);
                expect(isEventOfCategory("internal:database:data-imported", "INTERNAL_DATABASE")).toBe(true);
                expect(isEventOfCategory("internal:database:initialized", "INTERNAL_DATABASE")).toBe(true);
                expect(isEventOfCategory("internal:database:sites-refreshed", "INTERNAL_DATABASE")).toBe(true);
            });

            it("should return false for non-internal database events", () => {
                expect(isEventOfCategory("database:error", "INTERNAL_DATABASE")).toBe(false);
                expect(isEventOfCategory("site:added", "INTERNAL_DATABASE")).toBe(false);
            });
        });

        describe("INTERNAL_MONITOR category", () => {
            it("should return true for internal monitor events", () => {
                expect(isEventOfCategory("internal:monitor:all-started", "INTERNAL_MONITOR")).toBe(true);
                expect(isEventOfCategory("internal:monitor:all-stopped", "INTERNAL_MONITOR")).toBe(true);
                expect(isEventOfCategory("internal:monitor:manual-check-completed", "INTERNAL_MONITOR")).toBe(true);
                expect(isEventOfCategory("internal:monitor:started", "INTERNAL_MONITOR")).toBe(true);
                expect(isEventOfCategory("internal:monitor:stopped", "INTERNAL_MONITOR")).toBe(true);
            });

            it("should return false for non-internal monitor events", () => {
                expect(isEventOfCategory("monitor:up", "INTERNAL_MONITOR")).toBe(false);
                expect(isEventOfCategory("site:added", "INTERNAL_MONITOR")).toBe(false);
            });
        });

        describe("INTERNAL_SITE category", () => {
            it("should return true for internal site events", () => {
                expect(isEventOfCategory("internal:site:added", "INTERNAL_SITE")).toBe(true);
                expect(isEventOfCategory("internal:site:removed", "INTERNAL_SITE")).toBe(true);
                expect(isEventOfCategory("internal:site:updated", "INTERNAL_SITE")).toBe(true);
                expect(isEventOfCategory("internal:site:cache-updated", "INTERNAL_SITE")).toBe(true);
                expect(isEventOfCategory("internal:site:start-monitoring-requested", "INTERNAL_SITE")).toBe(true);
            });

            it("should return false for non-internal site events", () => {
                expect(isEventOfCategory("site:added", "INTERNAL_SITE")).toBe(false);
                expect(isEventOfCategory("monitor:up", "INTERNAL_SITE")).toBe(false);
            });
        });
        ...
        describe("MONITOR category", () => {
            it("should return true for monitor events", () => {
                ...
                expect(isEventOfCategory("monitor:check-completed", "MONITOR")).toBe(true);
                expect(isEventOfCategory("monitor:removed", "MONITOR")).toBe(true);
                expect(isEventOfCategory("monitor:status-changed", "MONITOR")).toBe(true);
            ...
            it("should return false for non-monitor events", () => {
                ...
                expect(isEventOfCategory("internal:monitor:started", "MONITOR")).toBe(false);
            });
        });

        describe("MONITORING category", () => {
            it("should return true for monitoring events", () => {
                expect(isEventOfCategory("monitoring:started", "MONITORING")).toBe(true);
                expect(isEventOfCategory("monitoring:stopped", "MONITORING")).toBe(true);
            });

            it("should return false for non-monitoring events", () => {
                expect(isEventOfCategory("monitor:up", "MONITORING")).toBe(false);
                expect(isEventOfCategory("site:added", "MONITORING")).toBe(false);
            });
        });

        describe("PERFORMANCE category", () => {
            it("should return true for performance events", () => {
                expect(isEventOfCategory("performance:metric", "PERFORMANCE")).toBe(true);
                expect(isEventOfCategory("performance:warning", "PERFORMANCE")).toBe(true);
            });

            it("should return false for non-performance events", () => {
                expect(isEventOfCategory("monitor:up", "PERFORMANCE")).toBe(false);
                expect(isEventOfCategory("site:added", "PERFORMANCE")).toBe(false);
            });
        });
        ...
        describe("SITE category", () => {
            it("should return true for site events", () => {
                ...
                expect(isEventOfCategory("sites:state-synchronized", "SITE")).toBe(true);
            });

            it("should return false for non-site events", () => {
                expect(isEventOfCategory("monitor:up", "SITE")).toBe(false);
                expect(isEventOfCategory("internal:site:added", "SITE")).toBe(false);
            });
        });
        ...
        describe("SYSTEM category", () => {
            it("should return true for system events", () => {
                ...
                expect(isEventOfCategory("system:shutdown", "SYSTEM")).toBe(true);
                expect(isEventOfCategory("system:startup", "SYSTEM")).toBe(true);
            });
        ...
        describe("Edge cases", () => {
            it("should handle events not in any category", () => {
                ...
                const result1 = isEventOfCategory("cache:invalidated", "MONITOR");
                const result2 = isEventOfCategory("config:changed", "PERFORMANCE");
    ...
    describe("getEventPriority - Complete Coverage", () => {
        ...
        describe("HIGH priority events", () => {
            it("should return HIGH for high priority events", () => {
                expect(getEventPriority("database:transaction-completed")).toBe("HIGH");
