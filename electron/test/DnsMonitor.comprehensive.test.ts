...
vi.mock('node:dns/promises', () => {
    const mockModule = {
        ...
        resolveMx: vi.fn(() => Promise.resolve([{ exchange: 'mail.example.com', priority: 10 }])),
        resolveTxt: vi.fn(() => Promise.resolve([['v=spf1 include:_spf.example.com ~all']])),
        resolveNs: vi.fn(() => Promise.resolve(['ns1.example.com'])),
        resolveSrv: vi.fn(() => Promise.resolve([{ name: 'service.example.com', port: 443, priority: 10, weight: 5 }])),
        resolvePtr: vi.fn(() => Promise.resolve(['example.com'])),
        resolveSoa: vi.fn(() => Promise.resolve({
        ...
        resolveAny: vi.fn(() => Promise.resolve([{ type: 'A', address: '192.168.1.1', ttl: 300 }])),
        resolveTlsa: vi.fn(() => Promise.resolve([{ usage: 3, selector: 1, matchingType: 1, certificate: 'abc123' }])),
        resolveCaa: vi.fn(() => Promise.resolve([{ critical: 0, issue: 'letsencrypt.org' }])),
        resolveNaptr: vi.fn(() => Promise.resolve([{ flags: 'u', order: 100, preference: 10, regexp: '', replacement: '', service: 'sip+d2u' }])),
    };
    return mockModule;
});
