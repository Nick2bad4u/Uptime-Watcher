...
describe("Retry Utilities", () => {
    ...
    describe("withRetry", () => {
        ...
        describe("Configuration options", () => {
            ...
            it("should handle empty options object", async () => {
                const operation = vi.fn().mockResolvedValue("empty options success");
        ...
        describe("Edge cases and error handling", () => {
            ...
            it("should handle maxRetries of 1", async () => {
                const operation = vi.fn().mockRejectedValue(new Error("single attempt"));
            ...
            it("should handle very high maxRetries", async () => {
                ...
                const resultPromise = withRetry(operation, { maxRetries: 1000 });
    ...
    describe("withDbRetry", () => {
        describe("Function behavior", () => {
            it("should use optimized database settings", async () => {
                ...
                const result = await withDbRetry(operation, "test db operation");
            ...
            it("should use default maxRetries of 5", async () => {
                ...
                const resultPromise = withDbRetry(operation, "test db operation");
            ...
            it("should accept custom maxRetries parameter", async () => {
                ...
                const resultPromise = withDbRetry(operation, "test db operation", 3);
            ...
            it("should pass operation name to logging", async () => {
                const operation = vi.fn().mockRejectedValue(new Error("db connection error"));
                
                const resultPromise = withDbRetry(operation, "database initialization");
                
                await vi.runAllTimersAsync();
                
                await expect(resultPromise).rejects.toThrow("db connection error");
            ...
            it("should work with different database operation return types", async () => {
                const insertOp = vi.fn().mockResolvedValue({ insertId: 1, changes: 1 });
                const selectOp = vi.fn().mockResolvedValue([{ id: 1, name: "test" }]);
                const countOp = vi.fn().mockResolvedValue(42);
                
                const insertResult = await withDbRetry(insertOp, "insert operation");
                const selectResult = await withDbRetry(selectOp, "select operation");
                const countResult = await withDbRetry(countOp, "count operation");
        ...
        describe("Integration with withRetry", () => {
            ...
            it("should preserve all withRetry functionality", async () => {
                ...
                const resultPromise = withDbRetry(operation, "functionality test");
        ...
        describe("Real-world database scenarios", () => {
            ...
            it("should handle transaction deadlocks", async () => {
                ...
                const resultPromise = withDbRetry(operation, "transaction retry");
    ...
    describe("Performance and memory considerations", () => {
        ...
        it("should handle rapid-fire operations efficiently", async () => {
            ...
            expect(results).toEqual(Array.from({ length: 10 }, (_, i) => `result-${i}`));
