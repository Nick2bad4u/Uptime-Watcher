/**
 * @fileoverview Comprehensive coverage tests for SiteWriterService.ts
 * This file addresses 25.98% coverage by testing all public and private methods
 */
...
describe('SiteWriterService Coverage Tests', () => {
  ...
  describe('createSite', () => {
    beforeEach(() => {
      (mockDatabaseService.executeTransaction as MockedFunction<any>).mockImplementation(
        async (callback) => {
    ...
    it('should create a new site successfully', async () => {
      ...
      expect(mockSiteRepository.upsertInternal).toHaveBeenCalledWith(mockDb, siteData);
      expect(mockMonitorRepository.deleteBySiteIdentifierInternal).toHaveBeenCalledWith(
        mockDb,
        siteData.identifier
      );
    ...
    it('should handle database transaction errors', async () => {
      ...
      (mockDatabaseService.executeTransaction as MockedFunction<any>).mockRejectedValue(error);

      await expect(siteWriterService.createSite(mockSite)).rejects.toThrow('Transaction failed');
    });
  });
  ...
  describe('deleteSite', () => {
    ...
    it('should delete site successfully when found in cache', async () => {
      (mockSitesCache.delete as MockedFunction<any>).mockReturnValue(true);

      const result = await siteWriterService.deleteSite(mockSitesCache, 'test-site');
      ...
      expect(mockSiteRepository.deleteInternal).toHaveBeenCalledWith(mockDb, 'test-site');
    ...
    it('should handle site not found in cache but still cleanup database', async () => {
      (mockSitesCache.delete as MockedFunction<any>).mockReturnValue(false);

      const result = await siteWriterService.deleteSite(mockSitesCache, 'nonexistent-site');
    ...
    it('should handle database errors during deletion', async () => {
      ...
      await expect(siteWriterService.deleteSite(mockSitesCache, 'test-site')).rejects.toThrow(
  ...
  describe('updateSite', () => {
    beforeEach(() => {
      (mockSitesCache.get as MockedFunction<any>).mockReturnValue(mockSite);
      (mockDatabaseService.executeTransaction as MockedFunction<any>).mockImplementation(
        async (callback) => {
          await callback(mockDb);
        }
    ...
    it('should update site successfully', async () => {
      ...
      const result = await siteWriterService.updateSite(mockSitesCache, 'test-site', updates);
    ...
    it('should create new monitors when updating', async () => {
      ...
      await siteWriterService.updateSite(mockSitesCache, 'test-site', updates);
    ...
    it('should remove obsolete monitors', async () => {
      ...
      expect(mockMonitorRepository.deleteInternal).toHaveBeenCalledWith(mockDb, 'monitor-1');
    });

    it('should throw SiteNotFoundError when site not in cache', async () => {
      (mockSitesCache.get as MockedFunction<any>).mockReturnValue(undefined);
    ...
    it('should throw SiteNotFoundError when identifier is empty', async () => {
      await expect(siteWriterService.updateSite(mockSitesCache, '', {})).rejects.toThrow(
        SiteNotFoundError
      );
    });
  ...
  describe('handleMonitorIntervalChanges', () => {
    ...
    it('should handle interval changes correctly', async () => {
      ...
      expect(mockMonitoringConfig.stopMonitoring).toHaveBeenCalledWith('test-site', 'monitor-1');
      expect(mockMonitoringConfig.startMonitoring).toHaveBeenCalledWith('test-site', 'monitor-1');
    ...
    it('should handle monitoring config errors gracefully', async () => {
      ...
      (mockMonitoringConfig.stopMonitoring as MockedFunction<any>).mockRejectedValue(error);

      const originalSite = {
        ...mockSite,
        monitors: [{ ...mockSite.monitors[0], checkInterval: 30_000, monitoring: true }],
      };
      const newMonitors = [{ ...originalSite.monitors[0], checkInterval: 60_000 }];
  ...
  describe('detectNewMonitors', () => {
    it('should detect new monitors with IDs', () => {
      const originalMonitors = [{ ...mockSite.monitors[0], id: 'monitor-1' }];
      const updatedMonitors = [
        ...originalMonitors,
        { ...mockSite.monitors[0], id: 'monitor-2' },
      ];

      const newIds = siteWriterService.detectNewMonitors(originalMonitors, updatedMonitors);
    ...
    it('should handle mixed scenarios', () => {
      const originalMonitors = [
        { ...mockSite.monitors[0], id: 'monitor-1', url: 'https://example.com' },
      ];
      const updatedMonitors = [
        ...originalMonitors,
        { ...mockSite.monitors[0], id: 'monitor-2', url: 'https://example.com' }, // New with ID
  ...
  describe('Private Methods Coverage', () => {
    ...
    it('should cover createMonitorSignature via detectNewMonitors', () => {
      ...
      const newIds = siteWriterService.detectNewMonitors([monitor1], [monitor1, monitor2]);
    ...
    it('should cover orphaned monitor handling in handleExistingMonitor', async () => {
      ...
      const orphanedMonitor = { ...mockSite.monitors[0], id: 'orphaned-id' };
    ...
    it('should cover createUpdatedSite cache update', async () => {
      ...
      expect(mockSitesCache.set).toHaveBeenCalledWith('test-site', expect.objectContaining({
        ...mockSite,
        name: 'Updated Name',
      }));
    });
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle monitors with all optional fields defined', () => {
      ...
      const signature = (siteWriterService as any).createMonitorSignature(monitor);
    ...
    it('should handle complex monitor update scenarios', async () => {
      ...
      const updates = {
        monitors: [
          { ...mockSite.monitors[0], id: 'monitor-1', checkInterval: 60_000 }, // Update existing
          // Remove monitor-2 (not included)
          { ...mockSite.monitors[0], id: '', url: 'https://newsite.com' }, // Add new
        ],
      };
      ...
      expect(mockMonitorRepository.deleteInternal).toHaveBeenCalledWith(mockDb, 'monitor-2');
      expect(mockMonitorRepository.createInternal).toHaveBeenCalledWith(
        mockDb,
        'test-site',
