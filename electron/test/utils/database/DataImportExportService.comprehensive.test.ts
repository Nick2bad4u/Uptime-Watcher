...
import type { DataImportExportConfig, ImportSite } from "../../../utils/database/DataImportExportService";
...
describe("DataImportExportService - Comprehensive Coverage", () => {
    ...
    beforeEach(() => {
        ...
        mockDatabaseService = {
            executeTransaction: vi.fn().mockImplementation(async (callback: Function) => {
    ...
    describe("exportAllData", () => {
        it("should export all data successfully with complete metadata", async () => {
            const { safeJsonStringifyWithFallback } = await import("../../../../shared/utils/jsonSafety");
            ...
            (safeJsonStringifyWithFallback as MockedFunction<any>).mockReturnValue('{"exported": true}');
        ...
        it("should handle export errors and emit database error event", async () => {
            ...
            await expect(service.exportAllData()).rejects.toThrow(SiteLoadingError);
            await expect(service.exportAllData()).rejects.toThrow("Failed to export data: Database export failed");
    ...
    describe("importDataFromJson", () => {
        it("should successfully parse and validate import data", async () => {
            ...
            expect(safeJsonParse).toHaveBeenCalledWith(mockJsonData, expect.any(Function));
        ...
        it("should handle parsing failure with invalid JSON", async () => {
            ...
            await expect(service.importDataFromJson(invalidJsonData)).rejects.toThrow(SiteLoadingError);
            await expect(service.importDataFromJson(invalidJsonData)).rejects.toThrow("Failed to parse import data");
            ...
            expect(mockEventEmitter.emitTyped).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    ...
                    details: expect.stringContaining("Invalid import data format")
                })
            );
        });
        ...
        it("should handle parsing success but no data returned", async () => {
            ...
            await expect(service.importDataFromJson("{}")).rejects.toThrow(SiteLoadingError);
        ...
        it("should handle non-Error objects in parsing failure", async () => {
            ...
            await expect(service.importDataFromJson("invalid")).rejects.toThrow(SiteLoadingError);
    ...
    describe("persistImportedData", () => {
        it("should successfully persist sites and settings in transaction", async () => {
            const { withDatabaseOperation } = await import("../../../utils/operationalHooks");
            ...
            (withDatabaseOperation as MockedFunction<any>).mockImplementation(async (operation: any) => {
            ...
            expect(mockDatabaseService.executeTransaction).toHaveBeenCalledWith(expect.any(Function));
            expect(mockRepositories.site.deleteAllInternal).toHaveBeenCalledWith(mockDatabase);
            expect(mockRepositories.settings.deleteAllInternal).toHaveBeenCalledWith(mockDatabase);
            expect(mockRepositories.monitor.deleteAllInternal).toHaveBeenCalledWith(mockDatabase);
            expect(mockRepositories.history.deleteAllInternal).toHaveBeenCalledWith(mockDatabase);

            // Verify bulk inserts
            expect(mockRepositories.site.bulkInsertInternal).toHaveBeenCalledWith(
                mockDatabase,
                [
            ...
            expect(mockRepositories.settings.bulkInsertInternal).toHaveBeenCalledWith(
                mockDatabase,
                mockSettings
            );
        ...
        it("should handle empty sites and settings arrays", async () => {
            ...
            expect(mockRepositories.site.bulkInsertInternal).toHaveBeenCalledWith(mockDatabase, []);
            expect(mockRepositories.settings.bulkInsertInternal).toHaveBeenCalledWith(mockDatabase, {});
    ...
    describe("importMonitorsWithHistory - Integration Testing", () => {
        it("should import monitors with history for sites with monitor data", async () => {
            ...
            const mockMonitors = [
                {
                    ...
                    checkInterval: 5000,
                    history: [
                        { status: "up" as const, timestamp: Date.now(), responseTime: 100 }
                    ]
                }
            ];
            ...
            mockRepositories.monitor.bulkCreate.mockResolvedValue(mockCreatedMonitors);
            ...
            expect(mockRepositories.history.addEntryInternal).toHaveBeenCalledWith(
                mockDatabase,
        ...
        it("should handle sites without monitors gracefully", async () => {
            ...
            expect(mockRepositories.history.addEntryInternal).not.toHaveBeenCalled();
        });
        ...
        it("should handle monitor creation errors and continue with other sites", async () => {
            ...
            const mockSites: ImportSite[] = [
                {
                    identifier: "site1",
                    monitors: [{ id: "mon1", type: "http" as const, url: "https://example.com" } as any]
                },
                {
                    identifier: "site2", 
                    monitors: [{ id: "mon2", type: "ping" as const, url: "https://test.com" } as any]
                }
            ];
        ...
        it("should match monitors correctly for history import", async () => {
            ...
            const originalMonitors = [
                {
                    ...
                    port: undefined,
                    history: [
                        { status: "up" as const, timestamp: 1000, responseTime: 50 },
                        { status: "down" as const, timestamp: 2000, responseTime: 0 }
                    ]
                }
            ];
            ...
            mockRepositories.monitor.bulkCreate.mockResolvedValue(createdMonitors);
            ...
            expect(mockRepositories.history.addEntryInternal).toHaveBeenCalledTimes(2);
            expect(mockRepositories.history.addEntryInternal).toHaveBeenNthCalledWith(
                1,
                mockDatabase,
                    ...
                    monitors: [
                        {
                            type: "http" as const,
                            url: "https://example.com",
                            history: [{ status: "up" as const, timestamp: 1000, responseTime: 50 }]
                        }
                    ] as any
                }
            ];
    ...
    describe("Type Guard Function - isImportData", () => {
        it("should be tested through importDataFromJson integration", async () => {
            ...
            (safeJsonParse as MockedFunction<any>).mockImplementation((jsonData, typeGuard) => {
                // Test the type guard with valid data
                const validData = { sites: [{ identifier: "test" }], settings: {} };
    ...
    describe("Error Handling Edge Cases", () => {
        it("should handle withDatabaseOperation errors during persist", async () => {
            ...
            (withDatabaseOperation as MockedFunction<any>).mockRejectedValue(operationError);
        ...
        it("should handle transaction callback errors", async () => {
            ...
            await expect(service.persistImportedData([], {})).rejects.toThrow(transactionError);
        });
        ...
        it("should handle repository method failures during transaction", async () => {
            ...
            await expect(service.persistImportedData([], {})).rejects.toThrow(deleteError);
        });
    });
    ...
    describe("Integration and Edge Cases", () => {
        it("should handle complex import data with multiple sites and monitors", async () => {
            ...
            mockRepositories.monitor.bulkCreate.mockResolvedValue([
                { id: "http-monitor-1", type: "http", url: "https://api.example.com" },
                { id: "port-monitor-1", type: "port", url: "database.example.com", port: 5432 }
            ]);

            (withDatabaseOperation as MockedFunction<any>).mockImplementation(async (operation) => {
                return await operation();
            });
            ...
            expect(mockRepositories.history.addEntryInternal).toHaveBeenCalledTimes(5); // Only HTTP monitor has history
            expect(mockLogger.info).toHaveBeenCalledWith(
        ...
        it("should maintain proper error context throughout operations", async () => {
            ...
            originalError.stack = "Error: Original database error\n    at someFunction";
            ...
            try {
                ...
                expect(error.message).toContain("Failed to export data: Original database error");
