...
export function assertSqliteDatabaseIntegrity(args: {
    ...
    try {
        ...
        if (
            messages.length === 1 &&
            arrayFirst(messages)?.toLowerCase() === "ok"
        ) {
            return;
        }
        ...
        const details =
            messages.length > 0 ? arrayJoin(messages, "; ") : "unknown";
...
export async function createDatabaseBackup(
    ...
    try {
        ...
        if (buffer.byteLength > DEFAULT_MAX_BACKUP_SIZE_BYTES) {
            throw new Error(
                `Database backup exceeds maximum allowed size (${buffer.byteLength} > ${DEFAULT_MAX_BACKUP_SIZE_BYTES} bytes)`
            );
        }
