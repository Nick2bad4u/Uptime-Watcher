 ...
 *
 * @remarks
 * These interfaces deliberately avoid extending `UnknownRecord` so callers are
 * encouraged to treat them as narrow, well-defined result contracts rather than
 * generic bags of data. The low-level validation layer still widens rows to
 * `UnknownRecord` internally via {@link EnforcedRow} where necessary.
 */
