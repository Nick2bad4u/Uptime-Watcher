...
export class MonitorRepository {
                ...
                await this.databaseService.executeTransaction((db) => {
                    for (const monitor of monitors) {
                        ...
                        const quotedColumns = columns.map((column) =>
                            this.escapeSqlIdentifier(column)
                        );

                        // eslint-disable-next-line sql-template/no-unsafe-query -- Safe: columns/placeholders are generated by app code
                        const insertSql = `INSERT INTO monitors (${quotedColumns.join(", ")}) VALUES (${placeholders}) RETURNING id`;
        ...
        for (const [key, value] of Object.entries(row)) {
            ...
            if (shouldProcess) {
                ...
                if (fieldValue !== undefined) {
                    updateFields.push(`${this.escapeSqlIdentifier(key)} = ?`);
     ...
     *
     * @remarks
     * This repository constructs some SQL fragments dynamically to support
     * dynamic monitor schemas. Values are always bound as parameters, but
     * identifiers must also be validated to prevent malformed SQL or injection
     * via unexpected column names.
     */
    private escapeSqlIdentifier(identifier: string): string {
        // SQLite identifiers are safest when restricted to alphanumerics and
        // underscores. Dynamic schema generation should already normalize
        // column names, so anything else is suspicious.
