 ...
 *
 * @remarks
 * This script reads the Istanbul coverage-final.json file from the coverage directory,
 * analyzes statement, function, and branch coverage for each file, and prints a summary
 * of files with the lowest coverage. It is intended to help developers identify files
 ...
 * @returns Console output listing files with low function and branch coverage, and a summary.
 */
...
const coveragePath = path.join(dirname, '..', 'coverage', 'coverage-final.json');
// eslint-disable-next-line unicorn/prefer-json-parse-buffer
const coverageData = JSON.parse(fs.readFileSync(coveragePath, { encoding: 'utf8' }));
...
if (noColor) {
  Object.keys(colors).forEach(k => { colors[k] = s => s; });
}
...
for (const [filePath, data] of Object.entries(coverageData)) {
   ...
   * @param {number | number[]} branchArray - The branch coverage data, either a number or an array of numbers.
   ...
   * @property {number} statements.percentage - Percentage of covered statements.
   ...
   * @property {number} functions.percentage - Percentage of covered functions.
...
function computeColWidths(displayed) {
  ...
  const minFileCol = 30;
  const fileCol = Math.max(minFileCol, ...displayed.map(f => f.file.length));
...
function printCoverageSection(header, files) {
  ...
  try {
    ...
    displayed.forEach(f => {
      ...
      table.push([ellipsize(f.file, DEFAULTS.truncateFilePath), fmt(f.functions), fmt(f.branches), fmt(f.statements), fmt(f.lines)]);
    });
  ...
  const fileCol = Math.max(minFileCol, ...displayed.map(f => ellipsize(f.file, DEFAULTS.truncateFilePath).length));
  ...
  displayed.forEach(f => {
  const rFile = padRight(ellipsize(f.file, DEFAULTS.truncateFilePath), fileCol);
    const cell = (cov) => {
    ...
    console.log(`${colors.cyan(rFile)}  ${cell(f.functions)}  ${cell(f.branches)}  ${cell(f.statements)}  ${cell(f.lines)}`);
  });
  console.log();
}
...
if (outputFormat === 'csv') {
  ...
  return `"${s.replaceAll('"', '""')}"`;
    ...
    const row = [
      csvEscape(f.file),
      f.functions.total, f.functions.covered, f.functions.percentage.toFixed(2),
      f.branches.total, f.branches.covered, f.branches.percentage.toFixed(2),
      f.statements.total, f.statements.covered, f.statements.percentage.toFixed(2),
      f.lines.total, f.lines.covered, f.lines.percentage.toFixed(2)
    ].join(',');
    console.log(row);
  }
  process.exit(0);
...
fileAnalysis.sort((a, b) => (a.lines?.percentage ?? 100) - (b.lines?.percentage ?? 100));
