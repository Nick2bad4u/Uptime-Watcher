...
console.log(`  ${hasWarmup ? '✅' : '❌'} Warmup configuration: ${hasWarmup ? 'FOUND' : 'MISSING'}`);
console.log(`  ${hasClientFiles ? '✅' : '❌'} Client files array: ${hasClientFiles ? 'FOUND' : 'MISSING'}`);
...
if (warmupFiles.length > 0) {
    ...
    // Group by category
    const categories = {
        'Core': warmupFiles.filter(f => f.includes('App.tsx') || f.includes('main.tsx')),
        'Stores': warmupFiles.filter(f => f.includes('/stores/')),
        'Theme': warmupFiles.filter(f => f.includes('/theme/')),
        'Charts': warmupFiles.filter(f => f.includes('chart') || f.includes('Chart')),
        'Components': warmupFiles.filter(f => f.includes('/components/') && !f.includes('chart')),
        'Utils': warmupFiles.filter(f => f.includes('/utils/') || f.includes('/services/')),
