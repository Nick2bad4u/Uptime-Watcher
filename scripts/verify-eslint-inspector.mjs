...
/**
 * @fileoverview Verification script for ESLint Config Inspector deployment
 * @description Verifies that all ESLint inspector components are properly deployed
...
async function verifyConfigFile(filePath) {
    try {
        ...
        if (filePath.includes('docusaurus.config.ts')) {
            const hasEslintLink = content.includes('ESLint Inspector') &&
                content.includes('/Uptime-Watcher/eslint-inspector/');
            return {
                valid: hasEslintLink,
                content: hasEslintLink ? 'ESLint Inspector navbar link found' : 'ESLint Inspector navbar link missing'
            };
        }
        ...
        if (filePath.includes('package.json')) {
            ...
            return {
                ...
                content: hasEslintScript ? 'ESLint inspector build script found' : 'ESLint inspector build script missing'
            };
        }
...
async function verifyESLintInspectorDeployment() {
    ...
    if (staticCheck.exists) {
        ...
        if (staticCheck.missing.length === 0) {
            ...
            console.log(`   ❌ Missing files: ${staticCheck.missing.join(', ')}`);
    ...
    if (buildCheck.exists) {
        ...
        if (buildCheck.missing.length === 0) {
            ...
            console.log(`   ❌ Missing files: ${buildCheck.missing.join(', ')}`);
            allChecksPassed = false;
    ...
    for (const configFile of configFiles) {
        ...
        if (configCheck.valid) {
            ...
            console.log(`   ❌ ${fileName}: ${configCheck.error || configCheck.content}`);
            allChecksPassed = false;
    ...
    console.log('\n🔨 Build Script Check:');
    try {
        const buildScriptPath = path.join(rootDir, 'scripts', 'build-eslint-inspector.mjs');
    ...
    if (allChecksPassed) {
        ...
        console.log('      https://nick2bad4u.github.io/Uptime-Watcher/eslint-inspector/');
        console.log('   3. Access via navbar "ESLint Config" link in documentation');
    } else {
