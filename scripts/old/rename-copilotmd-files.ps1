...
if ($WhatIf) {
    Write-Host "`n💡 This was a preview. Run without -WhatIf to execute the renames." -ForegroundColor Yellow
    exit 0
}
...
if ($Host.UI.RawUI.KeyAvailable -or $env:CI) {
    ...
    Write-Host "`n❓ Proceed with renaming these files? (y/N): " -ForegroundColor Yellow -NoNewline
    $Response = Read-Host
...
Write-Host "`n🚀 Starting rename operations..." -ForegroundColor Green
...
foreach ($file in $CopilotMdFiles) {
    try {
        ...
        if (Test-Path $NewPath) {
            Write-Warning "⚠️  Target file already exists: $NewPath"
...
if ($RemainingFiles.Count -gt 0) {
    Write-Host "`n⚠️  Remaining .copilotmd files:" -ForegroundColor Yellow
    foreach ($file in $RemainingFiles) {
        $RelativePath = $file.FullName.Replace($DocsPath, "docs")
        Write-Host "    $RelativePath" -ForegroundColor Yellow
    }
} else {
    Write-Host "`n🎊 No .copilotmd files remaining in docs folder!" -ForegroundColor Green
}
