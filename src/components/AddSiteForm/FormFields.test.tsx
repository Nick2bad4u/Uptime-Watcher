...
vi.mock("../../theme/components", () => ({
    ThemedInput: ({ children, ...props }: React.ComponentProps<"input">) => (
        <input {...props}>{children}</input>
    ),
    ThemedSelect: ({ children, ...props }: React.ComponentProps<"select">) => (
        <select {...props}>{children}</select>
    ),
    ThemedText: ({ children, ...props }: React.ComponentProps<"span">) => (
...
describe("FormFields", () => {
    ...
    describe("RadioGroup", () => {
        ...
        it("should handle multiple radio groups independently", () => {
            ...
            const group1Radios = screen.getAllByRole("radio").filter(radio => 
                radio.getAttribute("name") === "group1"
            );
            const group2Radios = screen.getAllByRole("radio").filter(radio => 
                radio.getAttribute("name") === "group2"
            );
