...
describe('Settings - Branch Coverage Tests', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(useThemeClasses).mockReturnValue({
      getBackgroundClass: vi.fn().mockReturnValue({ backgroundColor: '#ffffff' }),
      ...
      getSurfaceClass: vi.fn().mockReturnValue({ backgroundColor: '#f5f5f5' })
    } as any);
  ...
  describe('Sync Success Display Branches', () => {
    ...
    it('should not render sync success when error is present', () => {
      ...
      expect(screen.queryByText('✅ Data synced from database.')).not.toBeInTheDocument();
    });
  });
  ...
  describe('Loading State Branches', () => {
    it('should disable inputs when loading is true', () => {
      ...
      expect(screen.getByLabelText('Maximum number of history records to keep per site')).toBeDisabled();
      expect(screen.getByLabelText('Enable desktop notifications')).toBeDisabled();
    ...
    it('should enable inputs when loading is false', () => {
      ...
      expect(screen.getByLabelText('Maximum number of history records to keep per site')).not.toBeDisabled();
      expect(screen.getByLabelText('Enable desktop notifications')).not.toBeDisabled();
  ...
  describe('Async Error Handling Branches', () => {
    ...
    it('should handle backup download errors', async () => {
      ...
      await waitFor(() => {
        expect(mockErrorStore.setError).toHaveBeenCalledWith('Failed to download SQLite backup: Backup failed');
      });
    });
    ...
    it('should handle history limit update errors', async () => {
      ...
      const mockUpdateHistoryLimit = vi.fn().mockRejectedValue(historyError);
  ...
  describe('Error Object Property Access Branches', () => {
    it('should handle error objects with message property', async () => {
      ...
      vi.mocked(useSitesStore).mockReturnValue({
        ...mockSitesStore,
        fullSyncFromBackend: vi.fn().mockRejectedValue(errorWithMessage),
      });
      ...
      await waitFor(() => {
        expect(mockErrorStore.setError).toHaveBeenCalledWith('Failed to sync data: Detailed error message');
      });
    });
    ...
    it('should handle error objects without message property', async () => {
      ...
      vi.mocked(useSitesStore).mockReturnValue({
        ...mockSitesStore,
        downloadSQLiteBackup: vi.fn().mockRejectedValue(errorWithoutMessage),
      });
    ...
    it('should handle primitive error values', async () => {
      ...
      await waitFor(() => {
        expect(mockErrorStore.setError).toHaveBeenCalledWith('Failed to sync data: Simple string error');
      });
    });
  });
  ...
  describe('Event Handler Branches', () => {
    it('should handle all checkbox change events', () => {
      ...
      expect(mockSettingsStore.updateSettings).toHaveBeenCalledWith({ minimizeToTray: true });
    ...
    it('should handle history limit select change', () => {
      ...
      expect(mockSettingsStore.updateHistoryLimitValue).toHaveBeenCalledWith(200);
    });
