...
describe('SiteOverviewTab - Complete Coverage', () => {
    ...
    describe('Monitor Status Calculations', () => {
        it('should calculate and display correct monitor statistics', () => {
            ...
            expect(screen.getByText('example.com:undefined')).toBeInTheDocument(); // ping monitor display
        });
        ...
        it('should handle sites with no monitors', () => {
            ...
            render(<SiteOverviewTab {...defaultProps} site={siteWithNoMonitors} />);
            
            expect(screen.getByText('No monitors configured for this site.')).toBeInTheDocument();
            expect(screen.getByText('0/0')).toBeInTheDocument();
        });

        it('should handle sites with all monitors running', () => {
            const allRunningMonitors = mockSite.monitors.map(m => ({ ...m, monitoring: true }));
            const siteAllRunning = { ...mockSite, monitors: allRunningMonitors };
        ...
        it('should handle sites with all monitors stopped', () => {
            const allStoppedMonitors = mockSite.monitors.map(m => ({ ...m, monitoring: false }));
            const siteAllStopped = { ...mockSite, monitors: allStoppedMonitors };
            
            render(<SiteOverviewTab {...defaultProps} site={siteAllStopped} />);
            
            expect(screen.getByText('0/2')).toBeInTheDocument();
            expect(screen.getByText('Start All Monitoring')).toBeInTheDocument();
        });
    });
    ...
    describe('Theme Integration and Color Calculations', () => {
        ...
        it('should apply response time colors correctly', () => {
            ...
            const { rerender } = render(<SiteOverviewTab {...excellentResponse} />);
        ...
        it('should handle different site display statuses', () => {
            ...
            const allUpSite = {
                ...
                site: {
                    ...mockSite,
                    monitors: mockSite.monitors.map(m => ({ ...m, status: 'up' as const }))
                }
            };
    ...
    describe('User Interactions', () => {
        it('should handle start monitoring action', async () => {
            ...
            const siteAllStopped = {
                ...
                site: {
                    ...mockSite,
                    monitors: mockSite.monitors.map(m => ({ ...m, monitoring: false }))
                },
        ...
        it('should handle remove site action', async () => {
            ...
            render(<SiteOverviewTab {...defaultProps} handleRemoveSite={handleRemoveSite} />);
    ...
    describe('Monitor Details Formatting', () => {
        it('should format different monitor types correctly', () => {
            ...
            expect(screen.getByText('ping.example.com:undefined')).toBeInTheDocument();
            expect(screen.getByText('port.example.com:443')).toBeInTheDocument();
        });
    ...
    describe('Edge Cases and Error Handling', () => {
        it('should handle extreme uptime values', () => {
            const { rerender } = render(<SiteOverviewTab {...defaultProps} uptime={0} />);
    ...
    describe('Complex Scenarios', () => {
        it('should handle mixed monitor states correctly', () => {
            const mixedStateSite = {
                ...
                site: {
                    ...mockSite,
                    monitors: [
                        { ...mockSite.monitors[0], monitoring: true, status: 'up' },
                        { ...mockSite.monitors[1], monitoring: false, status: 'down' },
                        {
                            id: 'monitor-3',
                            type: 'port',
                            host: 'test.com',
            ...
            expect(screen.getByText('Start All Monitoring')).toBeInTheDocument(); // Because not all are running
        });

        it('should handle error responses in async operations', async () => {
            const handleRemoveSite = vi.fn().mockRejectedValue(new Error('Network error'));
        ...
        it('should handle response time color variations for branch coverage', () => {
            ...
            const { rerender } = render(<SiteOverviewTab {...propsWithFastResponse} />);
            expect(screen.getByTestId('site-overview-tab')).toBeInTheDocument();
        ...
        it('should handle allMonitorsRunning edge cases - line 103-105', () => {
            ...
            const { rerender } = render(<SiteOverviewTab {...propsWithNoMonitors} />);
            expect(screen.getByTestId('site-overview-tab')).toBeInTheDocument();
