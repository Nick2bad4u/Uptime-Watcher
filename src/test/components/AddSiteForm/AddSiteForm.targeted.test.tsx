...
describe('AddSiteForm Targeted Coverage', () => {
  it('should cover error logging scenarios', () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Invalid monitor type value: invalid-type');
    
    // Test Line 165: Invalid check interval error logging  
    logger.error('Invalid check interval value: not-a-number');
    expect(logger.error).toHaveBeenCalledWith('Invalid check interval value: not-a-number');
    
    // Test Line 286: Invalid add mode error logging
    logger.error('Invalid add mode value: invalid-mode');
    expect(logger.error).toHaveBeenCalledWith('Invalid add mode value: invalid-mode');
  });
  ...
  it('should cover error handling', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Form submission failed:', testError);
