...
vi.mock('../../../hooks/useMonitorTypes', () => ({
  useMonitorTypes: vi.fn(() => ({
    ...
    options: mockMonitorTypes.map(type => ({ label: type.name, value: type.id })),
    isLoading: false,
    error: null,
...
describe('AddSiteForm Component - Enhanced Coverage', () => {
  ...
  describe('Basic Rendering and Structure', () => {
    ...
    it('should render all required form elements', () => {
      ...
      expect(screen.getByTestId('monitor-type-field')).toBeInTheDocument();
    });
  ...
  describe('Form Field Components', () => {
    it('should render site name field with proper attributes', () => {
      ...
      expect(nameField.querySelector('input')).toHaveAttribute('type', 'text');
    });
    ...
    it('should render site URL field with proper attributes', () => {
      ...
      expect(urlField.querySelector('input')).toHaveAttribute('type', 'url');
    });
    ...
    it('should render interval field with numeric input', () => {
      ...
      expect(intervalField.querySelector('input')).toHaveAttribute('type', 'number');
    });
    ...
    it('should render timeout field with numeric input', () => {
      ...
      expect(timeoutField.querySelector('input')).toHaveAttribute('type', 'number');
    });
    ...
    it('should render retries field with numeric input', () => {
      ...
      expect(retriesField.querySelector('input')).toHaveAttribute('type', 'number');
    });
  });
  ...
  describe('User Interactions and Form Changes', () => {
    it('should handle site name input changes', async () => {
      ...
      renderAddSiteForm();
      const nameInput = screen.getByTestId('site-name-field').querySelector('input');
    ...
    it('should handle site URL input changes', async () => {
      ...
      renderAddSiteForm();
      const urlInput = screen.getByTestId('site-url-field').querySelector('input');
    ...
    it('should handle monitor type selection changes', async () => {
      ...
      renderAddSiteForm();
      const monitorSelect = screen.getByTestId('monitor-type-field').querySelector('select');
    ...
    it('should handle description textarea changes', async () => {
      ...
      renderAddSiteForm();
      const descriptionTextarea = screen.getByTestId('description-field').querySelector('textarea');
    ...
    it('should handle interval value changes', async () => {
      ...
      renderAddSiteForm();
      const intervalInput = screen.getByTestId('interval-field').querySelector('input');
    ...
    it('should handle timeout value changes', async () => {
      ...
      renderAddSiteForm();
      const timeoutInput = screen.getByTestId('timeout-field').querySelector('input');
    ...
    it('should handle retries value changes', async () => {
      ...
      renderAddSiteForm();
      const retriesInput = screen.getByTestId('retries-field').querySelector('input');
  ...
  describe('Form Submission Handling', () => {
    it('should handle form submission with onSuccess callback', async () => {
      ...
      expect(handleSubmit).toHaveBeenCalledWith(expect.any(Object), onSuccess);
    });
    ...
    it('should handle form submission without onSuccess callback', async () => {
      ...
      expect(handleSubmit).toHaveBeenCalledWith(expect.any(Object), undefined);
    });
  ...
  describe('Error Handling and Display', () => {
    it('should display field validation errors', () => {
      ...
      expect(screen.getByText('Monitor type is required')).toBeInTheDocument();
    });
    ...
    it('should handle error store integration', () => {
      const mockError = { message: 'Form submission failed', code: 'SUBMIT_ERROR' };
  ...
  describe('Validation Logic', () => {
    ...
    it('should handle complex validation scenarios', () => {
      ...
      renderAddSiteForm();
      expect(screen.getByText('Site name must be at least 3 characters')).toBeInTheDocument();
      expect(screen.getByText('Please enter a valid URL')).toBeInTheDocument();
      expect(screen.getByText('Timeout must be positive')).toBeInTheDocument();
      expect(screen.getByText('Retries must be between 0 and 5')).toBeInTheDocument();
    });
  });
  ...
  describe('Monitor Types Integration', () => {
    ...
    it('should handle monitor types with special configurations', () => {
      const specialMonitorTypes = [
        { id: 'custom', name: 'Custom Monitor', fields: ['custom_field1', 'custom_field2'] },
        { id: 'advanced', name: 'Advanced Monitor', fields: [] }
      ];

      mockUseMonitorTypes.mockReturnValue({
        monitorTypes: specialMonitorTypes,
        options: specialMonitorTypes.map(type => ({ label: type.name, value: type.id })),
        isLoading: false,
        error: null,
        refreshMonitorTypes: vi.fn()
      });
  ...
  describe('Dynamic Help Text Integration', () => {
    it('should display help text for fields', () => {
      ...
      renderAddSiteForm();
      expect(screen.getByText('Enter the URL of the site you want to monitor')).toBeInTheDocument();
    });
  ...
  describe('Edge Cases and Corner Scenarios', () => {
    ...
    it('should handle extreme numeric values', () => {
      ...
      renderAddSiteForm();
      expect(screen.getByDisplayValue(String(Number.MAX_SAFE_INTEGER))).toBeInTheDocument();
  ...
  describe('Performance and Re-rendering', () => {
    it('should handle frequent re-renders efficiently', () => {
      ...
      for (let i = 0; i < 10; i++) {
        rerender(<AddSiteForm {...defaultProps} className={`test-${i}`} />);
      }
    ...
    it('should maintain form state during re-renders', () => {
      ...
      expect(screen.getByDisplayValue('Persistent Site')).toBeInTheDocument();
    ...
    it('should handle large monitor types lists efficiently', () => {
      const largeMonitorTypesList = Array.from({ length: 100 }, (_, i) => ({
        id: `monitor${i}`,
        name: `Monitor Type ${i}`,
        fields: [`field${i}`]
      }));
      ...
      mockUseMonitorTypes.mockReturnValue({
        ...
        options: largeMonitorTypesList.map(type => ({ label: type.name, value: type.id })),
        isLoading: false,
        error: null,
        refreshMonitorTypes: vi.fn()
      });
