 ...
 * 
 * This test suite provides complete coverage for the MonitorTypesStore,
 * testing all state management, actions, error handling, and IPC interactions.
 */
...
describe('useMonitorTypesStore', () => {
  ...
  describe('loadMonitorTypes', () => {
    const mockMonitorTypes: MonitorTypeConfig[] = [
      ...
      },
      {
        ...
        version: '1.0.0',
        fields: [
          { name: 'host', type: 'string', required: true, label: 'Host' }
        ]
      }
    ];

    it('should load monitor types successfully', async () => {
      mockElectronAPI.monitorTypes.getMonitorTypes.mockResolvedValue(mockMonitorTypes);
    ...
    it('should handle loading errors', async () => {
      ...
      mockElectronAPI.monitorTypes.getMonitorTypes.mockRejectedValue(new Error(errorMessage));
  ...
  describe('validateMonitorData', () => {
    ...
    it('should validate monitor data successfully', async () => {
      mockElectronAPI.monitorTypes.validateMonitorData.mockResolvedValue(mockValidationResult);
      
      const { result } = renderHook(() => useMonitorTypesStore());
      
      let validationResult: ValidationResult;
      await act(async () => {
        validationResult = await result.current.validateMonitorData('http', { url: 'https://example.com' });
      });
    ...
    it('should handle validation with errors', async () => {
      ...
      mockElectronAPI.monitorTypes.validateMonitorData.mockResolvedValue(errorResult);
    ...
    it('should handle partial validation result with missing properties', async () => {
      ...
      mockElectronAPI.monitorTypes.validateMonitorData.mockResolvedValue(partialResult);
    ...
    it('should handle validation errors', async () => {
      ...
      mockElectronAPI.monitorTypes.validateMonitorData.mockRejectedValue(new Error(errorMessage));
  ...
  describe('formatMonitorDetail', () => {
    it('should format monitor detail successfully', async () => {
      ...
      await act(async () => {
        formatted = await result.current.formatMonitorDetail('http', 'Response time: 150ms');
      });
      
      expect(formatted!).toBe(formattedDetail);
      expect(mockElectronAPI.monitorTypes.formatMonitorDetail).toHaveBeenCalledWith('http', 'Response time: 150ms');
    });

    it('should handle formatting errors with fallback', async () => {
      const originalDetails = 'Response time: 150ms';
      mockElectronAPI.monitorTypes.formatMonitorDetail.mockRejectedValue(new Error('Formatting failed'));
      
      const { result } = renderHook(() => useMonitorTypesStore());
      
      await act(async () => {
        try {
          await result.current.formatMonitorDetail('http', originalDetails);
        } catch (error) {
          // Expected to throw
        }
      });
    ...
    it('should handle null response with fallback', async () => {
      ...
      let formatted: string;
      await act(async () => {
        formatted = await result.current.formatMonitorDetail('http', originalDetails);
      });
  ...
  describe('formatMonitorTitleSuffix', () => {
    ...
    it('should format monitor title suffix successfully', async () => {
      ...
      mockElectronAPI.monitorTypes.formatMonitorTitleSuffix.mockResolvedValue(titleSuffix);
      
      const { result } = renderHook(() => useMonitorTypesStore());
      
      let formatted: string;
      await act(async () => {
        formatted = await result.current.formatMonitorTitleSuffix('http', mockMonitor);
      });
      
      expect(formatted!).toBe(titleSuffix);
      expect(mockElectronAPI.monitorTypes.formatMonitorTitleSuffix).toHaveBeenCalledWith('http', mockMonitor);
    });

    it('should handle title suffix formatting errors', async () => {
      mockElectronAPI.monitorTypes.formatMonitorTitleSuffix.mockRejectedValue(new Error('Title formatting failed'));
      
      const { result } = renderHook(() => useMonitorTypesStore());
      
      await act(async () => {
        try {
          await result.current.formatMonitorTitleSuffix('http', mockMonitor);
        } catch {
          // Expected to throw
        }
      });
    ...
    it('should handle null response with empty string fallback', async () => {
      mockElectronAPI.monitorTypes.formatMonitorTitleSuffix.mockResolvedValue(null);
  ...
  describe('getFieldConfig', () => {
    ...
    it('should return undefined for non-existing type', () => {
      ...
      const config = result.current.getFieldConfig('unknown' as MonitorType);
  ...
  describe('Integration Tests', () => {
    it('should handle complete monitor types workflow', async () => {
      ...
      mockElectronAPI.monitorTypes.getMonitorTypes.mockResolvedValue(mockMonitorTypes);
      mockElectronAPI.monitorTypes.validateMonitorData.mockResolvedValue(mockValidationResult);
      mockElectronAPI.monitorTypes.formatMonitorDetail.mockResolvedValue('Formatted: 150ms');
      ...
      // Get field config
      const fieldConfig = result.current.getFieldConfig('http' as MonitorType);
      ...
      await act(async () => {
        formattedDetail = await result.current.formatMonitorDetail('http', 'Response: 150ms');
      });
    ...
    it('should handle error recovery workflow', async () => {
      ...
      mockElectronAPI.monitorTypes.getMonitorTypes.mockRejectedValue(new Error('Network error'));
  ...
  describe('Edge Cases', () => {
    it('should handle concurrent loadMonitorTypes calls', async () => {
      ...
      mockElectronAPI.monitorTypes.getMonitorTypes.mockImplementation(() => 
        new Promise(resolve => setTimeout(() => resolve(mockMonitorTypes), 100))
      );
    ...
    it('should handle malformed monitor type data', async () => {
      ...
      mockElectronAPI.monitorTypes.getMonitorTypes.mockResolvedValue(malformedData);
