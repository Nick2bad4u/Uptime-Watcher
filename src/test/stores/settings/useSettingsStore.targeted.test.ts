...
describe("useSettingsStore - Targeted Coverage", () => {
    beforeEach(() => {
        ...
        mockWithErrorHandling.mockImplementation(async (asyncFn, errorHandler) => {
            try {
    ...
    describe("Rehydration Error Handling (Line 75)", () => {
        it("should handle errors during settings sync after rehydration", async () => {
            ...
            await act(async () => {
                try {
                    const response = await mockElectronAPI.settings.getHistoryLimit();
                    const historyLimit = mockSafeExtractIpcData(response, 100) as number;
                    useSettingsStore.getState().updateSettings({ historyLimit });
                } catch (error) {
    ...
    describe("syncFromBackend Implementation (Lines 207-228)", () => {
        it("should successfully sync settings from backend", async () => {
            ...
            mockElectronAPI.settings.getHistoryLimit.mockResolvedValue(mockResponse);
            ...
            expect(mockSafeExtractIpcData).toHaveBeenCalledWith(mockResponse, 100);
        });

        it("should handle backend errors in syncFromBackend", async () => {
            const mockError = new Error("Backend sync failed");
            mockElectronAPI.settings.getHistoryLimit.mockRejectedValue(mockError);
            ...
            await expect(syncFromBackend()).rejects.toThrow("Backend sync failed");

            // Verify error handler was called
            expect(mockErrorStore.setStoreError).toHaveBeenCalledWith("settings", expect.stringContaining("Backend sync failed"));
        });
        ...
        it("should handle data extraction errors in syncFromBackend", async () => {
            ...
            await expect(syncFromBackend()).rejects.toThrow("Data extraction failed");
            expect(mockErrorStore.setStoreError).toHaveBeenCalledWith("settings", expect.stringContaining("Data extraction failed"));
        });
    });
