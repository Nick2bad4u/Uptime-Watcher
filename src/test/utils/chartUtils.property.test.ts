/**
 * @fileoverview Property-based tests for chart utilities using Fast-Check
 *
 * @remarks
 * These tests use property-based testing to verify chart utility functions
 * with comprehensive edge case coverage and randomized inputs. Tests chart
 ...
 * - Nested property navigation (getNestedScalePropertySafe, getNestedScaleProperty)
...
describe('Chart Utils Property-Based Tests', () => {
    ...
    const validChartConfigArbitrary = fc.record({
        scales: fc.record({
            x: fc.record({
                ...
                grid: fc.record({
                    display: fc.boolean(),
                    color: fc.string({ minLength: 6, maxLength: 6 }).map(s => `#${s.padStart(6, '0')}`)
                }, { requiredKeys: [] })
            }, { requiredKeys: ['type'] }),
            y: fc.record({
                ...
                ticks: fc.record({
                    stepSize: fc.float({ min: Math.fround(0.1), max: Math.fround(100), noNaN: true, noDefaultInfinity: true }),
    ...
    const invalidChartConfigArbitrary = fc.oneof(
        ...
        fc.constant({}),
        fc.oneof(fc.string(), fc.integer(), fc.boolean(), fc.float({ noNaN: true, noDefaultInfinity: true })),
        ...
        fc.record({
            scales: fc.record({
                x: fc.oneof(fc.constant(null), fc.oneof(fc.string(), fc.integer(), fc.boolean(), fc.float({ noNaN: true, noDefaultInfinity: true }))),
                y: fc.oneof(fc.constant(null), fc.oneof(fc.string(), fc.integer(), fc.boolean(), fc.float({ noNaN: true, noDefaultInfinity: true })))
            }, { requiredKeys: [] })
        })
    );
    ...
    const propertyPathArbitrary = fc.oneof(
        ...
        fc.constantFrom('title.display', 'title.text', 'grid.display', 'grid.color', 'ticks.stepSize'),
        // Deep nested paths
    ...
    describe('getScaleConfigSafe function', () => {
        ...
        it('should handle edge case axes consistently', () => {
            fc.assert(fc.property(
                fc.oneof(validChartConfigArbitrary, invalidChartConfigArbitrary),
                (config) => {
    ...
    describe('getNestedScalePropertySafe function', () => {
        it('should handle valid property paths on valid configurations', () => {
            fc.assert(fc.property(
                validChartConfigArbitrary,
                fc.constantFrom('x', 'y'),
                fc.constantFrom('type', 'display', 'title.display', 'title.text'),
                (config, axis, path) => {
        ...
        it('should handle empty and invalid property paths gracefully', () => {
            fc.assert(fc.property(
                validChartConfigArbitrary,
                fc.constantFrom('x', 'y'),
                fc.oneof(fc.constant(''), fc.constant('..'), fc.constant('invalid.path.that.does.not.exist')),
                (config, axis, path) => {
                    const result = getNestedScalePropertySafe(config, axis, path);
    ...
    describe('getNestedScaleProperty function', () => {
        it('should return property values for valid paths', () => {
            fc.assert(fc.property(
                ...
                (config, axis, path) => {
                    ...
                    const safeResult = getNestedScalePropertySafe(config, axis, path);
        ...
        it('should be consistent with getNestedScalePropertySafe', () => {
            fc.assert(fc.property(
                ...
                propertyPathArbitrary,
                (config, axis, path) => {
                    const directResult = getNestedScaleProperty(config, axis, path);
                    const safeResult = getNestedScalePropertySafe(config, axis, path);
    ...
    describe('getScaleProperty function', () => {
        it('should return property values for valid properties', () => {
            fc.assert(fc.property(
                ...
                (config, axis, property) => {
                    ...
                    if (config.scales && config.scales[axis] && property in config.scales[axis]) {
                        expect(result).toBeDefined();
        ...
        it('should handle non-existent properties gracefully', () => {
            fc.assert(fc.property(
                validChartConfigArbitrary,
                fc.constantFrom('x', 'y'),
                fc.string({ minLength: 1 }).filter(s => !['type', 'display', 'beginAtZero', 'title', 'grid', 'ticks'].includes(s)),
                (config, axis, property) => {
                    const result = getScaleProperty(config, axis, property);
            ...
            const malformedConfig = fc.record({
                scales: fc.record({
                    x: fc.oneof(
                        fc.constant(null),
                        fc.record({
                            type: fc.oneof(fc.constant(null), fc.constant(123), fc.constant({})),
                            nested: fc.record({
                                deep: fc.oneof(fc.constant(null), fc.array(fc.anything()), fc.oneof(fc.string(), fc.integer(), fc.boolean(), fc.float({ noNaN: true, noDefaultInfinity: true })))
                            })
                        })
                    )
                })
            ...
            fc.assert(fc.property(
                ...
                (config) => {
                    ...
                    expect(() => {
                        ...
                        getNestedScalePropertySafe(config, 'x', 'nested.deep.invalid');
                        getNestedScaleProperty(config, 'x', 'nested.deep.invalid');
        ...
        it('should be performant with very large configurations', () => {
            ...
            fc.assert(fc.property(
                ...
                (config) => {
                    ...
                    getScaleConfigSafe(config, 'x');
                    getNestedScalePropertySafe(config, 'x', 'property50.nested.deep');
    ...
    describe('Performance and determinism', () => {
        it('should be deterministic for same inputs', () => {
            fc.assert(fc.property(
                ...
                (config, axis, path) => {
                    ...
                    const nested1 = getNestedScalePropertySafe(config, axis, path);
                    const nested2 = getNestedScalePropertySafe(config, axis, path);
        ...
        it('should handle batch processing efficiently', () => {
            fc.assert(fc.property(
                fc.array(validChartConfigArbitrary, { minLength: 10, maxLength: 50 }),
                (configs) => {
                    const startTime = Date.now();
                    ...
                    const results = configs.map(config => ({
                        ...
                        yTitle: getNestedScaleProperty(config, 'y', 'title.text')
                    }));
