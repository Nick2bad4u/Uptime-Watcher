...
describe('Cache Utils Property-Based Tests', () => {
    ...
    describe('TypedCache basic operations', () => {
        ...
        fcTest.prop([
            fc.array(fc.tuple(fc.string(), fc.string()), { minLength: 0, maxLength: 30 })
        ...
        fcTest.prop([
            ...
            fc.array(fc.tuple(fc.string(), fc.string()), { minLength: 0, maxLength: 200 })
    ...
    describe('TTL (Time-To-Live) behavior', () => {
        ...
        ])('should use default TTL when no per-entry TTL is provided', (defaultTtl, entries) => {
        ...
        fcTest.prop([
            ...
            fc.array(fc.tuple(fc.string({ minLength: 1 }), fc.string()), { minLength: 1, maxLength: 10 })
        ])('should honor per-entry TTL over default TTL', (defaultTtl, perEntryTtl, entries) => {
    ...
    describe('LRU (Least Recently Used) eviction', () => {
        fcTest.prop([
            ...
            fc.array(fc.tuple(fc.string({ minLength: 1 }), fc.string()), { minLength: 20, maxLength: 40 })
        ])('should evict least recently used entries when maxSize is reached', (maxSize, entries) => {
        ...
        fcTest.prop([
            fc.integer({ min: 3, max: 8 })
        ])('should maintain LRU order correctly with access patterns', (maxSize) => {
            const cache = new TypedCache<string, string>({ maxSize });
    ...
    describe('Cache maintenance operations', () => {
        fcTest.prop([
            fc.array(fc.tuple(fc.string(), fc.string(), fc.integer({ min: 100, max: 2000 })), { minLength: 1, maxLength: 15 })
        ])('cleanup should remove only expired entries', (entriesWithTtl) => {
            const cache = new TypedCache<string, string>();

            // Set entries with different TTLs
            for (const [key, value, ttl] of entriesWithTtl) {
                cache.set(key, value, ttl);
            ...
            const halfMaxTtl = Math.max(...entriesWithTtl.map((entry) => entry[2])) / 2;
    ...
    describe('Application cache instances', () => {
        ...
        fcTest.prop([
            fc.array(fc.tuple(fc.string(), fc.oneof(fc.string(), fc.integer(), fc.boolean())), { minLength: 1, maxLength: 10 })
    ...
    describe('getCachedOrFetch utility', () => {
        fcTest.prop([
            fc.string(),
            fc.string()
        ])('should fetch and cache value on cache miss', async (key, value) => {
            const cache = new TypedCache<string, string>();
        ...
        fcTest.prop([
            ...
            fc.string(),
            fc.string()
        ])('should return cached value without calling fetcher on cache hit', async (key, cachedValue, fetchValue) => {
            const cache = new TypedCache<string, string>();
        ...
        ])('should handle fetcher errors correctly', async (key, ttl) => {
            const cache = new TypedCache<string, string>();
    ...
    describe('Edge cases and robustness', () => {
        ...
        ])('should handle small maxSize values', (maxSize) => {
            const cache = new TypedCache<string, string>({ maxSize });
        ...
        ])('should handle duplicate keys correctly', (keys) => {
            const cache = new TypedCache<string, number>();
