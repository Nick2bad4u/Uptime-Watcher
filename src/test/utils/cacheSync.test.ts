    ...
    describe("Property-based Tests", () => {
        ...
        test.prop([
            fc.record({
                ...
                identifier: fc.option(fc.string({ maxLength: 50 }), { nil: undefined }),
            })
        ])("should handle all valid cache invalidation data combinations", (invalidationData) => {
            setupCacheSync();
            ...
            expect(() => invalidationHandler(invalidationData)).not.toThrow();
            ...
            if (invalidationData.type === "all" || invalidationData.type === "monitor") {
        ...
        ])("should handle cache invalidation with various identifier patterns", (invalidationData) => {
            setupCacheSync();
            const invalidationHandler = mockOnCacheInvalidated.mock.calls[0]?.[0];
        ...
        test.prop([
            fc.record({
                ...
                reason: fc.oneof(
                    ...
                    fc.constant(""),
                    fc.string().map(s => s.repeat(Math.fround(Math.random() * 3) + 1)),
                ),
                identifier: fc.option(fc.string({ maxLength: 30 }), { nil: undefined }),
            })
        ])("should handle cache invalidation with various reason strings", (invalidationData) => {
            setupCacheSync();
            const invalidationHandler = mockOnCacheInvalidated.mock.calls[0]?.[0];
        ...
        ])("should handle cache clearing errors gracefully", (errorMessages) => {
        ...
        test.prop([
        ...
        ])("should handle setupCacheSync environment variations", (environment) => {
            ...
            if (!environment.hasWindow || !environment.hasElectronAPI || !environment.hasEvents) {
                expect(mockLogger.warn).toHaveBeenCalledWith(
                    "Cache invalidation events not available - frontend cache sync disabled"
                );
            }
