...
describe("siteStatus Property-based Tests", () => {
    ...
    const monitorStatusArb = fc.constantFrom("up", "down", "pending", "paused") as fc.Arbitrary<MonitorStatus>;
    const siteStatusArb = fc.constantFrom("up", "down", "pending", "paused", "mixed", "unknown") as fc.Arbitrary<SiteStatus>;
    ...
    describe(calculateSiteMonitoringStatus, () => {
        test.prop([siteForStatusArb])("should return correct monitoring status for any site configuration", (site) => {
            ...
            const monitoringCount = site.monitors.filter(m => m.monitoring).length;
        ...
        test.prop([fc.array(monitorArb, { minLength: 1, maxLength: 5 })])("should return running when all monitors are monitoring", (monitors) => {
        ...
        test.prop([fc.array(monitorArb, { minLength: 1, maxLength: 5 })])("should return stopped when no monitors are monitoring", (monitors) => {
            const site: SiteForStatus = {
                monitors: monitors.map(m => ({ ...m, monitoring: false }))
            };
    ...
    describe(calculateSiteStatus, () => {
        ...
        test.prop([monitorStatusArb, fc.integer({ min: 1, max: 5 })])("should return single status when all monitors have same status", (status, count) => {
            const site: SiteForStatus = {
                monitors: Array.from({ length: count }).map(() => ({ monitoring: true, status }))
            };
        ...
        test.prop([fc.array(monitorArb, { minLength: 2, maxLength: 5 })])("should return mixed when monitors have different statuses", (monitors) => {
            ...
            const uniqueStatuses = [...new Set(mixedMonitors.map(m => m.status))];
    ...
    describe(getSiteDisplayStatus, () => {
        test.prop([siteForStatusArb])("should return appropriate display status based on monitoring and operational status", (site) => {
            ...
            if (monitoringCount > 0 && monitoringCount < site.monitors.length) {
        ...
        test.prop([fc.array(monitorArb, { minLength: 1, maxLength: 5 })])("should return paused when no monitors are monitoring", (monitors) => {
            const site: SiteForStatus = {
                monitors: monitors.map(m => ({ ...m, monitoring: false }))
            };
    ...
    describe(getSiteStatusDescription, () => {
        test.prop([siteForStatusArb])("should return descriptive string for any site configuration", (site) => {
            ...
            if (site.monitors.length > 0) {
                expect(result).toMatch(new RegExp(site.monitors.length.toString()));
            }
        ...
        test.prop([monitorStatusArb, fc.integer({ min: 1, max: 10 })])("should include monitor count in description", (status, count) => {
            const site: SiteForStatus = {
                monitors: Array.from({length: count}).map(() => ({ monitoring: true, status }))
            };
    ...
    describe(getSiteStatusVariant, () => {
        ...
        test.prop([fc.constantFrom("up")])("should return success variant for up status", (status) => {
            const result = getSiteStatusVariant(status);
            expect(result).toBe("success");
        });

        test.prop([fc.constantFrom("down", "unknown")])("should return error variant for error states", (status) => {
            const result = getSiteStatusVariant(status);
            expect(result).toBe("error");
        });

        test.prop([fc.constantFrom("mixed", "paused")])("should return warning variant for warning states", (status) => {
            const result = getSiteStatusVariant(status);
            expect(result).toBe("warning");
        });

        test.prop([fc.constantFrom("pending")])("should return info variant for pending status", (status) => {
            const result = getSiteStatusVariant(status);
    ...
    describe("Integration Properties", () => {
        test.prop([siteForStatusArb])("getSiteDisplayStatus and getSiteStatusDescription should be consistent", (site) => {
        ...
        test.prop([siteForStatusArb])("status calculation functions should handle edge cases gracefully", (site) => {
