...
describe("Store", () => {
    ...
    describe("Advanced Backend Integration", () => {
        ...
        it("modifies site correctly", async () => {
            ...
            mockElectronAPI.sites.getSites.mockResolvedValue([{ ...mockSite, name: "Updated Name" }]);
    ...
    describe("Monitor Management", () => {
        ...
        it("handles add monitor to non-existent site", async () => {
            ...
            await expect(useStore.getState().addMonitorToSite("nonexistent", newMonitor)).rejects.toThrow(
                "Site not found"
            );
        ...
        it("handles start monitoring error", async () => {
            ...
            await expect(useStore.getState().startSiteMonitorMonitoring("site1", "monitor1")).rejects.toThrow(
                "Start failed"
            );
        ...
        it("handles stop monitoring error", async () => {
            ...
            await expect(useStore.getState().stopSiteMonitorMonitoring("site1", "monitor1")).rejects.toThrow(
                "Stop failed"
            );
        ...
        it("handles update check interval for non-existent site", async () => {
            await expect(useStore.getState().updateSiteCheckInterval("nonexistent", "monitor1", 60000)).rejects.toThrow(
                "Site not found"
            );
    ...
    describe("Settings Management", () => {
        ...
        it("handles update history limit error", async () => {
            ...
            await expect(useStore.getState().updateHistoryLimitValue(500)).rejects.toThrow("Update failed");
    ...
    describe("Data Export/Import", () => {
        ...
        it("handles download backup error", async () => {
            ...
            await expect(useStore.getState().downloadSQLiteBackup()).rejects.toThrow("Download failed");
        ...
        it("handles empty backup data", async () => {
            ...
            await expect(useStore.getState().downloadSQLiteBackup()).rejects.toThrow("No backup data received");
        });
    });
    ...
    describe("Monitor Configuration Updates", () => {
        ...
        it("should update monitor retry attempts", async () => {
            const mockSite: Site = {
                identifier: "site1",
                name: "Test Site",
                monitors: [{ id: "monitor1", type: "http", status: "up", history: [], retryAttempts: 3 }],
            };
        ...
        it("should handle site not found error for retry attempts update", async () => {
            ...
            await expect(useStore.getState().updateMonitorRetryAttempts("nonexistent", "monitor1", 5)).rejects.toThrow(
                "Site not found"
            );
        ...
        it("should update monitor timeout", async () => {
            const mockSite: Site = {
                identifier: "site1",
                name: "Test Site",
                monitors: [{ id: "monitor1", type: "http", status: "up", history: [], timeout: 5000 }],
            };
        ...
        it("should handle site not found error for timeout update", async () => {
            ...
            await expect(useStore.getState().updateMonitorTimeout("nonexistent", "monitor1", 5000)).rejects.toThrow(
                "Site not found"
            );
    ...
    describe("Status Updates Error Handling", () => {
        ...
        it("should trigger full sync when site not found in status update", async () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining("Site nonexistent not found in store"));
            expect(fullSyncSpy).toHaveBeenCalled();
        ...
        it("should handle status update processing errors", async () => {
            ...
            const fullSyncSpy = vi
                .spyOn(useStore.getState(), "fullSyncFromBackend")
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith("Error processing status update:", expect.any(Error));
            expect(consoleErrorSpy).toHaveBeenCalledWith("Fallback sync after error failed:", expect.any(Error));
        ...
        it("should handle fallback sync failure after site not found", async () => {
            ...
            const fullSyncSpy = vi
                .spyOn(useStore.getState(), "fullSyncFromBackend")
                .mockRejectedValue(new Error("Fallback sync failed"));
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith("Fallback full sync failed:", expect.any(Error));
