...
describe("Additional Coverage for Specific Lines", () => {
    ...
    describe("SiteMonitoring - Uncovered Lines", () => {
        it("should handle useSiteMonitoring lines 46-59, 76-89", async () => {
            ...
            const mockStartSiteMonitorMonitoring = vi
                .fn()
                .mockImplementation(async (siteId: string, monitorId: string) => {
                    if (!siteId || !monitorId) {
            ...
            const mockStopSiteMonitorMonitoring = vi
                .fn()
                .mockImplementation(async (siteId: string, monitorId: string) => {
                    if (!siteId || !monitorId) {
                        throw new Error("Invalid parameters");
    ...
    describe("SiteOperations - Uncovered Lines", () => {
        it("should handle useSiteOperations lines 123-127, 203-240", async () => {
            ...
            const mockUpdateMonitorTimeout = vi
                .fn()
                .mockImplementation(async (siteId: string, _monitorId: string, timeout: number) => {
                    if (timeout < 0) {
