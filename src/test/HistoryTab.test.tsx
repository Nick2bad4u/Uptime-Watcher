...
vi.mock("../theme/components", () => ({
    ThemedBox: ({
        ...
        border: _border,
        ...props
    }: React.PropsWithChildren<{ border?: boolean; [key: string]: unknown }>) => {
        return (
...
describe("HistoryTab", () => {
    ...
    describe("Details Rendering", () => {
        it("should render HTTP response code details", () => {
            const httpMonitorWithDetails = {
                ...mockMonitor,
                history: [{ timestamp: 1640995200000, status: "up" as const, responseTime: 150, details: "200" }],
            };
        ...
        it("should render port details for port monitor", () => {
            const portMonitorWithDetails = {
                ...mockPortMonitor,
                history: [{ timestamp: 1640995200000, status: "up" as const, responseTime: 150, details: "80" }],
            };
    ...
    describe("User Action Logging", () => {
        ...
        it("should not log duplicate monitor views", () => {
            ...
            expect(mockLogger.user.action).not.toHaveBeenCalledWith("History tab viewed", expect.any(Object));
        });
    ...
    /**
     * Edge cases to improve branch coverage for lines 105 and related
     * Tests the renderDetails function and getDetailLabel logic
     */
    describe("renderDetails function coverage", () => {
        it("should render nothing when details are falsy", () => {
            const historyWithoutDetails: StatusHistory[] = [{ timestamp: Date.now(), status: "up", responseTime: 100 }];

            const monitor: Monitor = {
                ...mockMonitor,
                history: historyWithoutDetails,
            };

            render(<HistoryTab {...defaultProps} selectedMonitor={monitor} />);

            // Should not render any details text
            expect(screen.queryByText(/Response Code:/)).not.toBeInTheDocument();
            expect(screen.queryByText(/Port:/)).not.toBeInTheDocument();
        });

        it("should render response code for HTTP monitor with details", () => {
            const historyWithDetails = [
                { timestamp: Date.now(), status: "up" as const, responseTime: 100, details: "200" },
            ];
            ...
            const httpMonitor: Monitor = {
                ...
                type: "http",
                url: "https://example.com",
                history: historyWithDetails as StatusHistory[],
            };

            render(<HistoryTab {...defaultProps} selectedMonitor={httpMonitor} />);

            // Should render response code
            expect(screen.getByText(/Response Code: 200/)).toBeInTheDocument();
        });

        it("should render port for port monitor with details", () => {
            const historyWithDetails = [
                { timestamp: Date.now(), status: "up" as const, responseTime: 100, details: "80" },
            ];
            ...
            render(<HistoryTab {...defaultProps} selectedMonitor={portMonitor} />);

            // Should render port
            expect(screen.getByText(/Port: 80/)).toBeInTheDocument();
        });

        it("should render plain details for unknown monitor type", () => {
            const historyWithDetails = [
                { timestamp: Date.now(), status: "up" as const, responseTime: 100, details: "custom-detail" },
            ];

            const unknownMonitor = {
                ...mockMonitor,
                type: "unknown" as unknown as MonitorType,
                history: historyWithDetails as StatusHistory[],
            } as Monitor;

            render(<HistoryTab {...defaultProps} selectedMonitor={unknownMonitor} />);

            // Should render plain details
            expect(screen.getByText("custom-detail")).toBeInTheDocument();
        });

        it("should handle unknown monitor type with null details (hits nullish coalescing)", () => {
            const historyWithNullDetails = [
                { timestamp: Date.now(), status: "up" as const, responseTime: 100, details: null as unknown as string },
            ];

            const unknownMonitor = {
                ...mockMonitor,
                type: "unknown" as unknown as MonitorType,
                history: historyWithNullDetails as StatusHistory[],
            } as Monitor;

            render(<HistoryTab {...defaultProps} selectedMonitor={unknownMonitor} />);

            // Should render empty string when details is null (tests the ?? "" part)
            const detailsElements = screen.queryAllByText("");
            // The empty string won't be visible, but the code path should be exercised
            expect(detailsElements).toBeDefined();
        });

        it("should handle empty string details", () => {
            const historyWithEmptyDetails = [
                { timestamp: Date.now(), status: "up" as const, responseTime: 100, details: "" },
            ];

            const monitor: Monitor = {
                ...mockMonitor,
                history: historyWithEmptyDetails as StatusHistory[],
            };

            render(<HistoryTab {...defaultProps} selectedMonitor={monitor} />);

            // Should not render details when empty
            expect(screen.queryByText(/Response Code:/)).not.toBeInTheDocument();
            expect(screen.queryByText(/Port:/)).not.toBeInTheDocument();
        });
    });
