 ...
 * 
 * This test specifically targets remaining uncovered functions to push
...
describe('Final 90% Function Coverage Push', () => {
  ...
  describe('Component Helper Functions', () => {
    ...
    it('should test form validation helpers', () => {
      ...
      const isUrlValid = formData.url && formData.url.startsWith('http');
      const isIntervalValid = formData.checkInterval && formData.checkInterval >= 1000;
  ...
  describe('Store Helper Functions', () => {
    it('should test store action helpers', () => {
      ...
      expect(mockStore.actions.addItem).toHaveBeenCalledWith({ id: 1, name: 'test' });
      expect(mockStore.actions.updateItem).toHaveBeenCalledWith(1, { name: 'updated' });
    ...
    it('should test store selector functions', () => {
      ...
      const siteById = (id: number) => state.sites.find(site => site.id === id);
      const getTheme = () => state.settings.theme;

      expect(enabledSites).toHaveLength(1);
      expect(siteById(1)).toEqual({ id: 1, name: 'site1', enabled: true });
  ...
  describe('Utility Helper Functions', () => {
    it('should test date and time helpers', () => {
      ...
      const isValidDate = (timestamp: number) => !isNaN(timestamp) && timestamp > 0;
    ...
    it('should test string utility functions', () => {
      ...
      expect(sanitize('hello<script>world</script>')).toBe('helloscriptworld/script');
    });
  ...
  describe('Error Handling Functions', () => {
    ...
    it('should test async error handling', async () => {
      ...
      const safeAsyncExecute = async <T>(fn: () => Promise<T>, fallback: T): Promise<T> => {
        try {
          return await fn();
        } catch {
          return fallback;
      ...
      await expect(safeAsyncExecute(asyncThrow, 'async fallback')).resolves.toBe('async fallback');
      await expect(safeAsyncExecute(asyncReturn, 'async fallback')).resolves.toBe('async success');
    });
  });
  ...
  describe('Configuration Helper Functions', () => {
    it('should test configuration utilities', () => {
      ...
      const mergeConfig = <T extends Record<string, any>>(base: T, override: Partial<T>): T => ({
        ...base,
        ...override
      });

      const validateConfig = <T>(config: T, required: (keyof T)[]): boolean => {
      ...
      const required: ('timeout' | 'retries' | 'enabled')[] = ['timeout', 'retries'];
    ...
    it('should test environment detection functions', () => {
      ...
      const getBrowserInfo = () => ({
        userAgent: typeof navigator === 'undefined' ? 'Node.js' : navigator.userAgent,
        platform: typeof navigator === 'undefined' ? 'node' : navigator.platform
      });
