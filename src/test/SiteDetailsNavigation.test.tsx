...
vi.mock("../theme/components", () => ({
    ...
    >) => (
        <span data-testid="themed-text" data-size={size} data-variant={variant} className={className} {...props}>
            {children}
        </span>
    ),
}));
...
describe("SiteDetailsNavigation", () => {
    ...
    describe("Tab Navigation", () => {
        ...
        it("should call logger when Settings tab is clicked (line 171 coverage)", async () => {
            ...
            expect(mockHandlers.setActiveSiteDetailsTab).toHaveBeenCalledWith("settings");
            expect(loggerSpy).toHaveBeenCalledWith("Site details tab changed", {
        ...
        it("should handle tab switching with monitor changes", async () => {
            const { rerender } = render(
                <SiteDetailsNavigation
                    {...defaultProps}
                    selectedMonitorId="http"
                    activeSiteDetailsTab="http-analytics"
                />
            );
            ...
            rerender(
                <SiteDetailsNavigation
                    {...defaultProps}
                    selectedMonitorId="ping"
                    activeSiteDetailsTab="ping-analytics"
                />
            );
