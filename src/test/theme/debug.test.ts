...
describe('Debug Availability Description', () => {
    it('should debug the implementation logic', () => {
        ...
        console.log('useAvailabilityColors function:', useAvailabilityColors.toString().slice(0, 500));
        
        // Test all percentage boundary conditions
        const testCases = [100, 99.9, 99.5, 99, 98.5, 98, 95.5, 95, 94.5, 90.5, 90, 85, 80, 60, 50, 30, 0];
        
        console.log('Complete test of all boundaries:');
        for (const percentage of testCases) {
            const result_desc = result.current.getAvailabilityDescription(percentage);
