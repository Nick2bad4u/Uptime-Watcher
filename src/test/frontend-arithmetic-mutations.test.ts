 ...
 * in frontend components that survived Stryker mutation testing.
 * 
...
describe("Frontend ArithmeticOperator Mutation Tests", () => {
    ...
    describe("ScreenshotThumbnail Positioning Calculations", () => {
        it("should calculate maximum image width using multiplication (kills viewportW / 0.9 mutant)", async ({ task, annotate }) => {
        ...
        it("should calculate maximum image height using multiplication (kills viewportH / 0.9 mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            await annotate("Component: ScreenshotThumbnail", "component");
            await annotate("Category: UI Layout", "category");
            await annotate("Type: Arithmetic", "type");
        ...
        it("should position overlay with correct gap using subtraction (kills rect.top - overlayH + 16 mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            await annotate("Component: ScreenshotThumbnail", "component");
            await annotate("Category: UI Layout", "category");
            await annotate("Type: Arithmetic", "type");
        ...
        it("should center overlay horizontally (kills various arithmetic mutants)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            await annotate("Component: ScreenshotThumbnail", "component");
            await annotate("Category: UI Layout", "category");
            await annotate("Type: Arithmetic", "type");
        ...
        it("should position overlay below when above doesn't fit (kills rect.bottom - 16 mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            await annotate("Component: ScreenshotThumbnail", "component");
            await annotate("Category: UI Layout", "category");
            await annotate("Type: Arithmetic", "type");
        ...
        it("should check horizontal overflow correctly (kills left - overlayW and viewportW + 8 mutants)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            await annotate("Component: ScreenshotThumbnail", "component");
            await annotate("Category: UI Layout", "category");
            await annotate("Type: Arithmetic", "type");
            ...
            const mutant2Overflow2 = overlayRight2 > mutant2Limit; // 1050 > 1008 = true
            const correctOverflow2 = overlayRight2 > viewportLimit; // 1050 > 992 = true
            ...
            const correctOverflowEdge = overlayRightEdge > viewportLimit; // 1036 > 992 = true
            const mutant2OverflowEdge = overlayRightEdge > mutant2Limit; // 1036 > 1008 = true
            
            // Find a case where they differ
            const leftDiff = 920;
            const overlayRightDiff = leftDiff + overlayW; // 1020
            const correctOverflowDiff = overlayRightDiff > viewportLimit; // 1020 > 992 = true
            const mutant2OverflowDiff = overlayRightDiff > mutant2Limit; // 1020 > 1008 = true
        ...
        it("should check vertical overflow correctly (kills top - overlayH and viewportH + 8 mutants)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            await annotate("Component: ScreenshotThumbnail", "component");
            await annotate("Category: UI Layout", "category");
            await annotate("Type: Arithmetic", "type");
    ...
    describe("AnalyticsTab Time Formatting", () => {
        it("should format time using division (kills ms * 1000 mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
    ...
    describe("HistoryTab Index Calculation", () => {
        it("should calculate history index using subtraction (kills historyLength + findIndex mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
    ...
    describe("SiteOverviewTab Monitor Counts", () => {
        it("should calculate stopped monitors using subtraction (kills total + running mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
    ...
    describe("Site Analytics Loop Index", () => {
        it("should iterate backwards through history (kills filteredHistory.length + 1 mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            ...
            const filteredHistory = ["item1", "item2", "item3"];
            ...
            expect(mutatedStartIndex).toBeGreaterThanOrEqual(filteredHistory.length);
            expect(filteredHistory[mutatedStartIndex]).toBeUndefined();
        });

        it("should calculate safe array index (kills arrayLength + 1 mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            await annotate("Component: useSiteAnalytics", "component");
            await annotate("Category: Data Processing", "category");
            await annotate("Type: Arithmetic", "type");
            ...
            const correctSafeIndex = Math.max(0, Math.min(index, arrayLength - 1));
            expect(correctSafeIndex).toBe(4); // Clamped to last valid index
            
            // Mutated calculation would use arrayLength + 1
            const mutatedSafeIndex = Math.max(0, Math.min(index, arrayLength + 1));
    ...
    describe("ErrorBoundary Retry Counter", () => {
        it("should increment retry count (kills prevState.retryCount - 1 mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
    ...
    describe("Store Utils Exponential Backoff", () => {
        it("should calculate exponential delay (kills baseDelay / 1.5 ** attempt mutant)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
    ...
    describe("ThemedProgress Percentage Calculation", () => {
        it("should calculate percentage using division and multiplication (kills various arithmetic mutants)", async ({ task, annotate }) => {
            await annotate(`Testing: ${task.name}`, "functional");
            ...
            const smallValue = 30;
            const smallMax = 100;
            const correctSmall = Math.min(Math.max((smallValue / smallMax) * 100, 0), 100);
            expect(correctSmall).toBe(30);
            
            const mutant1Small = Math.min(Math.max(smallValue / smallMax / 100, 0), 100);
