...
vi.mock("../theme/components", () => ({
  ...
  ThemedButton: ({ children, onClick, ...safeProps }: React.PropsWithChildren<{ onClick?: () => void } & Record<string, unknown>>) => {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { showLabel, iconColor, hoverable, showText, loading, variant, size, ...props } = safeProps;
  ...
  ThemedSelect: ({ children, onChange, ...safeProps }: React.PropsWithChildren<{ onChange?: (e: React.ChangeEvent<HTMLSelectElement>) => void } & Record<string, unknown>>) => {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { showLabel, iconColor, hoverable, showText, loading, variant, size, ...props } = safeProps;
    return <select onChange={onChange} {...props}>{children}</select>;
  },
  ThemedInput: ({ onChange, ...safeProps }: { onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void } & Record<string, unknown>) => {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { showLabel, iconColor, hoverable, showText, loading, variant, size, ...props } = safeProps;
...
describe("SiteDetails Component Coverage", () => {
  describe("SiteDetails", () => {
    ...
    it("should handle keyboard navigation", { timeout: 5000 }, async () => {
      ...
      const siteDetailsContainer = screen.getByText("Test Site").closest("[data-testid]") || screen.getByText("Test Site").closest("div");
