...
describe("SiteCardMetrics", () => {
    it("renders all metrics with correct values", () => {
        ...
        expect(
            cards.find((card) => card.getAttribute("data-label") === "Status")
        ).toHaveAttribute("data-value", "UP");
        expect(
            cards.find((card) => card.getAttribute("data-label") === "Uptime")
        ).toHaveAttribute("data-value", "99.5%");
        expect(
            cards.find((card) => card.getAttribute("data-label") === "Response")
        ).toHaveAttribute("data-value", "123 ms");
        expect(
            cards.find((card) => card.getAttribute("data-label") === "Checks")
