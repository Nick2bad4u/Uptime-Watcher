...
import type { Monitor, MonitorStatus, StatusUpdate } from "@shared/types";
...
/**
 * Helper converting a {@link StatusUpdate} into an alert input payload.
 *
 * @param update - Status update received from the backend.
 *
 * @returns Normalized alert input ready for enqueueing.
 */
const deriveSiteName = (update: StatusUpdate): string => {
...
const deriveAlertTimestamp = (timestamp: string): number => {
    const parsed = Date.parse(timestamp);
    return Number.isNaN(parsed) ? Date.now() : parsed;
};

export const mapStatusUpdateToAlert = (
    update: StatusUpdate
): StatusAlertInput => {
