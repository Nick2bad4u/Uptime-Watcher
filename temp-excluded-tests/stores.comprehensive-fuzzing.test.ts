    ...
    describe("Settings Store", () => {
        ...
        fcTest.prop([fc.integer({ min: 100, max: 10_000 })])(
            ...
            async (historyLimit) => {
                ...
                expect(typeof result.current.updateHistoryLimitValue).toBe(
                    "function"
                );

                // Test history limit update
                await expect(
                    result.current.updateHistoryLimitValue(historyLimit)
                ).resolves.toBeDefined();
            }
        );
    ...
    describe("Store Integration Tests", () => {
        fcTest.prop([arbitraryPartialSettings, fc.string()])(
            ...
            (settings, errorMessage) => {
                const utils = renderHook(() => useSettingsStore());
                const view = renderHook(() => useErrorStore());
                ...
                expect(() => {
                    ...
                    utils.result.current.updateSettings(cleanSettings);
                    view.result.current.setError(errorMessage);
                }).not.toThrow();

                // Both stores should maintain their state independently
                expect(utils.result.current.settings).toBeDefined();
                expect(view.result.current.lastError).toBe(errorMessage);
            }
        );
        ...
        fcTest.prop([fc.boolean(), fc.integer({ min: 0, max: 100 })])(
            ...
            (isChecking, progress) => {
                const { result: updatesResult } = renderHook(() =>
                    useUpdatesStore()
                );
                ...
                expect(() => {
                    updatesResult.current.setCheckingForUpdates(isChecking);
                    updatesResult.current.setDownloadProgress(progress);
                    monitorResult.current.setLoading(isChecking);
                }).not.toThrow();

                // Both stores should work independently
                expect(updatesResult.current.checkingForUpdates).toBe(
                    isChecking
                );
                expect(updatesResult.current.downloadProgress).toBe(progress);
                expect(monitorResult.current.isLoading).toBe(isChecking);
            }
        );
    });
    ...
    describe("Store State Consistency", () => {
        fcTest.prop([arbitraryPartialSettings])(
            ...
            (settings) => {
                ...
                firstResult.current.updateSettings(cleanSettings);
                ...
                expect(secondResult.current.settings).toBeDefined();
                expect(typeof secondResult.current.settings).toBe("object");
            }
        );
