...
describe(prefixBareMarkdownFileLinksInMarkdown, () => {
    it("prefixes bare .md links with ./", () => {
        expect(prefixBareMarkdownFileLinksInMarkdown("See [x](foo.md).")).toBe(
            "See [x](./foo.md)."
        );
    });
    ...
    it("does not change absolute/schemed links", () => {
        ...
        expect(
            prefixBareMarkdownFileLinksInMarkdown("[x](vscode:foo.md)")
        ).toBe("[x](vscode:foo.md)");
        expect(prefixBareMarkdownFileLinksInMarkdown("[x](/foo.md)")).toBe(
            "[x](/foo.md)"
        );
        expect(prefixBareMarkdownFileLinksInMarkdown("[x](#section)")).toBe(
            "[x](#section)"
        );
    });
    ...
    it("preserves link titles and only prefixes the destination", () => {
        expect(
            prefixBareMarkdownFileLinksInMarkdown('[x](foo.md "Title")')
        ).toBe('[x](./foo.md "Title")');
    });

    it("handles parentheses inside destinations", () => {
        expect(prefixBareMarkdownFileLinksInMarkdown("[x](foo(bar).md)")).toBe(
            "[x](./foo(bar).md)"
        );
    });
    ...
    it("does not rewrite inside inline code spans", () => {
        expect(
            prefixBareMarkdownFileLinksInMarkdown("`[x](foo.md)` [y](bar.md)")
        ).toBe("`[x](foo.md)` [y](./bar.md)");
    });

    it("does not rewrite random text containing ]( without a matching [label]", () => {
        expect(
            prefixBareMarkdownFileLinksInMarkdown("not-a-link ](foo.md)")
