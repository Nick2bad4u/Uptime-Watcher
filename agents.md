...
* __Frontend__: React + TypeScript + Zustand + TailwindCSS + Vite.
* __Backend__: Electron main (Node.js) with `node-sqlite3-wasm` repositories wrapped by `executeTransaction()`.
* __IPC__: Renderer services (`src/services/*`) wrap the secure contextBridge.
* __State__: Domain-scoped Zustand stores; avoid global shared state.
* __Events__: TypedEventBus with middleware, correlation IDs, and contracts.
* __Logging__: Centralized structured logging with level support.
* __Security__: Hardened IPC, rigorous input validation, Electron best practices.
* __Documentation__: Comprehensive TSDoc across architecture/features.
