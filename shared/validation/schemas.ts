...
export function validateMonitorData(type: string, data: unknown): ValidationResult {
  try {
    ...
    const schema = type === "http" ? httpMonitorSchema : 
                   type === "port" ? portMonitorSchema : 
                   null;
  ...
  try {
    ...
    if (fieldName === "url" && type === "http") {
      ...
      if (fieldName in commonFields) {
        fieldValidationResult = z.object({ [fieldName]: commonFields[fieldName as keyof typeof commonFields] }).parse(testData);
      } else {
