/**
 * @fileoverview Fast-Check fuzzing tests for validation schemas targeting uncovered lines
 * @module shared/test/fuzzing/schemas
 */

import { describe, expect, it } from "vitest";
import { fc, test } from "@fast-check/vitest";
import { validateMonitorData, validateMonitorField, validateSiteData } from "@shared/validation/schemas";
    ...
    describe("validateMonitorField - Line 404 Unknown field error", () => {
        test.prop([fc.string(), fc.anything()])(
            ...
            (fieldName, value) => {
                ...
                if (!["name", "type", "interval", "timeout", "retries", "url", "port", "host", "method", "headers", "body", "followRedirects", "maxRedirects", "validateSSL"].includes(fieldName)) {
                    expect(() => validateMonitorField("http", fieldName, value)).toThrow(/Unknown field/);
                }
            }
        );
        ...
        it("should trigger unknown field error for completely invalid fields", () => {
            ...
            expect(() => validateMonitorField("http", "totallyInvalidFieldName", "value")).toThrow("Unknown field: totallyInvalidFieldName");
        });
    });
