 ...
 * ```
 *
 * Targets uncovered line 31: `return error instanceof Error ? error : new
 * Error(String(error));` This line handles conversion of non-Error values to
 * Error instances in ensureError function.
 *
 * The ensureError function is internal and used by safeOperation, which is used
 * by all public JSON parsing functions. We need to trigger scenarios where
 * non-Error values are thrown and caught, forcing execution through the
 * String(error) path.
 */
...
test.prop([
    ...
    "safeJsonParse handles malformed JSON through ensureError conversion",
    (value) => {
...
test.prop([
    ...
    "safeJsonParseArray handles validation failures through ensureError",
    (nonArrayValue) => {
...
/**
 * Test safeJsonStringify with values that cause serialization errors Targets
 * ensureError function through JSON.stringify failures
 */
test.prop([
    fc.anything({
        maxDepth: 3,
    ...
    "safeJsonStringify handles serialization errors through ensureError",
    (problematicValue) => {
...
/**
 * Test edge case scenarios that trigger the ensureError String(error)
 * conversion Specifically targeting circular references and non-serializable
 * values
 */
test.prop([
    fc.oneof(
    ...
    "Edge cases trigger ensureError String conversion for uncommon values",
    (problematicValue) => {
        const result = safeJsonStringify(problematicValue);
        ...
        if (typeof problematicValue === "function") {
            ...
            expect(result.error).toMatch(
                /JSON stringification failed:|Value cannot be serialized to JSON/
            );
        }
    }
);
...
/**
 * Test circular reference objects that cause JSON.stringify to fail Forces
 * ensureError to handle TypeError from circular reference detection
 */
...
/**
 * Test malformed JSON strings that trigger various parser error types Ensures
 * ensureError handles different SyntaxError variations consistently
 */
test.prop([
    fc.string({ minLength: 1, maxLength: 50 }).filter((str) => {
        try {
            JSON.parse(str);
    ...
    "Malformed JSON strings trigger consistent error handling through ensureError",
    (malformedJson) => {
        const alwaysTrue = (_data: unknown): _data is unknown => true;
...
/**
 * Test safeJsonParse with custom validator that throws TypeError This
 * specifically targets the TypeError branch in ensureError handling
 */
test.prop([
    fc.json(), // Valid JSON strings
])(
    "Custom validator TypeErrors are handled correctly by ensureError",
    (validJson) => {
