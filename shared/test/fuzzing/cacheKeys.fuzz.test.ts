/**
 * Fuzzing tests for cacheKeys module using property-based testing
 * Targets functions f:11 and f:12 to improve coverage
 */

import { describe, test, expect } from 'vitest';
import * as fc from 'fast-check';
import { isStandardizedCacheKey, parseCacheKey } from '../../utils/cacheKeys.js';
...
describe('cacheKeys fuzzing tests', () => {
  ...
  const validCacheKeyPart = fc.string({ minLength: 1, maxLength: 50 })
    .filter(s => !s.includes(':') && s.trim().length > 0 && s.trim() === s);
