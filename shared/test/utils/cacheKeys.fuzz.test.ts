 ...
 *
 * @fileoverview
 * This file contains extensive fuzzing tests to achieve 100% test coverage
 * of the cache key utilities. Uses fast-check to generate edge cases
...
const VALID_CACHE_PREFIXES = ["config", "monitor", "site", "validation"] as const;

// Create arbitraries for valid cache key parts (no colons, no empty/whitespace-only strings)
const validCacheKeyPart = fc.string({ minLength: 1, maxLength: 50 }).filter(s =>
    s.length > 0 &&
    !s.includes(':') &&
    s.trim().length > 0 &&
    s.trim() === s
...
const shortValidCacheKeyPart = fc.string({ minLength: 1, maxLength: 20 }).filter(s =>
    s.length > 0 &&
    !s.includes(':') &&
    s.trim().length > 0 &&
    s.trim() === s
    ...
    describe("CacheKeys.config functions", () => {
        ...
        it("should handle edge cases in config key names", () => {
            fc.assert(
                fc.property(
                    fc.oneof(
                        fc.string({ minLength: 1, maxLength: 1 }).filter(s => !s.includes(":")), // Single character
                        fc.string().filter(s => /\s/.test(s) && !s.includes(":")), // String with whitespace but no colons
                        fc.string({ minLength: 1, maxLength: 100 }).filter(s => !s.includes(":")), // Long string
            ...
            fc.assert(
                fc.property(
                    fc.string({ minLength: 1, maxLength: 20 }).filter(s =>
                        !VALID_CACHE_PREFIXES.includes(s as any)
                    ),
        ...
        it("should correctly parse 2-part cache keys", () => {
            fc.assert(
                fc.property(
                    ...
                    validCacheKeyPart,
                    (prefix, identifier) => {
                        const key = `${prefix}:${identifier}` as StandardizedCacheKey;
        ...
        it("should correctly parse 3-part cache keys", () => {
            fc.assert(
                fc.property(
                    ...
                    validCacheKeyPart,
                    (prefix, operation, identifier) => {
                        const key = `${prefix}:${operation}:${identifier}` as StandardizedCacheKey;
                        const result = parseCacheKey(key);
                ...
                fc.property(
                    ...
                    (prefix, identifier) => {
                        ...
                        if (identifier === "" || identifier !== identifier.trim()) {
                            ...
                            expect(() => parseCacheKey(key as StandardizedCacheKey)).toThrow();
                        } else {
                            // Valid long identifiers should parse correctly
                            const result = parseCacheKey(key as StandardizedCacheKey);
                            expect(result.prefix).toBe(prefix);
                            expect(result.identifier).toBe(identifier);
                            expect(result.operation).toBeUndefined();
                        }
                ...
                fc.property(
                    ...
                    (configName, monitorId, siteId, validationType) => {
                        ...
                        const keys = [
                            ...
                            CacheKeys.validation.byType(validationType, "test-id"),
            ...
            fc.assert(
                fc.property(
                    ...
                    (identifier) => {
                        ...
                        const monitorParsed = parseCacheKey(monitorKey as StandardizedCacheKey);
                        const siteParsed = parseCacheKey(siteKey as StandardizedCacheKey);
