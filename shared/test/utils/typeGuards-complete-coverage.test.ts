/**
 * Comprehensive tests for type guards utility functions to achieve 95%+ coverage.
 * Tests all type guard functions with various input types and edge cases.
 */
...
describe('TypeGuards - Complete Function Coverage', () => {
	...
	describe('hasProperties', () => {
		it('should return true when object has all properties', () => {
	...
	describe('isArray', () => {
		...
		it('should validate array items with validator', () => {
			...
			const isNumberValidator = (item: unknown): item is number => typeof item === 'number';

			expect(isArray(['a', 'b', 'c'], isStringValidator)).toBe(true);
			expect(isArray([1, 2, 3], isNumberValidator)).toBe(true);
			expect(isArray(['a', 1, 'c'], isStringValidator)).toBe(false);
			expect(isArray([1, 'b', 3], isNumberValidator)).toBe(false);
		});

		it('should handle empty arrays with validator', () => {
			const isStringValidator = (item: unknown): item is string => typeof item === 'string';
	...
	describe('isValidTimestamp', () => {
		...
		it('should return true for timestamps up to 1 day in future', () => {
			expect(isValidTimestamp(now + oneDayInMs)).toBe(true);
			expect(isValidTimestamp(now + oneDayInMs - 1000)).toBe(true);
		});

		it('should return false for timestamps too far in future', () => {
	...
	describe('Integration tests', () => {
		it('should handle complex object validation', () => {
			...
			expect(hasProperties(complexObj, ['id', 'name', 'active'])).toBe(true);
