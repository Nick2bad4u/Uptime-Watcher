    ...
    describe("Property-based tests for object safety", () => {
        test("should safely access object properties with various inputs", () => {
            fc.assert(fc.property(
                ...
                fc.anything(),
                (obj, key, fallback) => {
                    const result = objectSafetyModule.safeObjectAccess(obj, key, fallback);

                    if (Object.hasOwn(obj, key)) {
                        expect(result).toBe(obj[key] === undefined ? fallback : obj[key]);
                    } else {
        ...
        test("should handle null and undefined objects safely", () => {
            fc.assert(fc.property(
                ...
                (nullishObj, key, fallback) => {
                    const result = objectSafetyModule.safeObjectAccess(nullishObj, key, fallback);
                    expect(result).toBe(fallback);
                }
            ));
        });
        ...
        test("should safely iterate over any object structure", () => {
            fc.assert(fc.property(
                ...
                (obj) => {
                    ...
                    // Should not throw
                    expect(() => {
                        objectSafetyModule.safeObjectIteration(obj, (key, value) => {
        ...
        test("should handle non-object inputs gracefully in iteration", () => {
            fc.assert(fc.property(
                ...
                (nonObject) => {
                    ...
                    expect(() => {
                        objectSafetyModule.safeObjectIteration(nonObject, (key, value) => {
                            collected.push([key, value]);
                        });
                    }).not.toThrow();
        ...
        test("should omit specified keys consistently", () => {
            fc.assert(fc.property(
                ...
                (obj, keysToOmit) => {
                    const result = objectSafetyModule.safeObjectOmit(obj, keysToOmit);
        ...
        test("should handle null/undefined objects in omit", () => {
            fc.assert(fc.property(
                ...
                fc.array(fc.string()),
                (nullishObj, keys) => {
                    const result = objectSafetyModule.safeObjectOmit(nullishObj, keys as any);
        ...
        test("should pick specified keys consistently", () => {
            fc.assert(fc.property(
                ...
                (obj, keysToPick) => {
                    const result = objectSafetyModule.safeObjectPick(obj, keysToPick);
        ...
        test("should handle property access with validators", () => {
            fc.assert(fc.property(
                ...
                (obj) => {
                    // String validator
                    const stringResult = objectSafetyModule.safeObjectAccess(
                        obj,
                        'stringProp',
                        'fallback',
                    ...
                    // Number validator
                    const numberResult = objectSafetyModule.safeObjectAccess(
                        obj,
                        'numberProp',
                        0,
                    ...
                    // Boolean validator
                    const booleanResult = objectSafetyModule.safeObjectAccess(
                        obj,
                        'booleanProp',
                        false,
        ...
        test("should handle complex nested object structures", () => {
            fc.assert(fc.property(
                ...
                (complexObj) => {
                    ...
                    const level1 = objectSafetyModule.safeObjectAccess(complexObj, 'level1', {});
                    expect(level1).toBe(complexObj.level1);

                    // Test omitting nested structures
                    const withoutLevel1 = objectSafetyModule.safeObjectOmit(complexObj, ['level1']);
                    expect(withoutLevel1).not.toHaveProperty('level1');
                    expect(withoutLevel1).toHaveProperty('topLevel');

                    // Test picking nested structures
                    const onlyLevel1 = objectSafetyModule.safeObjectPick(complexObj, ['level1']);
