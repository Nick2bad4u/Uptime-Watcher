...
const baseMonitorArbitrary = fc.record({
    ...
    history: fc.array(fc.record({
        ...
        details: fc.option(fc.string({ maxLength: 500 }), { nil: undefined }),
    }), { maxLength: 100 }),
    lastChecked: fc.option(fc.integer({ min: Date.parse('2020-01-01'), max: Date.parse('2030-01-01') }).map(timestamp => new Date(timestamp)), { nil: undefined }),
    activeOperations: fc.option(fc.array(fc.string(), { maxLength: 50 }), { nil: undefined }),
});
...
const validHostArbitrary = fc.oneof(
...
const dnsMonitorArbitrary = fc.record({
    ...
    activeOperations: fc.option(fc.array(fc.string(), { maxLength: 50 }), { nil: undefined }),
    host: validHostArbitrary,
    recordType: fc.constantFrom('A', 'AAAA', 'ANY', 'CAA', 'CNAME', 'MX', 'NAPTR', 'NS', 'PTR', 'SOA', 'SRV', 'TLSA', 'TXT'),
});

const monitorArbitrary = fc.oneof(httpMonitorArbitrary, portMonitorArbitrary, pingMonitorArbitrary, dnsMonitorArbitrary);

const siteArbitrary = fc.record({
    identifier: fc.string({ minLength: 1, maxLength: 100 }).filter(s => /^[\w-]+$/.test(s) && s.trim().length > 0),
...
describe('Schema Property-Based Tests', () => {
    describe('baseMonitorSchema', () => {
        test.prop([baseMonitorArbitrary])(
            ...
            (monitorData) => {
                ...
                if (result.success) {
                    ...
                    expect(result.data.type).toBeOneOf(['http', 'port', 'ping', 'dns']);
                    expect(result.data.status).toBeOneOf(['up', 'down', 'pending', 'paused']);
                    expect(result.data.monitoring).toBeTypeOf('boolean');
                    expect(result.data.checkInterval).toBeGreaterThanOrEqual(5000);
                    expect(result.data.checkInterval).toBeLessThanOrEqual(2_592_000_000);
        ...
        test('should round-trip through parsing and serialization', () => {
            ...
            if (parseResult.success) {
                ...
                if (deserialized.lastChecked && typeof deserialized.lastChecked === 'string') {
                    deserialized.lastChecked = new Date(deserialized.lastChecked);
                }
    ...
    describe('portMonitorSchema', () => {
        ...
        test('should reject invalid port numbers', () => {
            ...
            const invalidPorts = [0, -1, 65_536, 999_999, Number.NaN, Number.POSITIVE_INFINITY];
                ...
                if (result.success) {
                    ...
                    switch (result.data.type) {
                        ...
                        default: {
                            ...
                            throw new Error(`Unknown monitor type: ${(exhaustiveCheck as { type: string }).type}`);
                        }
                    }
                }
            }
