...
describe('Final 100% Coverage Tests', () => {
	...
	describe('schemas - Lines 399,482', () => {
		it('should handle field validation error at line 399', () => {
			...
			const result = validateMonitorField('invalidType' as any, 'someField', 'someValue');
			expect(result).toEqual({
				errors: ['Unknown monitor type: invalidType'],
				metadata: { fieldName: 'someField', monitorType: 'invalidType' },
				success: false,
				warnings: []
			});
		});
		...
		it('should handle validation error categorization at line 482', () => {
			...
			const result = validateMonitorField('http', 'nonExistentField' as any, 'someValue');
			expect(result).toEqual({
				errors: ['Field validation failed: Unknown field: nonExistentField'],
				metadata: { fieldName: 'nonExistentField', monitorType: 'http' },
				success: false,
				warnings: []
			});
		});
