...
// NOTE: This file intentionally does not wrap all tests in a single outer test
// suite callback to avoid tripping max-lines-per-function.
...
describe("httpMonitorSchema", () => {
    ...
    it("should accept both HTTP and HTTPS URLs", async ({ task, annotate }) => {
        await annotate(`Testing: ${task.name}`, "functional");
        await annotate("Component: schemas", "component");
        await annotate("Category: Validation", "category");
        await annotate("Type: Business Logic", "type");
...
describe(validateMonitorField, () => {
    ...
    it("should handle field validation errors", async ({ task, annotate }) => {
        await annotate(`Testing: ${task.name}`, "functional");
        await annotate("Component: schemas", "component");
        await annotate("Category: Validation", "category");
        await annotate("Type: Error Handling", "type");
...
describe("Type exports", () => {
    ...
    it("should properly type ping monitors", async ({ task, annotate }) => {
        ...
        await fc.assert(
            fc.property(pingMonitorArbitrary, (pingMonitor: PingMonitor) => {
                const parsed = pingMonitorSchema.parse(pingMonitor);
                expect(parsed.type).toBe("ping");
                expect(parsed.host).toBe(pingMonitor.host);
                expect(parsed.retryAttempts).toBe(pingMonitor.retryAttempts);
            })
        );
    });

    it("should properly type generic monitors", async ({ task, annotate }) => {
        await annotate(`Testing: ${task.name}`, "functional");
        await annotate("Component: schemas", "component");
        await annotate("Category: Validation", "category");
        await annotate("Type: Monitoring", "type");
...
describe("Missing Branch Coverage", () => {
    ...
    describe("Edge cases for branch coverage", () => {
        ...
        it("should handle non-Error objects in validateSiteData", async ({
            ...
            expect(result.success).toBeFalsy();
            expect(result.errors[0]).toContain("Site validation string error");
...
describe("replicationMonitorSchema", () => {
    it("should validate replication monitor", async ({ task, annotate }) => {
        await annotate(`Testing: ${task.name}`, "functional");
        await annotate("Component: schemas", "component");
        await annotate("Category: Validation", "category");
        await annotate("Type: Validation", "type");
