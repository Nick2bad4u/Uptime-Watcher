/**
 * @fileoverview Tests to reach 100% coverage for schemas.ts lines 399 and 482
 * Targeting the field validation error and validation error categorization
 */

import { describe, expect, test } from "vitest";
import { validateMonitorField, validateMonitorData } from "../../validation/schemas.js";
...
describe("Schemas - 100% Coverage Tests", () => {
	describe("Targeting Lines 399,482", () => {
		test("should handle field validation error at line 399", () => {
			...
			expect(result.errors).toContain("Field validation failed: Unknown field: unknownField");
		});
		...
		test("should trigger optional field warning classification", () => {
			...
			const result = validateMonitorData("http", dataWithUndefinedOptionalField);
